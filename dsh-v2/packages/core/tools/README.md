...
- `ToolDefinition` — `ToolSchema` + mandatory `output { schema, render, presentationMeta? }` + `execute(args, exec)`, optional final-content and presentation callbacks, cooperative `timeoutMs`, and optional per-call `isConcurrencySafe(args)` classification. A body returns only the canonical JSON value declared by the output schema and cooperatively stops through `exec.signal`. `finalizeContent(exec, result)` runs exactly once for every normalized result, including failures that bypass post-policy, and can replace only `content`; it must be synchronous and total. An optional `projectSchema({ agent })` returns replacement model-facing `description`/`parameters` for the exact live agent, or `undefined` to keep the static schema — at the authoring layer the `parameters` are the same DSL property map the static field uses, and `defineTool` compiles them into the object-rooted JSON Schema (the raw `ToolDefinition.projectSchema` always exposes the compiled form); it drives `schemasFor(agent)` and every system-prompt/Code Mode assembly that carries a real agent, never the static `schemas(scope)` diagnostic projection.
- `ctx.tools.schemas(scope?)` is the STATIC projection (deep-cloned, never invokes `projectSchema`) — for diagnostics, catalogs, and tools that enumerate the surface; `ctx.tools.schemasFor(agent)` is the per-agent projection model-bound surfaces use. A tool's `output.render(args, value, context?)` may receive an optional render context carrying the calling `agent` for deterministic per-call decisions (e.g. whether to attach escalation guidance) without side effects.
...
- `PreToolDecision` — `{kind:'allow'}` | `{kind:'deny', reason, code?}` | `{kind:'ask', reason?}`. Input rewrite is deliberately not offered; `ask` is serviced by [`ctx.approval`](../../interaction/user-approval/README.md) when mounted and otherwise degrades to deny. An approval-denied call carries a stable `TOOL_APPROVAL_*` code (`TOOL_APPROVAL_BLOCKED` / `TOOL_APPROVAL_REJECTED` / `TOOL_APPROVAL_CANCELLED` / `TOOL_APPROVAL_UNAVAILABLE`) on the resulting `ToolFailure.info`; a pre-execute `deny` without its own `code`, or a monotonic `tools.guard()` denial, defaults to `TOOL_POLICY_DENIED`.
- `PostToolDecision` — accept may replace `content` or `value`, never both, and may attach `additionalContexts`; block turns feedback into a valueless failure carrying `TOOL_POST_BLOCKED`. Content replacement preserves the canonical value and metadata. Value replacement is revalidated and rerenders content/metadata. Accept preserves tool-deferred contexts before decision contexts; block discards tool-deferred contexts and exposes only contexts explicitly supplied by the blocking decision.
