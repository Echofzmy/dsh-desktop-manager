...
- `ToolDefinition`：`ToolSchema` + 必填的 `output { schema, render, presentationMeta? }` + `execute(args, exec)`，以及可选的最终内容回调、呈现回调、协作式 `timeoutMs` 和逐调用的 `isConcurrencySafe(args)` 分类器。主体只能返回输出 schema 声明的规范 JSON 值，并通过 `exec.signal` 协作停止。`finalizeContent(exec, result)` 对每个规范化结果都恰好运行一次，包括绕过后置策略的失败，并且只能替换 `content`；它必须是同步且对所有输入都有定义的函数。可选的 `projectSchema({ agent })` 为确切的实时 agent 返回替换的模型可见 `description`／`parameters`，或返回 `undefined` 以保留静态 schema——在作者层，`parameters` 是与静态字段相同的 DSL 属性表，由 `defineTool` 编译为对象根 JSON Schema（原始 `ToolDefinition.projectSchema` 始终暴露编译后的形式）；它驱动 `schemasFor(agent)` 以及所有携带真实 agent 的系统提示词／Code Mode 组装，绝不驱动静态的 `schemas(scope)` 诊断投影。
- `ctx.tools.schemas(scope?)` 是静态投影（深拷贝，绝不调用 `projectSchema`）——用于诊断、目录与枚举表面的工具；`ctx.tools.schemasFor(agent)` 是模型绑定表面所用的按 agent 投影。工具的 `output.render(args, value, context?)` 可接收携带调用 `agent` 的可选渲染上下文，用于确定性的逐调用决策（例如是否附带升权引导），且不产生副作用。
...
- `PreToolDecision`：`{kind:'allow'}` | `{kind:'deny', reason, code?}` | `{kind:'ask', reason?}`。该类型有意不提供输入改写；`ask` 在挂载 [`ctx.approval`](../../interaction/user-approval/README.zh.md) 时由它处理，否则退化为拒绝。被审批拒绝的工具调用会在结果的 `ToolFailure.info` 上携带稳定的 `TOOL_APPROVAL_*` 错误码（`TOOL_APPROVAL_BLOCKED`／`TOOL_APPROVAL_REJECTED`／`TOOL_APPROVAL_CANCELLED`／`TOOL_APPROVAL_UNAVAILABLE`）；不带自定义 `code` 的前置执行 `deny` 或单调 `tools.guard()` 拒绝则默认为 `TOOL_POLICY_DENIED`。
- `PostToolDecision`：接受决定可以替换 `content` 或 `value`（不能同时替换），并可附加 `additionalContexts`；阻止决定会把反馈变成携带 `TOOL_POST_BLOCKED` 错误码的无值失败。替换内容会保留规范值和元数据。替换值会重新验证，并重新呈现内容／元数据。接受决定会先保留工具延迟的上下文，再附加决定上下文；阻止决定会丢弃工具延迟的上下文，只公开阻止决定显式提供的上下文。
