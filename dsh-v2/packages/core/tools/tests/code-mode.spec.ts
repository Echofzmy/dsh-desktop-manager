...
describe('mode-aware wire contribution', () => {
  ...
  it('applies per-agent projectSchema in the SDK section when the assembly carries the agent, static otherwise', async () => {
    const { ctx, systemPrompt } = await setup({ mode: 'code' })
    ctx.tools.register(defineTool({
      name: 'projectable',
      description: 'static sdk description',
      parameters: { mode: { type: 'string', enum: ['a', 'b'] } },
      output: { schema: { type: 'json' }, render: () => [] },
      projectSchema: ({ agent }) => ({
        description: `projected sdk for ${(agent as unknown as { id: string }).id}`,
        parameters: { mode: { type: 'string', enum: ['a'] } },
      }),
      async execute() { return [] },
    }))
    const agent = { id: SessionId('sdk-agent') } as Agent

    const withAgent = await systemPrompt.assemble({ agent, scope: agent })
    const sdkWithAgent = withAgent.sections.find(section => section.name === 'tools:sdk')?.text
    expect(sdkWithAgent).toContain('projected sdk for sdk-agent')
    expect(sdkWithAgent).not.toContain('static sdk description')

    // No agent in the assemble context: sdkSchemas falls back to the static schema.
    const bare = await systemPrompt.assemble()
    const sdkBare = bare.sections.find(section => section.name === 'tools:sdk')?.text
    expect(sdkBare).toContain('static sdk description')
    expect(sdkBare).not.toContain('projected sdk for sdk-agent')
  })
