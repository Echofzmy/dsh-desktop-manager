...
describe('ToolRuntime', () => {
  ...
  it('turns a post-execute block into a valueless failure with the TOOL_POST_BLOCKED code', async () => {
    const ctx = await setup()
    ctx.tools.register(echoTool)
    ctx.on('tools/post-execute', async () => ({
      kind: 'block',
    ...
    expect(result).toEqual({
      isError: true,
      error: { message: 'blocked by policy', info: { name: 'HarnessError', code: 'TOOL_POST_BLOCKED' } },
  ...
  it('lets a tools/pre-execute listener deny a call (permission pattern)', async () => {
    ...
    expect(result.error).toMatchObject({ info: { name: 'HarnessError', code: 'TOOL_POLICY_DENIED' } })
    expect(postSawFrozen).toBe(true)
  })

  it('a pre-execute deny keeps a custom code, defaulting to TOOL_POLICY_DENIED without one', async () => {
    const customCtx = await setup()
    customCtx.tools.register(echoTool)
    customCtx.on('tools/pre-execute', async (_exec, _next): Promise<PreToolDecision> =>
      ({ kind: 'deny', reason: 'custom denial', code: 'CUSTOM_POLICY' }))
    const custom = await customCtx.tools.execute({ signal: testToolSignal, callId: CallId('custom-deny'), name: 'echo', arguments: {} })
    expect(custom.error).toMatchObject({ info: { name: 'HarnessError', code: 'CUSTOM_POLICY' } })

    const plainCtx = await setup()
    plainCtx.tools.register(echoTool)
    plainCtx.on('tools/pre-execute', async (_exec, _next): Promise<PreToolDecision> =>
      ({ kind: 'deny', reason: 'plain denial' }))
    const plain = await plainCtx.tools.execute({ signal: testToolSignal, callId: CallId('plain-deny'), name: 'echo', arguments: {} })
    expect(plain.error).toMatchObject({ info: { name: 'HarnessError', code: 'TOOL_POLICY_DENIED' } })
  })

  it('a monotonic guard denial carries the TOOL_POLICY_DENIED code', async () => {
    const ctx = await setup()
    ctx.tools.register(echoTool)
    ctx.tools.guard(exec => exec.name === 'echo' ? 'guarded' : undefined)

    const result = await ctx.tools.execute({ signal: testToolSignal, callId: CallId('guarded'), name: 'echo', arguments: {} })
    expect(result.isError).toBe(true)
    expect(result.error).toMatchObject({ info: { name: 'HarnessError', code: 'TOOL_POLICY_DENIED' } })
  })
  ...
  describe('ask routing through ctx.approval', () => {
    ...
    it('denies with the user-rejection reason on rejected', async () => {
      ...
      expect(result.error).toMatchObject({ info: { name: 'HarnessError', code: 'TOOL_APPROVAL_REJECTED' } })
    })

    it('denies with the policy-block reason and TOOL_APPROVAL_BLOCKED code on blocked', async () => {
      const ctx = await approvalSetup()
      ctx.on('approval/request', () => Promise.resolve<ApprovalOutcome>('blocked'))
      ctx.on('tools/pre-execute', async (_exec, _next): Promise<PreToolDecision> => ({ kind: 'ask' }))

      const result = await ctx.tools.execute({ signal: testToolSignal, callId: CallId('c1'), name: 'echo', arguments: {}, agent: fakeAgent() })
      expect(result.isError).toBe(true)
      expect(result.content[0]).toMatchObject({ text: 'Error: tool "echo" is blocked by the current approval policy' })
      expect(result.error).toMatchObject({ info: { name: 'HarnessError', code: 'TOOL_APPROVAL_BLOCKED' } })
    })
    ...
    it('denies with the cancellation reason on cancelled', async () => {
      ...
      expect(result.error).toMatchObject({ info: { name: 'HarnessError', code: 'TOOL_APPROVAL_CANCELLED' } })
    })
    ...
    it('denies with the no-channel reason when the seam is mounted but nobody answers', async () => {
      ...
      expect(result.error).toMatchObject({ info: { name: 'HarnessError', code: 'TOOL_APPROVAL_UNAVAILABLE' } })
    })
  ...
  it('preserves a pre-execute denial that settles after cancellation', async () => {
    ...
    await expect(pending).resolves.toEqual({
      ...
      isError: true,
      error: { message: 'policy denied the call', info: { name: 'HarnessError', code: 'TOOL_POLICY_DENIED' } },
    })
    expect(dispatched).toBe(0)
  })
...
describe('per-agent schema projection (projectSchema / schemasFor)', () => {
  const projectingTool = defineTool({
    name: 'projectable',
    description: 'static description',
    parameters: { mode: { type: 'string', enum: ['a', 'b'] } },
    output: { schema: { type: 'json' }, render: () => [] },
    projectSchema: ({ agent }) => ({
      description: `projected for ${(agent as unknown as { id: string }).id}`,
      parameters: {
  ...
  function fakeAgent(id: string): Agent {
    return { id, session: { events: [{ type: 'turn/start' }], append: () => ({}) } } as unknown as Agent
  }

  it('schemas(scope) stays static and never invokes projectSchema', async () => {
    const ctx = await setup()
    ctx.tools.register(projectingTool)
    expect(ctx.tools.schemas()).toEqual([{
      name: 'projectable',
      description: 'static description',
  ...
  it('schemasFor(agent) exposes the COMPILED object-rooted JSON Schema, never the DSL map', async () => {
    const ctx = await setup()
    ctx.tools.register(projectingTool)
    const schema = ctx.tools.schemasFor(fakeAgent('agent-1'))[0]!
    expect(schema.name).toBe('projectable')
    expect(schema.description).toBe('projected for agent-1')
    // Root type/object plus per-property compilation (nested additionalProperties
    // and type-correct scalar enum) — the exact shape `parameterSchemaSpecToJsonSchema`
    // produces for the projected DSL map, pinned without casts.
    ...
    // The raw DSL property map must never leak to the model surface.
    expect(schema.parameters).not.toEqual({ mode: { type: 'string', enum: ['a'] } })
  })

  it('the raw ToolDefinition.projectSchema returns the final compiled JSON schema (defineTool compiles the DSL map)', async () => {
    const ctx = await setup()
    ctx.tools.register(projectingTool)
    const projected = ctx.tools.get('projectable')!.projectSchema?.({ agent: fakeAgent('raw') })
    expect(projected).toEqual({
      description: 'projected for raw',
      parameters: {
        type: 'object',
        properties: {
          mode: { type: 'string', enum: ['a'] },
    ...
    // The author wrote the DSL map; the raw definition exposes the compiled form.
    expect(projected?.parameters).not.toEqual({ mode: { type: 'string', enum: ['a'] } })
  })

  it('the projection context carries the real Agent — a scope-typed read never casts', async () => {
    const ctx = await setup()
    const seen: Agent[] = []
    ctx.tools.register(defineTool({
      name: 'witness',
      ...
      parameters: {},
      output: { schema: { type: 'json' }, render: () => [] },
      projectSchema: (context) => { seen.push(context.agent); return undefined },
      async execute() { return [] },
    }))
    const agent = fakeAgent('agent-2')
    ctx.tools.schemasFor(agent)
    expect(seen).toEqual([agent])
    // The static scope-based projection must not invoke the hook at all.
    ctx.tools.schemas(agent)
    expect(seen).toEqual([agent])
  })

  it('the system-prompt assembly projects when it carries the agent, static otherwise', async () => {
    const ctx = await setup()
    ctx.tools.register(projectingTool)
    const agent = fakeAgent('agent-3')
    const withAgent = await ctx.systemPrompt.assemble({ agent, scope: agent })
    expect(withAgent.tools[0]!.description).toBe('projected for agent-3')
    expect(withAgent.tools[0]!.parameters).toEqual({
      type: 'object',
      properties: {
        mode: { type: 'string', enum: ['a'] },
        nested: { type: 'object', additionalProperties: false, properties: { x: { type: 'number' } } },
    ...
    // No agent in the assemble context: wireSchemas falls back to the static
    // schema — neither description nor parameters are projected.
    const bare = await ctx.systemPrompt.assemble()
    expect(bare.tools[0]!.description).toBe('static description')
    expect(bare.tools[0]!.parameters).toMatchObject({
      type: 'object',
