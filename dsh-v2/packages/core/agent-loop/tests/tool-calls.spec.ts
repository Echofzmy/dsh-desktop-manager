...
describe('tool-call scheduler: ordered middleware and additional contexts', () => {
  ...
  it('orders pre-execute denials and errors without dispatching them', async () => {
    ...
    // The parallel-group pre-execute deny persists the default TOOL_POLICY_DENIED code.
    expect(results[1]!.data.error).toEqual({ name: 'HarnessError', code: 'TOOL_POLICY_DENIED' })
