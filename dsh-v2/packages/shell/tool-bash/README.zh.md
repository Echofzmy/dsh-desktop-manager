...
| `sandbox_permissions` | string enum | 仅当已挂载的执行器启用沙箱时才会公开（`ctx.shell.sandboxMode` 报告一个具有限制作用的默认值）：被拒命令所需的更宽模式，取自封闭的目标词汇 `workspace-write`/`danger-full-access`（绝不能缩减为执行器默认值——有效模式按会话确定）。执行时会基于有效模式检查是否严格拓宽：严格更宽的请求才走 `ctx.approval`；相等或更窄的请求属于严格 schema 的冗余字段，在有效模式下运行，不发起审批、也不报错。 |
...
需要升权的 bash 调用会在执行前解析 `ctx.approval`。`allowed-once` 只对该次调用应用请求模式；策略阻止（`SANDBOX_ESCALATION_BLOCKED`）、审批被拒（`SANDBOX_ESCALATION_REJECTED`）、取消（`SANDBOX_ESCALATION_CANCELLED`）、不可用（`SANDBOX_ESCALATION_UNAVAILABLE`）或缺少审批上下文时，命令完全不会执行，并返回携带相应稳定错误码的不同错误。发生真实拒绝后，模型可以在同一轮次中使用满足需要的最窄模式和理由重试同一命令一次；审批提示本身就是征求同意的步骤。升权绝不能预先推测，禁用或拒绝审批即为最终结果。其理由见 [沙箱 Agent Note](../../../.agents/notes/implemented/feature/2026-07-06-sandbox.zh.md)。Schema 广告按 agent 进行：有效审批策略为 `never`、或有效沙箱模式已是 `danger-full-access` 的 agent，其 schema 会省略升权字段与描述段落（已无可升权的目标），其拒绝结果也不会附带升权提示——同一谓词同时门控模型表面与提示。将会话模式切回 `danger-full-access` 之下（或策略切回 `ask`）即可同时恢复两者。执行期强制保持不变：schema 省略只是广告，绝不是绕过护栏。
...
验证和策略失败统一为 `Error: <message>`。此包的稳定消息包括 `invalid command: expected a non-empty string`、`invalid description: expected a non-empty string`、`invalid timeoutMs: expected a positive number, got <value>`、`invalid escalation: sandbox_permissions requires a justification`（错误码 `INVALID_ARGS`）、`invalid escalation: justification is only valid together with sandbox_permissions`（错误码 `INVALID_ARGS`）、`invalid justification: expected a non-empty sentence`（错误码 `INVALID_ARGS`）、`background execution is disabled for this bash tool`、`background jobs unavailable: load @deepseek-ai/dsh-jobs and @deepseek-ai/dsh-tool-jobs`、`sandbox_permissions is not available in this composition (no sandboxing executor to escalate)`（错误码 `SANDBOX_ESCALATION_UNAVAILABLE`）、`sandbox escalation to "<mode>" is blocked by the current approval policy`（错误码 `SANDBOX_ESCALATION_BLOCKED`）、审批不可用／拒绝／取消变体（错误码 `SANDBOX_ESCALATION_UNAVAILABLE`/`SANDBOX_ESCALATION_REJECTED`/`SANDBOX_ESCALATION_CANCELLED`），以及 `tool call aborted`。相等或更窄的 `sandbox_permissions` 请求不是错误：它在有效模式下运行。
