...
class RecordingSandboxExecutor extends ShellExecutor {
  ...
  /** When true, every run reports a sandbox denial (to exercise the hint gating). */
  deny = false
...
/** The sandboxed harness with the deterministic `'never'` approval policy. */
async function setupSandboxedNever() {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
...
describe('sandbox escalation through the generic task producer', () => {
  ...
  it('advertises the sandbox fields and validates their pairing', async () => {
    ...
    // Each unpaired/blank form fails with the structured INVALID_ARGS code,
    // never a message-only error, so retry/classifier layers can route on it.
    for (const args of [
      ...
      const result = await call(ctx, 'bash', args)
      expect(result.isError).toBe(true)
      expect(result.error).toMatchObject({ info: { name: 'HarnessError', code: 'INVALID_ARGS' } })
    }
  })

  it('rejects injected escalation without a sandbox and passes redundant escalation through without prompting', async () => {
    const plain = await setup()
    const injected = await call(plain, 'bash', escalate)
    expect(injected.isError).toBe(true)
    expect(text(injected)).toContain('not available in this composition')
    expect(injected.error).toMatchObject({ info: { name: 'HarnessError', code: 'SANDBOX_ESCALATION_UNAVAILABLE' } })

    // A requested mode equal to the effective mode is a strict-schema
    // redundant field: the call runs under the effective mode, no approval,
    // no error.
    ...
    // An effective mode outside the widening ladder cannot prove strict
    // widening, so the request is treated as redundant and the call runs
    // under that effective mode without prompting.
    const malformed = sandboxAgent()
    ;(malformed.session.events as unknown as Array<{ type: string; data: { mode: string } }>).push({
      type: 'sandbox/mode',
      data: { mode: 'unknown-mode' },
    })
    const unknown = await call(ctx, 'bash', escalate, malformed)
    expect(unknown.isError).toBe(false)
    expect(bash.modes).toEqual(['workspace-write', 'unknown-mode'])
  })

  it.each([
    // Effective mode override, requested sandbox_permissions, expected stamp.
  ...
  ] as const)('a redundant %s effective + %s request runs under the effective mode without approval', async (effective, requested, expected) => {
    const { ctx, bash } = await setupSandboxed(true)
    const prompted = vi.fn()
    ctx.on('approval/request', () => { prompted(); return Promise.resolve<ApprovalOutcome>('allowed-once') })
    const result = await call(ctx, 'bash', { ...escalate, sandbox_permissions: requested }, sandboxAgent(effective))
    expect(result.isError).toBe(false)
    expect(bash.modes).toEqual([expected])
    expect(prompted).not.toHaveBeenCalled()
  })

  it('a genuinely wider escalation under the never policy fails as a structured BLOCKED error', async () => {
    const { ctx, bash } = await setupSandboxedNever()
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('is blocked by the current approval policy')
    expect(result.error).toMatchObject({ info: { name: 'HarnessError', code: 'SANDBOX_ESCALATION_BLOCKED' } })
  ...
  it.each([
    ...
    ['blocked', 'blocked by the current approval policy'],
...
describe('per-agent escalation schema projection and denial hint gating', () => {
  async function bashSchemaFor(ctx: Context, agent: Agent) {
    const assembly = await ctx.systemPrompt.assemble({ agent, scope: agent })
    const schema = assembly.tools.find(t => t.name === 'bash')
    if (schema === undefined) throw new Error('bash tool missing from assembly')
    return schema
  }

  it('the static schemas() projection still advertises the escalation surface', async () => {
    const { ctx } = await setupSandboxed()
    const schema = ctx.tools.schemas().find(s => s.name === 'bash')!
    const properties = schema.parameters.properties as Record<string, unknown>
    expect(properties['sandbox_permissions']).toBeDefined()
    expect(schema.description).toContain('escalate')
  })

  it('omits the escalation fields and description clause under a never policy, restores under ask', async () => {
    const { ctx } = await setupSandboxed(true)
    const agent = sandboxAgent()
    const ask = await bashSchemaFor(ctx, agent)
    expect((ask.parameters.properties as Record<string, unknown>)['sandbox_permissions']).toBeDefined()
    expect(ask.description).toContain('escalate')

    ;(agent.session as unknown as { append: (t: string, d: Record<string, unknown>) => void }).append('approval/policy', { policy: 'never' })
    const never = await bashSchemaFor(ctx, agent)
    const neverProps = never.parameters.properties as Record<string, unknown>
    expect(neverProps['sandbox_permissions']).toBeUndefined()
    expect(neverProps['justification']).toBeUndefined()
    expect(never.description).not.toContain('escalate')
    expect(never.description).not.toContain('approval prompt')
    expect(never.description).toContain('Execute a bash command')
  })

  it('omits under an effective danger-full-access mode and restores when switched narrower', async () => {
    const { ctx } = await setupSandboxed(true)
    const agent = sandboxAgent('danger-full-access')
    const danger = await bashSchemaFor(ctx, agent)
    expect((danger.parameters.properties as Record<string, unknown>)['sandbox_permissions']).toBeUndefined()
    expect(danger.description).not.toContain('escalate')

    ;(agent.session as unknown as { append: (t: string, d: Record<string, unknown>) => void }).append('sandbox/mode', { mode: 'read-only' })
    const readOnly = await bashSchemaFor(ctx, agent)
    expect((readOnly.parameters.properties as Record<string, unknown>)['sandbox_permissions']).toBeDefined()
    expect(readOnly.description).toContain('escalate')
  })

  it('attaches the escalation hint to a denial only when escalation is advertised for the agent', async () => {
    const { ctx, bash } = await setupSandboxed(true)
    bash.deny = true
    const agent = sandboxAgent()
    const deniedAsk = await call(ctx, 'bash', { command: 'denied', description: 'denied command' }, agent)
    expect(text(deniedAsk)).toContain('escalation available')

    ;(agent.session as unknown as { append: (t: string, d: Record<string, unknown>) => void }).append('approval/policy', { policy: 'never' })
    const deniedNever = await call(ctx, 'bash', { command: 'denied', description: 'denied command' }, agent)
    expect(text(deniedNever)).toContain('[sandbox: file access denied under read-only mode]')
    expect(text(deniedNever)).not.toContain('escalation available')
  })

  it('execution-time escalation enforcement is unchanged when the schema omits the fields', async () => {
    const { ctx } = await setupSandboxedNever()
    const agent = sandboxAgent()
    const result = await call(ctx, 'bash', {
      command: 'true', description: 'd',
      sandbox_permissions: 'workspace-write', justification: 'bypass attempt',
    }, agent)
    expect(result.isError).toBe(true)
    expect(result.error).toMatchObject({ info: { name: 'HarnessError', code: 'SANDBOX_ESCALATION_BLOCKED' } })
  })
