...
import { ESCALATION_TARGETS, ESCALATION_ERROR_CODES, approveEscalation, escalationError, validateEscalationArgs } from '@deepseek-ai/dsh-sandbox'
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  /* jscpd:ignore-start -- deliberate mirror of dsh-tool-bash's per-agent escalation advertisement (pwsh-tool-and-executor Agent Note). */
  /**
   * Whether this agent's model surface advertises escalation: false under a
   * `'never'` approval policy (an approval can never grant) or an effective
   * mode of `danger-full-access` (nothing is strictly wider), true otherwise.
   * `undefined` (no live agent) keeps the composition's static advertisement.
   * The SAME predicate drives the per-agent schema projection AND the
   * same-turn denial hint, so the model is never invited to escalate where
   * escalation cannot be granted. Execution-time enforcement is unchanged:
   * schema omission is advertising, not a bypass guard.
   */
  const advertiseEscalationFor = (agent: Agent | undefined): boolean => {
    if (escalationModes.length === 0) return false
    if (agent === undefined) return true
    if (ctx.get('approval')?.effectivePolicy(agent.session) === 'never') return false
    return sandboxPolicy?.resolve({ session: agent.session }).mode !== 'danger-full-access'
  }

  /** The model-facing parameter map, with the escalation pair only when advertised. */
  const parametersFor = (advertise: boolean) => ({
   ...
   * anything executes, delegating the shared sequence (redundant-field
   * passthrough for a non-strictly-wider request, strict-widening approval,
   * channel resolution, outcome mapping) to {@link approveEscalation}. This
   * tool contributes only the composition guard (the fields are unadvertised
   * without a sandboxing executor, yet schema validation checks advertised
   * keys only, so an unadvertised `sandbox_permissions` still reaches execute)
   * and the approval ingredients. The shared policy resolver is required
   * whenever the executor advertises confinement, so a split composition
   * fails at tool-plugin load.
   */
  const approvePwshEscalation = (
    ...
    if (escalationModes.length === 0) {
      throw escalationError(ESCALATION_ERROR_CODES.unavailable, 'sandbox_permissions is not available in this composition (no sandboxing executor to escalate)')
    }
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: parametersFor(escalationModes.length > 0),
    // Per-agent projection: an agent whose effective approval policy is
    // 'never' or whose effective mode is already danger-full-access sees no
    // escalation fields and no escalation description clause — there is
    // nothing to escalate TO. The static `parameters` above stays the
    // composition-wide advertisement for diagnostics and `schemas(scope)`.
    projectSchema: ({ agent }) => {
      const advertise = advertiseEscalationFor(agent)
      return {
        description: pwshDescription(backgroundEnabled, advertise ? escalationModes : []),
        parameters: parametersFor(advertise),
      }
    },
    /* jscpd:ignore-end */
    output: {
      ...
      /* jscpd:ignore-end */
      render: (_args, value, context) => [{
        type: 'text',
        text: value.kind === 'background'
          ? `started background job ${value.jobId}`
          : renderPwshResult(
            value as RenderablePwshResult,
            advertiseEscalationFor(context?.agent) ? escalationModes : [],
          ),
      }],
    },
          ...
          run: () => {
            ...
            return {
              ...
              readOutput: () => renderPwshProcessRead(
                proc.readOutput(),
                proc.sandbox,
                advertiseEscalationFor(exec.agent) ? escalationModes : [],
              ),
            }
          },
        })
