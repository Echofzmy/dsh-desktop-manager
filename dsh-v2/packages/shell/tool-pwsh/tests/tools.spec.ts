...
/** The sandboxed composition with the deterministic `'never'` approval policy. */
...
describe('sandbox escalation through ctx.approval', () => {
  ...
  it('rejects injected escalation without a sandbox and passes redundant escalation through without prompting', async () => {
    const plain = await setup()
    const injected = await call(plain.ctx, 'pwsh', escalate)
    ...
    const unknown = await call(ctx, 'pwsh', escalate, malformed)
  ...
  it.each([
    ...
    const prompted = vi.fn()
    ctx.on('approval/request', () => { prompted(); return Promise.resolve<ApprovalOutcome>('allowed-once') })
    const result = await call(ctx, 'pwsh', { ...escalate, sandbox_permissions: requested }, sandboxAgent(effective))
    expect(result.isError).toBe(false)
...
describe('per-agent escalation schema projection and denial hint gating', () => {
  async function pwshSchemaFor(ctx: Context, agent: Agent) {
    const assembly = await ctx.systemPrompt.assemble({ agent, scope: agent })
    const schema = assembly.tools.find(t => t.name === 'pwsh')
    if (schema === undefined) throw new Error('pwsh tool missing from assembly')
    return schema
  }
  ...
  it('the static schemas() projection still advertises the escalation surface', async () => {
    const { ctx } = await setupSandboxed()
    const schema = ctx.tools.schemas().find(s => s.name === 'pwsh')!
    expect((schema.parameters.properties as Record<string, unknown>)['sandbox_permissions']).toBeDefined()
  ...
  it('omits the escalation fields and description clause under a never policy, restores under ask', async () => {
    ...
    const ask = await pwshSchemaFor(ctx, agent)
    ...
    const never = await pwshSchemaFor(ctx, agent)
  ...
  it('omits under an effective danger-full-access mode and restores when switched narrower', async () => {
    ...
    const danger = await pwshSchemaFor(ctx, agent)
    ...
    const readOnly = await pwshSchemaFor(ctx, agent)
  ...
  it('attaches the escalation hint to a denial only when escalation is advertised for the agent', async () => {
    ...
    bash.deny = true
    const agent = sandboxAgent()
    const deniedAsk = await call(ctx, 'pwsh', { command: 'Write-Output denied', description: 'denied command' }, agent)
    expect(text(deniedAsk)).toContain('escalation available')

    ;(agent.session as unknown as { append: (t: string, d: Record<string, unknown>) => void }).append('approval/policy', { policy: 'never' })
    const deniedNever = await call(ctx, 'pwsh', { command: 'Write-Output denied', description: 'denied command' }, agent)
