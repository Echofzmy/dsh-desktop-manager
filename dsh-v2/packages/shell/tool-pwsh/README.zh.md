...
| `sandbox_permissions` | string enum | 仅当已挂载 sandbox 执行器时才会公开（`ctx.shell.sandboxMode` 已定义）。用于对刚被 sandbox 拒绝的命令做一次性重试的更宽 sandbox 模式——取刚好足够的最窄更宽模式，要求 `justification` 并在执行**之前**经 `ctx.approval` 获得用户批准。严格更宽的请求经审批解析；相等或更窄的请求属于严格 schema 的冗余字段，在有效模式下运行，不发起审批、也不报错；无法获批的更宽请求 fail-closed，不运行任何内容。按 agent 广告：有效审批策略为 `never`、或有效模式已是 `danger-full-access` 的 agent，其 schema 会省略这两个字段与升权描述段落，其拒绝结果也不附带升权提示——将会话模式切回 `danger-full-access` 之下（或策略切回 `ask`）即可同时恢复。执行期强制保持不变。 |
...
校验与基础设施失败规范化为 `Error: <message>`。本包的稳定消息包括 `invalid command: expected a non-empty string`、`invalid description: expected a non-empty string`、`invalid timeoutMs: expected a positive number, got <value>`、`invalid escalation: sandbox_permissions requires a justification`（错误码 `INVALID_ARGS`）、`invalid escalation: justification is only valid together with sandbox_permissions`（错误码 `INVALID_ARGS`）、`invalid justification: expected a non-empty sentence`（错误码 `INVALID_ARGS`）、`sandbox_permissions is not available in this composition (no sandboxing executor to escalate)`（错误码 `SANDBOX_ESCALATION_UNAVAILABLE`）、共享的升权失败（被当前审批策略阻止——错误码 `SANDBOX_ESCALATION_BLOCKED`／无审批服务——`SANDBOX_ESCALATION_UNAVAILABLE`／无 agent 可路由——`SANDBOX_ESCALATION_UNAVAILABLE`／无审批通道——`SANDBOX_ESCALATION_UNAVAILABLE`／用户拒绝——`SANDBOX_ESCALATION_REJECTED`／已取消——`SANDBOX_ESCALATION_CANCELLED`）、`run_in_background is disabled for this deployment (enableRunInBackground: false)`、`background jobs unavailable: load @deepseek-ai/dsh-jobs and @deepseek-ai/dsh-tool-jobs` 与 `tool call aborted`。
