...
- `update_goal(goal_id, revision, action, objective?, max_goal_rounds?, blocked_reason?)` 支持 `edit`、`pause`、`resume`、`complete` 和 `blocked`。`revision` 必须是正整数。替换值只属于 `edit`；`blocked_reason` 只有在 action 为 `blocked` 时才必填，并以稳定代码 `model-reported` 持久化。schema 明确要求省略不属于所选 `action` 的字段；严格 schema 下的空字符串和零填充值仍视为省略。
