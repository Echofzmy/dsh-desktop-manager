...
describe('workspaces', () => {
  ...
  it('records the workspace browse calls: default serves both row kinds, stubs override and see the signal', async () => {
    const runtime = await runtimeWithFrame()
    // Default: one directory and one file row at the workspace root, per the
    // wire contract's directories-first order.
    const root = await runtime.workspaces.browse('w1' as WorkspaceId)
    expect(root).toEqual({
      relativePath: '',
      entries: [
        { name: 'docs', relativePath: 'docs', path: '/ws/w1/docs', kind: 'directory', hidden: false },
        { name: 'README.md', relativePath: 'README.md', path: '/ws/w1/README.md', kind: 'file', hidden: false },
      ],
      truncated: false,
    })
    const nested = await runtime.workspaces.browse('w1' as WorkspaceId, 'docs')
    expect(nested.entries.map(entry => entry.relativePath)).toEqual(['docs/docs', 'docs/README.md'])
    expect(runtime.workspaces.calls).toEqual([
      { method: 'browse', args: ['w1', undefined, undefined] },
      { method: 'browse', args: ['w1', 'docs', undefined] },
    ])
    // Stubs replace the default like every sibling method, and receive the
    // signal like the production face gives the wire.
    const listing = { relativePath: '', entries: [], truncated: false }
    const browseStub = vi.fn(() => Promise.resolve(listing as never))
    runtime.workspaces.stub('browse', browseStub)
    const scan = new AbortController()
    await expect(runtime.workspaces.browse('w2' as WorkspaceId, '', scan.signal)).resolves.toBe(listing)
    expect(browseStub).toHaveBeenLastCalledWith('w2', '', scan.signal)
    await runtime.dispose()
  })
})
