...
describe('cordisDefineCard', () => {
  ...
  it('classifies policy/approval denials and correctable conditions blocked with the envelope stripped', () => {
    const blocked = cordisDefineCard(settled({
      isError: true,
      content: [{ type: 'text', text: 'Error: cordis_define is disabled by policy' }],
      error: { name: 'E', code: 'TOOL_POLICY_DENIED' },
    }))
    expect(blocked.state).toBe('blocked')
    expect(blocked.errorSummary).toBe('cordis_define is disabled by policy')
    expect(blocked.output).toBe('Error: cordis_define is disabled by policy')

    expect(cordisActionCard(settled({
      call: { name: 'cordis_stop', argsRaw: '{"pluginId":"clock-1"}' },
      isError: true,
      content: [{ type: 'text', text: 'Error: stop rejected by approval' }],
      error: { name: 'E', code: 'TOOL_APPROVAL_REJECTED' },
      meta: undefined,
    })).errorSummary).toBe('stop rejected by approval')

    // An unknown code stays a real error and keeps its envelope.
    const unknown = cordisDefineCard(settled({
      isError: true,
      content: [{ type: 'text', text: 'Error: plugin exploded' }],
      error: { name: 'E', code: 'PLUGIN_ONLY_CODE' },
    }))
    expect(unknown.state).toBe('error')
    expect(unknown.errorSummary).toBe('Error: plugin exploded')
  })
