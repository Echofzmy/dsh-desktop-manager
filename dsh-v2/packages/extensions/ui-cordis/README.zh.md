...
**卡片是一份记录。** 它显示模型写下的 name 与 purpose、它写的源码，以及那个定义是否在跑——没有开关、没有审批，只有一句指向面板的指引。素材只取自冻结的 call/result 切片（标签取自 `argsRaw`，host 铸出的 id 取自结果的 `meta.id`），因此 replay 渲染出同一张卡；而有一个读数天生属于卡片：本会话日志里成功的 `cordis_undefine` 是持久且终态的，它**压过 wire**——在 wire 上，retract 公告与一次普通 stop 长得一模一样。每个生命周期卡片的运行状态同样取自冻结切片，经 Runtime 的单一 `toolOutcomeKind` 分类器得出：`blocked`（策略/审批拒绝、可纠正条件）与 `stopped`（取消/中断）以非红警告点呈现，只有真正的 `error` 显示为红色；被阻止的行的收起拒绝行会去掉注册表的 `Error: ` 前缀。
