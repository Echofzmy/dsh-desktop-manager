...
describe('SkillRow', () => {
  ...
  it('renders stopped, blocked, structured, and structured-error durable outcomes', () => {
    ...
    const blockedView = render(<SkillRow {...props(settled({
      content: [{ type: 'text', text: 'Error: skill invocation is disabled by policy' }],
      isError: true,
      error: { name: 'E', code: 'TOOL_POLICY_DENIED' },
    }))} />)
    const blockedRow = blockedView.container.querySelector('[data-tool="skill"]')!
    expect(blockedRow.getAttribute('data-state')).toBe('blocked')
    expect(blockedView.container.querySelector('[data-state="warning"]')).not.toBeNull()
    // The collapsed denial line drops the `Error: ` envelope and stays non-red.
    expect(blockedRow.textContent).toContain('skill invocation is disabled by policy')
    expect(blockedRow.textContent).not.toContain('Error: skill invocation is disabled by policy')
    cleanup()
