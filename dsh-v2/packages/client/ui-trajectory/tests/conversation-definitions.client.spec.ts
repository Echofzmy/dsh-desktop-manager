...
describe('Trajectory conversation Definitions', () => {
  ...
  it('classifies a user/parent cancellation-finalized prefix as a stopped request, not a failure', () => {
  ...
  it('classifies an aborted turn with no visible result as a stopped request', () => {
    const current = snapshot(assembler([
      at(1, 'turn/start', { turn: 1 }),
      at(2, 'step/start', { turn: 1, step: 1 }),
      at(3, 'turn/end', {
  ...
  it('never downgrades a provider-failure turn to stopped', () => {
    const current = snapshot(assembler([
      at(1, 'turn/start', { turn: 1 }),
      at(2, 'step/start', { turn: 1, step: 1 }),
      at(3, 'turn/end', {
        turn: 1,
        reason: { kind: 'error', error: { message: 'provider 500', code: 'SERVER' } },
      }),
    ]))
