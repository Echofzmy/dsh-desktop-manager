...
describe('TrajectoryTable', () => {
  ...
  it('renders stopped and blocked tool rows without the red error treatment', () => {
    const turns: readonly TrajectoryTurnModel[] = [{
      turn: 1,
      groups: [{
        title: 'Step 1',
        cells: [
          {
            index: 1,
            kind: 'tool',
            text: 'bash · {"command":"sleep"}',
            inputDetail: '{"command":"sleep"}',
            outputDetail: 'Error: tool call aborted',
          ...
          },
          {
            index: 2,
            kind: 'tool',
            text: 'bash · {"command":"rm"}',
            inputDetail: '{"command":"rm"}',
            outputDetail: 'Error: bash is disabled by policy',
            result: 'bash is disabled by policy',
            isError: true,
            state: 'blocked',
            timeSeconds: 0.5,
          },
    ...
    const view = render(<TrajectoryTable turns={turns} {...FOLD_PROPS} />)
    // Non-red: no data-error on either row, and the accessible label names
    // the state (`, Stopped` / `, Blocked`) instead of a bare failure.
    expect(view.container.querySelector('tr[data-error="true"]')).toBeNull()
    expect(screen.getByRole('row', { name: /TOOL, bash .*sleep.*, Stopped/ })).toBeTruthy()
    expect(screen.getByRole('row', { name: /TOOL, bash .*rm.*, Blocked/ })).toBeTruthy()
    fireEvent.click(screen.getByRole('row', { name: /rm/ }))
    expect(screen.getByText('Blocked')).toBeTruthy()
  })

  it('renders a stopped request record non-red with the Stopped label', () => {
    const turns: readonly TrajectoryTurnModel[] = [{
      turn: 1,
      groups: [{
        title: 'Step 1',
    ...
    const view = render(<TrajectoryTable turns={turns} {...FOLD_PROPS} />)
    // Non-red: no data-error row treatment and no red request boundary marker,
    // and the accessible label names the state instead of a bare failure.
    expect(view.container.querySelector('tr[data-error="true"]')).toBeNull()
    expect(view.container.querySelector('[data-request-status="error"]')).toBeNull()
    expect(screen.getByRole('row', { name: /Stopped/ })).toBeTruthy()
    expect(screen.queryByRole('row', { name: /Failed/ })).toBeNull()
  })
