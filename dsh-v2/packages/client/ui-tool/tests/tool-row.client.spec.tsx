...
describe('tool-call-model', () => {
  ...
  it('derives state across running/ok/error/interrupted/blocked', () => {
    ...
    expect(toolRowModel('bash', result({ isError: true, error: { name: 'E', code: 'ABORTED' } })).state).toBe('stopped')
    expect(toolRowModel('bash', result({ isError: true, error: { name: 'E', code: 'ABORTED_BEFORE_DISPATCH' } })).state).toBe('stopped')
    // Policy/approval refusals and correctable conditions are blocked, not failed.
    expect(toolRowModel('bash', result({ isError: true, error: { name: 'E', code: 'TOOL_POLICY_DENIED' } })).state).toBe('blocked')
    expect(toolRowModel('bash', result({ isError: true, error: { name: 'E', code: 'TOOL_APPROVAL_BLOCKED' } })).state).toBe('blocked')
    expect(toolRowModel('bash', result({ isError: true, error: { name: 'E', code: 'INVALID_ARGS' } })).state).toBe('blocked')
    expect(toolRowModel('bash', result({ isError: true, error: { name: 'E', code: 'GOAL_TOOL_AGENT_REQUIRED' } })).state).toBe('blocked')
    // An unknown structured code stays a real failure.
    expect(toolRowModel('bash', result({ isError: true, error: { name: 'E', code: 'PLUGIN_ONLY_CODE' } })).state).toBe('error')
  })

  it('shows the outcome first line on stopped rows with the envelope kept', () => {
    const stopped = result({
      content: [{ type: 'text', text: 'Error: tool call aborted\ndetail' }],
      isError: true, error: { name: 'AbortError', code: 'ABORTED' },
    })
    const model = toolRowModel('bash', stopped)
    expect(model.state).toBe('stopped')
    // Like an error row (envelope kept), unlike a blocked row (envelope stripped).
    expect(model.errorSummary).toBe('Error: tool call aborted')
    expect(model.output).toBe('Error: tool call aborted\ndetail')
  })
  ...
  it('strips the Error: envelope from a blocked denial line but keeps it on an error row', () => {
    const blocked = (text: string, code = 'TOOL_POLICY_DENIED') => result({
      content: [{ type: 'text', text }], isError: true, error: { name: 'E', code },
    })
    expect(toolRowModel('bash', blocked('Error: the user rejected tool "bash"')).errorSummary)
      .toBe('the user rejected tool "bash"')
    expect(toolRowModel('bash', blocked('Error: invalid arguments: missing required property "path"', 'INVALID_ARGS')).errorSummary)
      .toBe('invalid arguments: missing required property "path"')
    // A denial whose text has no envelope passes through unchanged.
    expect(toolRowModel('bash', blocked('output rejected: try again', 'TOOL_POST_BLOCKED')).errorSummary)
      .toBe('output rejected: try again')
    // The error row keeps the envelope verbatim.
    expect(toolRowModel('bash', result({ content: [{ type: 'text', text: 'Error: boom' }], isError: true })).errorSummary)
      .toBe('Error: boom')
    // The expanded output text is never rewritten.
    expect(toolRowModel('bash', blocked('Error: denied by policy')).output).toBe('Error: denied by policy')
  })
