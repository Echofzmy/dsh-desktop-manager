...
describe('Tool presentation tails', () => {
  ...
  it('BashRow keeps a stopped call with durable output expandable, warning-styled and never red', () => {
    ...
    // The collapsed summary keeps the args-derived summary, not the stop text.
    expect(row.textContent).toContain('Wait')
    expect(row.textContent).not.toContain('Error: tool call aborted')
    fireEvent.click(row)
