...
Generic rows classify known Tool names into search, read, shell, write, edit, code, or generic variants. Running, successful, failed, stopped, and blocked lifecycle states come only from the frozen call/result slice via the Runtime's single `toolOutcomeKind` classifier (structured `error.code`, never text parsing): `blocked` (policy/approval refusal, `INVALID_ARGS`, …) and `stopped` (cancellation/interruption) render non-red with a warning dot, while only a real `error` renders red. A blocked row's collapsed denial line drops the registry's `Error: ` envelope (an error row keeps it); the expanded output text is never rewritten. A stopped Bash call that settles without terminal material stays expandable to its recorded command and full output exactly like a failure or denial — the row keeps the warning (non-red) treatment, and its expanded output text is never marked red. File paths resolve against the session `cwd` only when the user invokes the Host open-file callback; presentation code does not read Session services.
