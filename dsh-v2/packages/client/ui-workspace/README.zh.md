...
区域顶部的紧凑模式开关（会话／文件）取代了原先的区头标签。会话保留分组／单列表与搜索；文件显示当前 Workspace 的目录树——即包含当前 Session 的 Workspace，否则为最近活跃的 Workspace，再否则为注册顺序中的第一个 Workspace，没有任何 Workspace 时显示空状态。该树是懒加载的：选中文件模式时加载根目录，目录只在首次展开时加载并把该层级缓存起来（收起再展开不会重新拉取），**刷新**会清空缓存并重新加载根目录。行保持服务端返回的目录优先顺序，隐藏条目默认隐藏，**显示隐藏文件**开关可将其揭示；点击文件会把其绝对路径交给 Host 的默认应用打开（通过注入的 `openFile`，即 `ctx.workspaces.openPath`）。加载中、空目录、条目过多、浏览失败与打开失败等状态都会内联显示。在折叠轨道中，一枚文件图标会展开侧边栏并进入文件模式，搜索与添加控件则保持为会话模式入口，执行前先把区域切回会话模式。
