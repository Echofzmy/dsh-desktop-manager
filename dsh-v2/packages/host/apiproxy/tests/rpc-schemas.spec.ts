...
import {
  ...
  workspaceBrowseRequestSchema, workspaceBrowseValueSchema,
...
describe('workspace domain schemas', () => {
  ...
  it('browse requires the workspace and defaults the path to the workspace root', () => {
    expect(workspaceBrowseRequestSchema.parse({ workspaceId: 'w1' }).relativePath).toBeUndefined()
    expect(workspaceBrowseRequestSchema.parse({ workspaceId: 'w1', relativePath: 'notes' }).relativePath)
      .toBe('notes')
    expect(() => workspaceBrowseRequestSchema.parse({})).toThrow()
    expect(() => workspaceBrowseRequestSchema.parse({ relativePath: 'notes' })).toThrow()
  })

  it('browse value carries both kinds and the hidden flag, rejecting an unknown kind', () => {
    const listing = {
      relativePath: '',
      entries: [
        { name: 'docs', relativePath: 'docs', path: '/p/docs', kind: 'directory', hidden: false },
        { name: 'README.md', relativePath: 'README.md', path: '/p/README.md', kind: 'file', hidden: false },
        { name: '.config', relativePath: '.config', path: '/p/.config', kind: 'directory', hidden: true },
      ],
      truncated: true,
    }
    expect(workspaceBrowseValueSchema.parse(listing)).toEqual(listing)
    expect(workspaceBrowseValueSchema.parse({ ...listing, entries: [] }).truncated).toBe(true)
    expect(() => workspaceBrowseValueSchema.parse({
      ...listing,
      entries: [{ name: 'x', relativePath: 'x', path: '/p/x', kind: 'symlink', hidden: false }],
    })).toThrow()
  })
})
