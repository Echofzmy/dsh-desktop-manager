...
/**
 * The completed acquisition of one `rg` run: complete stdout plus the
 * resolved workdir. When a search is a PARTIAL success (real results on
 * stdout, but some paths unreadable under macOS TCC or a permission denial),
 * `warnings` carries the retained permission-diagnostic excerpts so the
 * tools can say the result is partial instead of pretending it is complete.
 */
export interface RipgrepRun {
  ...
  workdir: string
  /**
   * Diagnostic excerpts for a PARTIAL result: non-empty only when ripgrep
   * exited 2, stdout already held results, and every retained stderr line was
   * a path-level permission denial. Empty for a complete search or a failure.
   */
  warnings: string[]
}
...
/**
 * A path-level unreadable-path diagnostic. macOS TCC refusals and ordinary
 * file/directory permission errors surface as one of these on a line
 * (`Access is denied` on Windows included).
 */
const PERMISSION_DENIAL_LINE = /operation not permitted|permission denied|access (?:is )?denied/i

/**
 * Whether a nonzero-exit `rg` run is a recoverable PARTIAL success: stdout
 * already holds real results, the retained stderr tail is COMPLETE
 * (untruncated — a lossy tail could hide a non-permission diagnostic), and
 * every non-empty stderr line — at least one such line exists — is a
 * path-level permission denial. The search itself finished; only some paths
 * were unreadable. A mixed failure (any other diagnostic line), a truncated
 * tail, or no diagnostic at all fails this test so a genuinely failed search
 * is never presented as a partial success.
 */
function isPermissionPartial(stdoutText: string, stderrText: string, stderrLossy: boolean): boolean {
  if (stdoutText.length === 0 || stderrLossy) return false
  let sawDiagnostic = false
  for (const line of stderrText.split('\n')) {
    const trimmed = line.trim()
    if (trimmed.length === 0) continue
    if (!PERMISSION_DENIAL_LINE.test(trimmed)) return false
    sawDiagnostic = true
  }
  // A nonzero exit with no diagnostic at all is a failure, not a partial run.
  return sawDiagnostic
}

/**
 * The model-facing warning lines appended to a PARTIAL search result: one
 * line per retained warning, each explicitly naming the partial basis and the
 * retained diagnostic excerpt, e.g. `rg: /path: Operation not permitted
 * (os error 1)`.
 * @param warnings - the retained diagnostic excerpts ({@link RipgrepRun.warnings}).
 * @returns the warning lines, one per non-empty excerpt, or '' without any.
 */
export function formatSearchWarnings(warnings: readonly string[]): string {
  return warnings
    .filter(warning => warning.trim().length > 0)
    .map(warning => `[warning: partial result — some paths were not readable: ${warning.trim()}]`)
    .join('\n')
}

/**
 * Append the partial-result warning lines to a rendered body with one blank
 * line of separation; the body passes through unchanged without warnings.
 * @param body - the already-formatted result body (paths or matches).
 * @param warnings - the retained diagnostic excerpts ({@link RipgrepRun.warnings}).
 * @returns the body with the warning lines appended, or the body unchanged.
 */
export function appendSearchWarnings(body: string, warnings: readonly string[]): string {
  const formatted = formatSearchWarnings(warnings)
  if (formatted.length === 0) return body
 ...
 * stream. The overflow recovery copy is tool-accurate: `glob` has no
 * `include` parameter, so its suggestion never names one.
 */
function completeStdout(toolName: string, stdout: SubprocessOutputRead, rawOutputMaxBytes: number): string {
  // The only recovery lever for glob is pattern/path; naming include would
  // suggest a parameter the glob schema does not have.
  const narrow = toolName === 'glob' ? 'narrow pattern or path and retry' : 'narrow pattern, path, or include and retry'
  if (!stdout.lossy) {
 ...
 * `SEARCH_RAW_OUTPUT_OVERFLOW`). One exception is a PARTIAL success: an
 * exit 2 whose stdout already holds real results and whose complete,
 * untruncated stderr names ONLY path-level permission denials (macOS TCC
 * `Operation not permitted`, or an unreadable file/directory) returns the
 * results with the diagnostic excerpts in {@link RipgrepRun.warnings} —
 * the search finished, only some paths were unreadable. Both launch-time
 * failure domains are classified: a synchronous throw at spawn CREATION (a
 * NUL in argv, an abort racing the pre-check, a rejected
 * `@vscode/ripgrep` resolution) and a rejection of `handle.done` (the
 * seam's infrastructure failures) both become `SEARCH_FAILED` with the
 ...
 * @returns the complete stdout, the zero-result flag, the resolved workdir,
 *   and the partial-result warnings (empty for a complete search or a failure).
 */
export async function runRipgrep(
  ...
  if (outcome.exitCode !== 0 && outcome.exitCode !== 1) {
    // A permission-only exit 2 with real stdout is a PARTIAL success: the
    // search completed, only some paths were unreadable (macOS TCC, or an
    // unreadable file/directory). Warnings ride the result so the model sees
    // the partial basis; every other nonzero exit keeps the existing
    // classification (a mixed or truncated diagnostic tail is never partial).
    if (outcome.exitCode === 2 && isPermissionPartial(stdout.text, stderr.text, stderr.lossy)) {
      const text = completeStdout(toolName, stdout, rawOutputMaxBytes)
      return { stdout: text, noMatches: false, workdir, warnings: [stderrExcerpt(stderr.text, stderr.lossy)] }
    }
    throw classifyRunFailure(toolName, outcome.exitCode, stderr.text, stderr.lossy)
  }
  const text = completeStdout(toolName, stdout, rawOutputMaxBytes)
  return { stdout: text, noMatches: outcome.exitCode === 1, workdir, warnings: [] }
}
