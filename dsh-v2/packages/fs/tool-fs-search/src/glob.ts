...
import { appendSearchWarnings, runRipgrep, toWorkdirRelative, trySaveFormattedResult } from './search-core.ts'
...
/**
 * Directory names a discovery listing excludes when {@link GlobToolCaps.excludeNodeModules}
 * is enabled (the Config default): the VCS metadata stores plus `node_modules`.
 * `--no-ignore` would otherwise surface every vendored file in any broad `*`
 * search; deployments that must inspect dependency sources turn the node_modules
 * arm off via `globExcludeNodeModules: false` and only the VCS names remain.
 * Excluded with the same two negated `--glob`s as the VCS names.
 */
export const GLOB_DEFAULT_EXCLUDES: readonly string[] = [...GLOB_VCS_EXCLUDES, 'node_modules']
...
export interface GlobToolCaps {
  ...
  /** Whether `node_modules` is excluded from discovery (the `globExcludeNodeModules` config; the VCS names are always excluded). */
 ...
 * --hidden` searches ignored and hidden files, and the exclusion globs keep
 * VCS metadata out always plus `node_modules` when `excludeNodeModules` is
 * enabled ({@link GLOB_DEFAULT_EXCLUDES} vs {@link GLOB_VCS_EXCLUDES}).
 *
 * @param input - the validated arguments.
 * @param excludeNodeModules - whether `node_modules` joins the always-excluded
 *   VCS names (default true; a deployment that must inspect dependency sources
 *   resolves it from the `globExcludeNodeModules` config).
 * @returns the complete ripgrep argument vector (excluding the binary itself).
 */
export function buildGlobCommand(input: GlobInput, excludeNodeModules = true): string[] {
  const excluded = excludeNodeModules ? GLOB_DEFAULT_EXCLUDES : GLOB_VCS_EXCLUDES
  const parts = [
    '--files',
    `--glob=${input.pattern}`,
    '--sort=modified',
    '--no-ignore',
    '--hidden',
    // Two negated globs per excluded name: the bare form prunes the directory
...
/**
 * Bound and format one canonical path list for the Native surface relative to
 * its search root. A PARTIAL result's warnings are appended with an explicit
 * partial basis so the model never mistakes unreadable paths for absence.
 * @param paths - the discovered paths in modification-time order.
 * @param caps - the resolved glob caps (the inline cap and the sampling switch).
 * @param root - the search root in the same display-path space as `paths`.
 * @param spillRef - the saved complete-result reference, when the page was capped.
 * @param warnings - partial-result diagnostic excerpts ({@link module:@deepseek-ai/dsh-tool-fs-search/search-core}.RipgrepRun.warnings).
 * @returns the model-facing text: the page, then the partial warnings when present.
 */
function renderGlobPaths(paths: string[], caps: GlobToolCaps, root: string, spillRef?: SpillRef, warnings: readonly string[] = []): string {
  ...
  if (paths.length <= caps.maxResults) {
    body = paths.join('\n')
  } else if (!caps.sampleOverCapGlobResults) {
    body = formatGlobPage(paths.slice(0, caps.maxResults), paths.length, spillRef, '.')
  } else {
    body = formatGlobOutput(sampleAcrossTopLevel(paths, caps.maxResults, root), paths.length, spillRef)
  }
  return appendSearchWarnings(body, warnings)
}
...
export function applyGlobTool(ctx: Context, caps: GlobToolCaps): void {
  ...
  // The discovery-exclusion claim is composed from the caps so a deployment
  // with globExcludeNodeModules: false never advertises an exclusion it does
  // not apply — node_modules is then searched like any other directory.
  const exclusionClause = caps.excludeNodeModules
    ? 'VCS metadata and node_modules directories are excluded by default'
    : 'VCS metadata directories are excluded by default, node_modules is searched'
  ctx.systemPrompt.section({
    ...
    text: 'Use the glob tool — not shell find — to discover files by path pattern. A pattern with no "/" matches basenames at any depth, so "*" matches every file in the tree rather than its top level. '
      + `Results are files only, never directories, and include hidden and ignored files (${exclusionClause}): a result that fits comes back in modification-time order, ${overCapGuidance}`,
  })
  ...
  const tool = defineTool({
    ...
    description: 'Find files whose paths match a glob pattern. Returns matching file paths — never directories — '
      + `including hidden and ignored files (${exclusionClause}). `
    ...
    output: {
      schema: {
        ...
        properties: {
          ...
          // Present only for a PARTIAL result: ripgrep found paths but could
          // not read some of them (macOS TCC, unreadable file/directory).
          warnings: { type: 'array', items: { type: 'string' } },
        },
      },
      render: (_args, value) => [{
        type: 'text',
        text: renderGlobPaths(value.paths, caps, value.root, undefined, value.warnings),
      }],
    ...
    async execute(args, exec) {
      ...
      const run = await runRipgrep(ctx, exec, 'glob', buildGlobCommand(input, caps.excludeNodeModules), caps.rawOutputMaxBytes, caps.graceMs, caps.stderrMaxBytes)
      ...
      // The canonical value carries the partial-result warnings only when the
      // search was partial, so complete results keep the exact `{ root, paths }`
      // shape consumers already know.
      return run.warnings.length > 0
        ? { root, paths: all, warnings: run.warnings }
        : { root, paths: all }
    },
  ...
  ctx.on('tools/post-execute', async (exec, result, next) => {
    ...
    const value = (
      acceptedDirectCallValue(ctx, tool, exec, result, decision)
    ) as { root: string; paths: string[]; warnings?: string[] } | undefined
    if (value === undefined) return decision
    const paths = value.paths
    if (paths.length <= caps.maxResults) return decision
    const warnings = value.warnings ?? []
    // The spill artifact holds the complete formatted result AND the partial
    // basis, so a follow-up read of the saved file says the result was partial.
    const spillContent = appendSearchWarnings(paths.join('\n'), warnings)
    const spillRef = await trySaveFormattedResult(ctx, exec, 'glob-results.txt', spillContent)
    return {
      kind: 'accept',
      content: [{ type: 'text', text: renderGlobPaths(paths, caps, value.root, spillRef, warnings) }],
