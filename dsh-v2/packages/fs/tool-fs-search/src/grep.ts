...
import { SearchError, appendSearchWarnings, previewLine, retainGrepMatches, runRipgrep, toWorkdirRelative, trySaveFormattedResult } from './search-core.ts'
 ...
 * A PARTIAL result's warnings are appended with an explicit partial basis.
 *
...
export function formatGrepOutput(
  retained: RetainedItems<GrepMatch>,
  ...
  if (!retained.truncated) return appendSearchWarnings(`${header}\n\n${body}`, warnings)
  ...
  return appendSearchWarnings(`${header}\n\n${body}\n\n(${recovery})`, warnings)
}

/** Format one already-retained match list for the Native surface. */
function formatRetainedGrep(retained: RetainedItems<GrepMatch>, spillRef?: SpillRef, warnings: readonly string[] = []): string {
  if (retained.seen === 0) return 'No matches found'
  return formatGrepOutput(retained, spillRef, warnings)
}
...
export function applyGrepTool(ctx: Context, caps: GrepToolCaps): void {
  ...
  const tool = defineTool({
    ...
    output: {
      schema: {
        ...
        properties: {
          ...
          // Present only for a PARTIAL result: ripgrep matched lines but could
          // not read some paths (macOS TCC, unreadable file/directory).
          warnings: { type: 'array', items: { type: 'string' } },
        },
      },
      render: (_args, value) => [{
        type: 'text',
        text: formatRetainedGrep(retainGrepMatches(value.matches, caps.maxMatches, caps.maxLineBytes), undefined, value.warnings),
      }],
    ...
    async execute(args, exec) {
      ...
      // search was partial, so complete results keep the exact `{ matches }`
      // shape consumers already know.
      return run.warnings.length > 0
        ? { matches: all, warnings: run.warnings }
        : { matches: all }
    },
  ...
  ctx.on('tools/post-execute', async (exec, result, next) => {
    ...
    const value = acceptedDirectCallValue(ctx, tool, exec, result, decision) as { matches: GrepMatch[]; warnings?: string[] } | undefined
    ...
    // The saved file also names the partial basis, so a follow-up read of the
    // spill says the search was partial when some paths were unreadable.
    const spillBody = appendSearchWarnings(
      `Found ${matches.length} ${matchNoun(matches.length)}\n\n${formatGrepMatches(previewedAll)}`,
      warnings,
    )
    const spillRef = await trySaveFormattedResult(ctx, exec, 'grep-results.txt', spillBody)
    return {
      kind: 'accept',
      content: [{
        type: 'text',
        text: formatRetainedGrep(retainGrepMatches(matches, caps.maxMatches, caps.maxLineBytes), spillRef, warnings),
      }],
