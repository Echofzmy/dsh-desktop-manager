...
| `globExcludeNodeModules` | `true` | Whether a `glob` discovery listing excludes `node_modules` — a safety/size default, since `--no-ignore` would otherwise surface every vendored file in any broad `*` search. Deployments that must inspect dependency sources set it to `false`; the VCS metadata names are always excluded regardless. |
...
|---|---|---|
| `glob` | `pattern`, `path?` | `rg --files --glob <pattern> --sort=modified --no-ignore --hidden` plus VCS metadata excludes (`.git`, `.svn`, `.hg`, `.bzr`, `.jj`, `.sl`), and — when `globExcludeNodeModules` is true (the default) — `node_modules` too (a safety/size default; deployments that must inspect dependency sources set it false). `path` is an optional **directory** search root; omitted means the resolved workdir. Returns one FILE path per line; `rg --files` never emits directory entries. The pattern keeps ripgrep semantics: without a `/` it matches the basename at any depth, so `*` matches the whole tree. Complete results stay modification-time ordered; over-cap presentation follows `sampleOverCapGlobResults`. When ripgrep exits 2 after emitting results because some paths were unreadable (macOS TCC or a permission denial, with every diagnostic naming a permission error), the call returns the readable results with an explicit `[warning: partial result …]` line instead of failing. |
...
Search failures carry the package-owned `SearchError` (a `HarnessError` subclass), surfaced as `{ name, code }` on `isError` results: `SEARCH_INVALID_PATTERN` (ripgrep rejected the regex/glob), `SEARCH_FAILED` (a failed `rg` launch, inaccessible target, signal kill, malformed `--json` output), `SEARCH_RAW_OUTPUT_OVERFLOW` (raw output over `rawOutputMaxBytes`, or still lossy after the requested stdout capture budget), and `SEARCH_ABORTED` (cooperative tool timeout or caller cancellation). ripgrep exit semantics are tool-owned: exit 0 is success with results, exit 1 is a successful empty search (`No files found` / `No matches found`), and only other exits are failures — with one deliberate exception: an exit 2 whose stdout already holds real results and whose untruncated stderr names ONLY path-level permission denials (macOS TCC `Operation not permitted`, or an unreadable file/directory) is a **partial success** returning those results plus `warnings` — never `SEARCH_FAILED` — so the model sees readable results with an explicit partial basis. Model argument mistakes (blank pattern, a list-valued `include`) stay ordinary tool argument errors.
...
```markdown
Use the glob tool — not shell find — to discover files by path pattern. A pattern with no "/" matches basenames at any depth, so "*" matches every file in the tree rather than its top level. Results are files only, never directories, and include hidden and ignored files (VCS metadata and node_modules directories are excluded by default): a result that fits comes back in modification-time order, while a larger one is sampled across top-level entries, so it spans the tree instead of one subtree.
```

##### Glob guidance with `sampleOverCapGlobResults: false`

```markdown
Use the glob tool — not shell find — to discover files by path pattern. A pattern with no "/" matches basenames at any depth, so "*" matches every file in the tree rather than its top level. Results are files only, never directories, and include hidden and ignored files (VCS metadata and node_modules directories are excluded by default): a result that fits comes back in modification-time order, while a larger one keeps the modification-time-ordered head.
```
