...
describe('search tools over the real subprocess service + the packaged rg', () => {
  beforeEach(async () => {
    ...
    await writeFile(join(dir, 'node_modules', 'pkg', 'vendored.ts'), 'export const vendored = 4\n')
  ...
  describe('glob', () => {
    ...
    it('excludes node_modules by default, even when the search root IS the directory', async () => {
      // The default config keeps --no-ignore from surfacing vendored files in
      // a broad search; the paired contents glob still excludes them when the
      // explicit root is the directory itself.
      const wholeTree = text(await call('glob', { pattern: '*' }, agent())).split('\n')
      expect(wholeTree).not.toContain(join('node_modules', 'pkg', 'vendored.ts'))
      expect(wholeTree).toContain(join('src', 'alpha.ts'))
      expect(text(await call('glob', { pattern: '*', path: 'node_modules' }, agent()))).toBe('No files found')
    })

    it('globExcludeNodeModules: false searches node_modules (dependency-source deployments)', async () => {
      // A deployment that must inspect dependency sources rebuilds the plugin
      // with the exclusion off; vendored files then surface in a broad search.
      const previous = ctx
      ctx = new Context()
      await ctx.plugin(SystemPrompt)
      await ctx.plugin(ToolRuntime)
      await ctx.plugin(LocalSubprocessRuntime)
      await ctx.plugin(ToolFsSearch, { sampleOverCapGlobResults: true, globExcludeNodeModules: false })
      try {
        const wholeTree = text(await call('glob', { pattern: '*' }, agent())).split('\n')
        expect(wholeTree).toContain(join('node_modules', 'pkg', 'vendored.ts'))
      } finally {
        ctx = previous
      }
    })
