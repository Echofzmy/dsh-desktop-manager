...
describe('sandbox escalation API (write/edit)', () => {
  ...
  it.each([
    // Session override, requested sandbox_permissions, expected stamp — an
    // equal or narrower request is a strict-schema redundant field.
  ...
  ] as const)('a redundant %s effective + %s write request runs under the effective mode without approval', async (mode, requested, expected) => {
    const { ctx, fs } = await setupConfining({ approval: true })
    ...
    const result = await call(ctx, 'write', {
      ...
      sandbox_permissions: requested, justification: 'strict-schema filler',
    }, escalationAgent([{ type: 'sandbox/mode', data: { mode } }]))
    expect(result.isError).toBe(false)
    expect(fs.stamped).toEqual([{ mode: expected, workspaceRoot: resolve('/session-project') }])
  ...
  it('a genuinely wider escalation under the never policy fails as a structured BLOCKED error', async () => {
    ...
    ctx.on('approval/request', () => { consulted(); return Promise.resolve('allowed-once' as const) })
    const result = await call(ctx, 'write', {
      file_path: 'a.txt', content: 'x',
      sandbox_permissions: 'danger-full-access', justification: 'wider write',
  ...
  async function writeSchemaFor(ctx: Context, agent: object) {
    const assembly = await ctx.systemPrompt.assemble({ agent: agent as never, scope: agent })
    const schema = assembly.tools.find(t => t.name === 'write')
    if (schema === undefined) throw new Error('write tool missing from assembly')
    return schema
  }

  it('the static schemas() projection still advertises the escalation fields', async () => {
    const { ctx } = await setupConfining()
    for (const name of ['write', 'edit'] as const) {
      const props = fsSchema(ctx, name).parameters.properties
  ...
  it('omits the escalation fields under a never policy or an effective danger-full-access mode, restores after switching', async () => {
    const { ctx } = await setupConfining({ approval: true })
    const never = await writeSchemaFor(ctx, escalationAgent([{ type: 'approval/policy', data: { policy: 'never' } }]))
    expect((never.parameters.properties as Record<string, unknown>)['sandbox_permissions']).toBeUndefined()

    const danger = await writeSchemaFor(ctx, escalationAgent([{ type: 'sandbox/mode', data: { mode: 'danger-full-access' } }]))
    expect((danger.parameters.properties as Record<string, unknown>)['sandbox_permissions']).toBeUndefined()

    const readOnly = await writeSchemaFor(ctx, escalationAgent([{ type: 'sandbox/mode', data: { mode: 'read-only' } }]))
    expect((readOnly.parameters.properties as Record<string, unknown>)['sandbox_permissions']).toBeDefined()
  })

  it('a denied write keeps the marker but drops the escalation hint under a never policy', async () => {
    const { ctx, fs } = await setupConfining({ approval: true })
    fs.rejectWith = new FsError('denied', 'FS_SANDBOX_DENIED')
    const agent = escalationAgent([{ type: 'approval/policy', data: { policy: 'never' } }])
    const result = await call(ctx, 'write', { file_path: 'a.txt', content: 'x' }, agent)
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('[sandbox: file access denied under workspace-write mode]')
    expect(text(result)).not.toContain('retry this exact operation once with sandbox_permissions')

    const askAgent = escalationAgent()
    const ask = await call(ctx, 'write', { file_path: 'a.txt', content: 'x' }, askAgent)
    expect(text(ask)).toContain('retry this exact operation once with sandbox_permissions')
  })
})
