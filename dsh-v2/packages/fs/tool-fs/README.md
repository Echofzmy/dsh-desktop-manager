...
When `ctx.fs.sandboxMode` reports confinement, write/edit advertise `sandbox_permissions` and `justification` and resolve approved retries through `ctx.approval`. A requested mode that is not strictly wider than the effective mode (equal or narrower) is a strict-schema redundant field: the mutation runs under the effective mode with no approval and no error. Escalation failures throw structured errors with stable codes (`SANDBOX_ESCALATION_BLOCKED` policy block, `SANDBOX_ESCALATION_REJECTED` human/answerer rejection, `SANDBOX_ESCALATION_CANCELLED` withdrawal, `SANDBOX_ESCALATION_UNAVAILABLE` missing channel). Schema advertisement is per agent: an agent whose effective approval policy is `never`, or whose effective mode is already `danger-full-access`, sees the escalation fields omitted and its denial results carry no escalation hint — switching the session mode below `danger-full-access` (or policy back to `ask`) restores both; execution-time enforcement is unchanged. The policy owner contributes capability-neutral standing policy; the tool results retain operation-specific denial and retry guidance.
