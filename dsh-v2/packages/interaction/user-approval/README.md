...
`approval/asked` and `approval/decided` are log-only. The model sees only the asking consumer's eventual allowed, rejected, blocked, cancelled, or unavailable tool outcome; the human permission UI is not context.
