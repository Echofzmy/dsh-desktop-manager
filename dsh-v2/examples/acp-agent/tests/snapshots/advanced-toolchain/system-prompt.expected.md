...
interface ToolArgsMap {
  ...
  /** Update the exact current goal revision. edit, pause, and resume require a direct top-level human request. During an automatic continuation of the current goal, complete and blocked are also allowed. blocked is rejected before the configured minimum round count; the model remains responsible for judging that the same condition persisted across those rounds and must explain it in blocked_reason. Provide only the fields that belong to the chosen action and omit the others. */
  update_goal: {
    ...
    /** Replacement objective; omit unless action is edit. */
    objective?: string;
    /** Replacement cap; omit unless action is edit. */
    max_goal_rounds?: number;
    /** Concrete blocking condition; required only with action blocked; omit for other actions. */
