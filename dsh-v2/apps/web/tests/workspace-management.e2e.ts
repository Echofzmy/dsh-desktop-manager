...
describe('web e2e: workspace management (create / rename / flat view / hover affordances)', () => {
  ...
  it('switches to the flat "In one list" view and persists the preference', async () => {
    ...
    // Grouped default: the Sessions segment is pressed and workspace group
    // rows render (the seeded session sits under Ungrouped; the created
    // workspaces are empty groups).
    await expect.poll(
    ...
    // Flat mode: the region stays in Sessions mode and the seeded session is
    // a top-level row with no group headers above it.
    await expect.poll(
      () => page.getByRole('button', { name: 'Sessions', pressed: true }).count(),
      { timeout: 10_000 },
    ).toBe(1)
