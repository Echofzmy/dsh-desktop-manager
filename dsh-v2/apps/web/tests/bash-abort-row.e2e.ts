...
describe.skipIf(MODE === 'record')('web e2e: cancelled Bash row disclosure', () => {
  ...
  it('expands a stopped aborted row to its command and full stop text', async () => {
    ...
    await expect.poll(() => call.getByText('Error: tool call aborted', { exact: true }).count()).toBe(0)
    ...
    // Expanded, the stop text appears exactly once — in the OUT section, never
    // in the collapsed summary — and the stopped row carries no red marker.
