...
实例行显示实例名、运行时、环境、端口、工作区与状态，并提供启动、停止、强制停止、重新启动和日志操作。运行时显示版本与预检状态；环境显示独立、克隆或生产类型及当前占用者。

### 5.2 新建实例与环境

侧栏和概览均可打开新建实例对话框。字段包括名称、运行时、工作区、环境和端口；端口为 `0` 时每次启动都由操作系统原子分配，并同时显示自动模式和当前实际端口。
...
实例工作面由 58px 工具栏与嵌入的 DSH Web GUI 构成。工具栏显示实例、运行时、环境、实际端口和工作区，并提供日志、外部浏览器、重新启动与停止操作。
