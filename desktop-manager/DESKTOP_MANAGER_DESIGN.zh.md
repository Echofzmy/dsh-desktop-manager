...
统一配置工作面由管理器 renderer 即时渲染，不选择运行时、不启动 DSH 进程，也不挂载额外 `WebContentsView`。左侧为“通用 / 模型”内部导航；通用页维护语言、主题、默认权限、忙碌时 Enter 行为、默认模型和默认思考深度，模型页预置 DeepSeek provider、`DEEPSEEK_API_KEY` 凭据槽位和 `https://api.deepseek.com`，其初始模型列表为空；同时维护自定义 pi-ai provider、Base URL、协议、请求/流空闲超时、模型自动发现和模型目录。自动发现读取 DeepSeek 及 OpenAI-compatible `GET /models`：主进程限制请求时间与响应体大小，已保存 Key 只能发送到该 provider 已保存的同一 Base URL，变更地址后必须使用一次性输入 Key；响应仅作为候选，用户确认后才加入 draft，现有模型不会被覆盖。新增自定义 provider 直接在列表末尾展开完整编辑器，不使用预备弹窗；provider ID、Base URL、协议和至少一个模型决定 route 是否可保存，显示名称选填并回退到 ID。已有 `agent-presets.default` 在读写和投影中透明保留，但不作为跨 runtime 的统一用户控件暴露。保存操作位于当前配置内容底部。API Key 输入为 write-only：已保存值只显示 configured 状态，成功写入后立即清空输入；启用 Full access 需要显式风险确认。自定义 route、协议、URL、模型 ID 和容量在主进程持久化边界再次验证。
