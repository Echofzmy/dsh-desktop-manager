...
function sortInstances(items: readonly InstanceRecord[]): InstanceRecord[] { return [...items].sort((a, b) => Date.parse(b.createdAt) - Date.parse(a.createdAt) || b.id.localeCompare(a.id)) }
function runtimeSourceRank(source: RuntimeRecord['source']): number { return source === 'bundled' ? 0 : source === 'downloaded' ? 1 : 2 }
function sortRuntimes(items: readonly RuntimeRecord[]): RuntimeRecord[] { return [...items].sort((a, b) => runtimeSourceRank(a.source) - runtimeSourceRank(b.source) || Date.parse(b.registeredAt) - Date.parse(a.registeredAt) || b.id.localeCompare(a.id)) }
...
export function App(): ReactNode {
  ...
  const [sidebarCollapsed, setSidebarCollapsed] = useState(() => localStorage.getItem('dsh-manager.sidebar-collapsed') === 'true')
  const [sidebarSettled, setSidebarSettled] = useState(() => localStorage.getItem('dsh-manager.sidebar-collapsed') === 'true')
  ...
  useEffect(() => {
    const duration = sidebarCollapsed ? 150 : 300
    setSidebarSettled(false)
    const timer = window.setTimeout(() => setSidebarSettled(true), duration)
    return () => window.clearTimeout(timer)
  }, [sidebarCollapsed])
  useEffect(() => { localStorage.setItem('dsh-manager.sidebar-collapsed', String(sidebarCollapsed)) }, [sidebarCollapsed])
  ...
  const sidebarWide = !sidebarCollapsed || !sidebarSettled
  return <div className={`app-shell ${log ? 'with-details' : ''}${sidebarCollapsed ? ' sidebar-collapsed' : ''}`}>
    <aside className={`sidebar${sidebarCollapsed && sidebarSettled ? ' collapsed' : ''}${sidebarCollapsed && !sidebarSettled ? ' fading' : ''}${!sidebarCollapsed && !sidebarSettled ? ' expanding' : ''}`}>
      <div className="drag-region" />
      <div className="brand-row">{sidebarWide ? <><img className="brand-icon" src={appIcon} alt="" aria-hidden="true" /><span><strong>dsh 管理器</strong></span><IconButton label="收起管理器侧栏" onClick={() => setSidebarCollapsed(true)}><PanelLeft size={16} /></IconButton></> : <IconButton label="展开管理器侧栏" onClick={() => setSidebarCollapsed(false)}><img className="brand-icon" src={appIcon} alt="dsh 管理器" /></IconButton>}</div>
      <button className="new-instance" title="新建实例" onClick={() => setModal('instance')} disabled={!snapshot.runtimes.length || !snapshot.environments.length}><Plus size={17} /><span>新建实例</span></button>
      <nav className="sidebar-nav" aria-label="主导航">
        <button className={page.kind === 'home' ? 'active' : ''} onClick={() => navigate({ kind: 'home' })}><LayoutDashboard size={17} /><span>概览</span></button>
        <button className={page.kind === 'runtimes' ? 'active' : ''} onClick={() => navigate({ kind: 'runtimes' })}><Boxes size={17} /><span>运行时</span></button>
        <button className={page.kind === 'models' ? 'active' : ''} onClick={() => navigate({ kind: 'models' })}><SlidersHorizontal size={17} /><span>统一配置</span></button>
      </nav>
      <div className="sidebar-section"><div className="sidebar-section-title"><span>实例</span><span>{snapshot.instances.length}</span></div><div className="sidebar-instances">
        ...
        {sortInstances(snapshot.instances).map(instance => <button key={instance.id} className={page.kind === 'instance' && page.id === instance.id ? 'active' : ''} onClick={() => navigate({ kind: 'instance', id: instance.id })}><StatusDot status={instance.status} /><span><strong>{instance.name}</strong><small>{instanceStatusLabel(instance)}</small></span></button>)}
      </div></div>
...
function RuntimesPage({ snapshot, busy, onModal, onRun, onLog, onConfirm, onError }: { snapshot: ManagerSnapshot; busy: string | null; onModal: (modal: Modal) => void; onRun: Runner; onLog: (taskId: string, name: string) => Promise<void>; onConfirm: (confirmation: Confirmation) => void; onError: (value: string | null) => void }): ReactNode {
  const runtimes = useMemo(() => sortRuntimes(snapshot.runtimes), [snapshot.runtimes])
  ...
  const [runtimeMenuId, setRuntimeMenuId] = useState<string | null>(null)
  return <div className="page narrow-page"><header className="page-heading"><div><h1>运行时</h1><p>官方版本、源码构建与启动门禁</p></div><div className="heading-actions"><button className="button outline" onClick={() => void check()}><RefreshCw size={16} />检查更新</button><button className="button outline" disabled={!snapshot.runtimes.some(runtime => runtime.source === 'local')} onClick={() => onModal('worktree')}><Copy size={16} />创建工作树</button><button className="button primary" onClick={() => onModal('runtime')}><Plus size={16} />注册本地运行时</button></div></header>
    ...
    <section className="content-section">{!snapshot.runtimes.length ? <EmptyState icon={<Boxes size={22} />} title="暂无运行时" detail="可以安装官方版本，或注册本地 DSH 源码目录。" action={<button className="button primary" onClick={() => onModal('runtime')}><Plus size={16} />注册运行时</button>} /> : <div className="runtime-list">{runtimes.map(runtime => {
      ...
      return <article className={`runtime-detail${expandedRuntimeIds.has(runtime.id) ? ' expanded' : ''}`} key={runtime.id}><header><button className="runtime-summary" type="button" aria-expanded={expandedRuntimeIds.has(runtime.id)} onClick={() => setExpandedRuntimeIds(current => { const next = new Set(current); if (next.has(runtime.id)) next.delete(runtime.id); else next.add(runtime.id); return next })}><span className="runtime-chevron">{expandedRuntimeIds.has(runtime.id) ? <ChevronDown size={17} /> : <ChevronRight size={17} />}</span><span><h3>{runtime.name}{snapshot.settings.defaultRuntimeId === runtime.id && <span className="inline-badge">默认</span>}</h3><p>{shortPath(runtime.path)}</p></span></button><span className={`preflight-badge ${runtime.preflight.ready && !runtime.taskBlocked ? 'ready' : 'blocked'}`}>{runtime.preflight.ready && !runtime.taskBlocked ? '可以启动' : '启动受阻'}</span><span className="runtime-actions"><IconButton label="更多操作" onClick={() => setRuntimeMenuId(current => current === runtime.id ? null : runtime.id)}><MoreHorizontal size={17} /></IconButton>{runtimeMenuId === runtime.id && <div className="runtime-menu" role="menu"><button type="button" role="menuitem" disabled={Boolean(activeTask) || busy !== null || runtime.source !== 'local' || runtime.immutable} onClick={() => { setRuntimeMenuId(null); void onRun(`task:${runtime.id}:install`, () => window.manager.startRuntimeTask(runtime.id, 'install')) }}>安装依赖</button><button type="button" role="menuitem" disabled={Boolean(activeTask) || busy !== null || runtime.source !== 'local' || runtime.immutable} onClick={() => { setRuntimeMenuId(null); void onRun(`task:${runtime.id}:typecheck`, () => window.manager.startRuntimeTask(runtime.id, 'typecheck')) }}>类型检查</button><button type="button" role="menuitem" disabled={Boolean(activeTask) || busy !== null || runtime.source !== 'local' || runtime.immutable} onClick={() => { setRuntimeMenuId(null); void onRun(`task:${runtime.id}:test`, () => window.manager.startRuntimeTask(runtime.id, 'test')) }}>运行测试</button>{snapshot.settings.defaultRuntimeId !== runtime.id && <button type="button" role="menuitem" onClick={() => { setRuntimeMenuId(null); void onRun(`default:${runtime.id}`, () => window.manager.setDefaultRuntime(runtime.id)) }}>设为默认</button>}{runtime.source !== 'bundled' && <button type="button" role="menuitem" className="danger-menu-item" onClick={() => { setRuntimeMenuId(null); onConfirm({ title: runtime.source === 'local' ? '注销运行时' : '删除运行时', detail: runtime.source === 'local' ? '只移除管理器记录，不会删除源码目录。仍被实例或回退记录引用时会拒绝操作。' : '删除管理器下载的完整运行时目录。仍被实例或回退记录引用时会拒绝操作。', actionLabel: runtime.source === 'local' ? '注销' : '删除', action: () => window.manager.deleteRuntime(runtime.id) }) }}> {runtime.source === 'local' ? '注销运行时' : '删除运行时'}</button>}</div>}</span></header>
        {expandedRuntimeIds.has(runtime.id) && <><div className="runtime-meta"><span>{runtime.source === 'bundled' ? '内置官方' : runtime.source === 'downloaded' ? '已下载官方' : '本地源码'}</span><span>{runtime.version ?? runtime.preflight.packageVersion ?? '版本未知'}</span>{runtime.preflight.gitCommit && <span>{runtime.preflight.gitCommit.slice(0, 10)}{runtime.preflight.gitDirty ? ' · 有修改' : ' · 干净'}</span>}</div>
        ...
        {runtime.source === 'local' && !runtime.immutable && <div className="task-toolbar"><button className="button outline small" disabled={Boolean(activeTask) || busy !== null} onClick={() => void onRun(`task:${runtime.id}:build`, () => window.manager.startRuntimeTask(runtime.id, 'build'))}><Wrench size={14} />完整构建</button></div>}
        ...
        <footer><span>检查于 {new Date(runtime.preflight.checkedAt).toLocaleString('zh-CN')}</span><div className="row-actions"><button className="button outline small" onClick={() => void onRun(`refresh:${runtime.id}`, () => window.manager.refreshRuntime(runtime.id))}><RefreshCw size={15} />重新预检</button></div></footer></>}
      </article> })}</div>}</section>
  </div>
}
...
function InstancePage({ instance, snapshot, busy, obscured, onRun, onLog, onError, onDelete, onSwitchEnvironment, onSaveTemplate }: { instance: InstanceRecord; snapshot: ManagerSnapshot; busy: string | null; obscured: boolean; onRun: Runner; onLog: (instance: InstanceRecord) => Promise<void>; onError: (error: string | null) => void; onDelete: () => void; onSwitchEnvironment: (environmentId: string) => Promise<void>; onSaveTemplate: () => void }): ReactNode {
  const host = useRef<HTMLDivElement>(null); const externalOpened = useRef(''); const [switchOpen, setSwitchOpen] = useState(false); const runtime = snapshot.runtimes.find(item => item.id === instance.runtimeId); const environment = snapshot.environments.find(item => item.id === instance.environmentId)
  useLayoutEffect(() => {
  ...
  return <div className="instance-page"><header className="instance-toolbar"><div className="instance-title"><StatusDot status={instance.status} /><span><strong>{instance.name}</strong><small>{runtime?.name} · {environment?.name} · {instance.port ? `127.0.0.1:${instance.port}` : '自动端口'}</small></span></div><div className="toolbar-actions"><IconButton label="查看日志" onClick={() => void onLog(instance)}><FileText size={17} /></IconButton><IconButton label="保存为模板" onClick={onSaveTemplate}><Copy size={17} /></IconButton><IconButton label="切换环境" disabled={active || busy !== null || environment?.kind === 'production'} onClick={() => setSwitchOpen(true)}><ArrowLeftRight size={17} /></IconButton><IconButton danger label="删除实例" disabled={active} onClick={onDelete}><Trash2 size={17} /></IconButton>{!recovery && <><IconButton label="在浏览器中打开" disabled={instance.status !== 'running'} onClick={() => void window.manager.openExternal(instance.id)}><ExternalLink size={17} /></IconButton><IconButton label="重新启动" disabled={instance.status !== 'running' || busy !== null} onClick={() => void onRun(`restart:${instance.id}`, () => window.manager.restartInstance(instance.id))}><RotateCw size={17} /></IconButton>{active ? <button className="button outline danger" onClick={() => void onRun(`stop:${instance.id}`, () => window.manager.stopInstance(instance.id, instance.status === 'stopping'))}><CircleStop size={16} />{instance.status === 'stopping' ? '强制停止' : '停止'}</button> : busy === `start:${instance.id}` ? <button className="button outline progress-button" disabled><LoaderCircle size={16} className="spin" />启动中</button> : <button className="button primary" onClick={() => void onRun(`start:${instance.id}`, () => window.manager.startInstance(instance.id))}><Play size={16} />启动</button>}</>}</div>{switchOpen && <EnvironmentSwitchDialog instance={instance} snapshot={snapshot} onClose={() => setSwitchOpen(false)} onSwitch={async environmentId => { await onSwitchEnvironment(environmentId); setSwitchOpen(false) }} />}</header><div className={`web-host ${instance.status !== 'running' || snapshot.settings.openMode === 'external' ? 'inactive' : ''}`} ref={host}>{(instance.status !== 'running' || snapshot.settings.openMode === 'external') && <div><TerminalSquare size={28} /><strong>{snapshot.settings.openMode === 'external' && instance.status === 'running' ? '已在浏览器打开' : instanceStatusLabel(instance)}</strong><span>{instance.lastError ?? (snapshot.settings.openMode === 'external' ? '可以使用右上角浏览器按钮再次打开。' : '启动实例后将在这里打开 DSH 界面。')}</span>{recovery && <div className="recovery-actions"><button className="button primary" disabled={busy !== null} onClick={() => recover(true)}>确认已停止，使用自动端口</button><button className="button outline" disabled={busy !== null || instance.port <= 0} onClick={() => recover(false)}>确认已停止，保留端口 {instance.port || ''}</button></div>}</div>}</div></div>
}

function EnvironmentSwitchDialog({ instance, snapshot, onClose, onSwitch }: { instance: InstanceRecord; snapshot: ManagerSnapshot; onClose: () => void; onSwitch: (environmentId: string) => Promise<void> }): ReactNode {
  const current = snapshot.environments.find(item => item.id === instance.environmentId)
  const choices = snapshot.environments.filter(item => item.id !== instance.environmentId && item.kind !== 'production' && !snapshot.instances.some(other => other.id !== instance.id && other.environmentId === item.id && ['starting', 'running', 'stopping'].includes(other.status)))
  const [environmentId, setEnvironmentId] = useState(choices[0]?.id ?? '')
  const [submitting, setSubmitting] = useState(false)
  const submit = async (event: FormEvent): Promise<void> => { event.preventDefault(); setSubmitting(true); try { await onSwitch(environmentId) } finally { setSubmitting(false) } }
  return <Dialog title="切换实例环境" onClose={onClose}><form className="form" onSubmit={event => void submit(event)}><p className="form-note">当前实例：{instance.name} · 当前环境：{current?.name ?? '未知'}</p><label className="field"><span>目标环境</span><select value={environmentId} onChange={event => setEnvironmentId(event.target.value)} required><option value="" disabled>选择目标环境</option>{choices.map(item => <option value={item.id} key={item.id}>{item.name}{item.kind === 'clone' ? '（克隆）' : '（独立）'}</option>)}</select></label><p className="form-note">只能切换已停止的非生产环境；实例工作区、端口和实例 ID 会保留。</p><footer><button className="button outline" type="button" onClick={onClose}>取消</button><button className="button primary" disabled={submitting || !environmentId}>{submitting ? '正在切换…' : '切换环境'}</button></footer></form></Dialog>
}
