...
`ApprovalOutcome` is closed and fail-closed. `allowed-once` grants only the asked-about action; callers deny on `blocked`, `rejected`, `cancelled`, and `unavailable`. A missing, non-owning, throwing, or non-conforming answerer becomes `unavailable` rather than opening the gate.
 ...
 * rejection.
 */
type ApprovalOutcome = 'allowed-once' | 'rejected' | 'blocked' | 'cancelled' | 'unavailable'
```

## Per-session policy

`ApprovalPolicy` determines what happens before interactive answerers run. `ask` delegates to the composed answerer chain, whose no-answer default is `unavailable`; `never` deterministically returns `blocked` without dispatching any answerer. The effective value is the last `approval/policy` event in the session log, falling back to the service config. `setApprovalPolicy(session, policy)` is the single write path, so replay reconstructs the override.
 ...
 * - `'never'` — never prompt anyone: every ask resolves `'blocked'`
 *   deterministically (the policy-blocked outcome, distinct from a human or
 *   answerer `'rejected'`). The strict headless stance (CI, unattended runs)
 *   and the policy whose outcome is knowable without asking.
 */
 ...
 * `'ask'`; the `??` only narrows the optional-input TYPE). Public so
 * consumers that shape model-visible surfaces (e.g. per-agent tool-schema
 * projections) can apply the same policy every ask resolves under.
