...
/**
 * Restore one archived session to every ordinary grouping surface. Its
 * workspace account was retained by archive, so its prior position returns.
...
/**
 * Forget a permanently deleted session from archive state, workspace
 * accounting, and the registry's header indexes. Persistence deletion is
 * owned by the caller and must commit before this cleanup begins.
 * @param sessionId - deleted session identity.
 * @returns resolution after all registry records are durable.
 */
