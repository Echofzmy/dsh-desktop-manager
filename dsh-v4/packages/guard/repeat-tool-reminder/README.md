...
`failureRetryThreshold` (default `2`, must be an integer >= 2) is a separate, failure-aware tier: when the PREVIOUS result for the same chain key was a canonical tool-result failure (`isError`), or the call was blocked by a downstream `tools/post-execute` listener, the retry at this count draws a corrective reminder instead of waiting for `thresholds` — so a model that keeps retrying a failing call hears about it one step earlier. The success path is untouched: an OK previous result never fires this tier, so ordinary repetition still follows `thresholds` exactly. Only values below `thresholds[0]` are earlier than the general first tier; at an equal count the failure reminder wins.
...
- **Denied calls count.** Detection sits on `tools/post-execute`, which also runs for calls a `tools/pre-execute` listener denied — a model hammering a denied call is exactly the loop worth breaking. Denied calls also register as failures for the `failureRetryThreshold` tier, since their canonical result is an `isError` result.
- **The failure signal is the canonical tool-result `isError`, plus downstream blocks.** A bash command that exits non-zero settles `isError: false` (the exit status is result data, per the harness's terminal-card semantics), so it neither triggers nor refreshes the failure tier — only a failed/blocked *result* does. The association is scoped to the exact `(tool, canonical arguments)` run: a different tracked call clears it.
...
### Failure-retry context message

#### What the model sees

When a call at `failureRetryThreshold` repeats a chain key whose previous result failed (`isError`) or was blocked, that agent receives the corrective reminder below instead of the general tiers (at an equal count the failure reminder wins). The `previous_error_code` line appears only when the previous result carried a structured error code (`ToolFailure.info.code`); the guard never parses the previous result's free-text message.
...
Each failure reminder is retained history, bounded the same way as the detailed form by `argumentsPreviewChars`; agents keep independent counters.
