...
describe('createFixtureApi', () => {
  ...
  it('workspace.browse lists one workspace level with file and directory rows, directories first', async () => {
    const api = createFixtureApi()
    const root = await api.workspace.browse(req({ workspaceId: 'fx-ws-fixture' as WorkspaceId }), new AbortController().signal)
    if (!root.result.ok) throw new Error('browse failed')
    expect(root.result.value).toEqual({
      relativePath: '',
      entries: [
        { name: 'notes', relativePath: 'notes', path: '/tmp/fixture/notes', kind: 'directory', hidden: false },
        { name: 'src', relativePath: 'src', path: '/tmp/fixture/src', kind: 'directory', hidden: false },
        { name: '.gitignore', relativePath: '.gitignore', path: '/tmp/fixture/.gitignore', kind: 'file', hidden: true },
        { name: 'AGENTS.md', relativePath: 'AGENTS.md', path: '/tmp/fixture/AGENTS.md', kind: 'file', hidden: false },
        { name: 'README.md', relativePath: 'README.md', path: '/tmp/fixture/README.md', kind: 'file', hidden: false },
      ],
      truncated: false,
    })

    // A child directory browses under the workspace root; its rows carry the
    // same workspace-rooted relativePath the client passes back verbatim.
    const child = await api.workspace.browse(req({ workspaceId: 'fx-ws-fixture' as WorkspaceId, relativePath: 'notes' }), new AbortController().signal)
    if (!child.result.ok) throw new Error('child browse failed')
    expect(child.result.value).toEqual({ relativePath: 'notes', entries: [], truncated: false })

    // The workspace-rooted home workspace lists its file rows under the root.
    const home = await api.workspace.browse(req({ workspaceId: 'fx-ws-home' as WorkspaceId }), new AbortController().signal)
    if (!home.result.ok) throw new Error('home browse failed')
    expect(home.result.value.entries.map(entry => entry.name)).toEqual(['package.json', 'README.md'])

    const missing = await api.workspace.browse(req({ workspaceId: 'fx-ws-void' as WorkspaceId }), new AbortController().signal)
    expect(missing.result).toMatchObject({ ok: false, error: { code: 'workspace-not-found', details: { workspaceId: 'fx-ws-void' } } })
    const unreadable = await api.workspace.browse(req({ workspaceId: 'fx-ws-fixture' as WorkspaceId, relativePath: 'void' }), new AbortController().signal)
    expect(unreadable.result).toMatchObject({ ok: false, error: { code: 'directory-unreadable', details: { path: '/tmp/fixture/void' } } })
  })
