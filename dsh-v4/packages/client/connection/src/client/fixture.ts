...
import type {
  ...
  ToolCallView, ToolEventView, ToolResultView, WorkspaceFileEntry, WorkspaceId, WorkspaceView,
} from './api.ts'
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  // by its parent lists as empty until something is created inside it. The
  // tree holds directories only — the picker capability (host.listDirectory)
  // lists directories, while workspace.browse adds file rows from fileTree.
  const directoryTree = new Map<string, string[]>([
    ['/', ['home']],
    ['/home', ['fixture']],
    // The default fixture workspace root, so workspace.browse can walk the
    // same tree the picker serves, plus the file rows fileTree adds.
    ['/tmp/fixture', ['notes', 'src']],
  ...
  // File rows per directory, consulted only by workspace.browse (the picker
  // capability's DirectoryEntry carries no kind, so it keeps serving
  // directories; the workspace browser reports every real child).
  const fileTree = new Map<string, string[]>([
    ['/tmp/fixture', ['AGENTS.md', 'README.md', '.gitignore']],
    [`${FIXTURE_HOME}/Documents/project`, ['package.json', 'README.md']],
  ])
  ...
  const api: ApiProxy = {
    ...
    workspace: {
      ...
      browse: (request) => {
        const { workspaceId, relativePath } = request.payload
        ...
        // Same workspace-rooted mapping the host backend applies: '' (absent)
        // is the workspace directory itself, children resolve under it. File
        // rows come from fileTree; directory rows from the shared tree. The
        // host backend sorts directories first, then files, each by name.
        const root = relativePath ?? ''
        const target = root === '' ? workspace.path : `${workspace.path}/${root}`
        const children = childrenOf(target)
        if (children === undefined) {
          return err(request, {
        ...
        const row = (name: string, kind: 'file' | 'directory'): WorkspaceFileEntry => ({
          name,
          relativePath: root === '' ? name : `${root}/${name}`,
          path: `${target}/${name}`,
          kind,
          hidden: name.startsWith('.'),
        })
        const entries = [
          ...(fileTree.get(target) ?? []).map(name => row(name, 'file')),
          ...[...children].map(name => row(name, 'directory')),
        ].sort((left, right) => left.kind === right.kind
          ? left.name.localeCompare(right.name)
          : left.kind === 'directory' ? -1 : 1)
        return ok(request, { relativePath: root, entries, truncated: false })
      },
      create: (request) => {
      ...
      unarchiveSession: (request) => {
        ...
        if (archiveIndex !== -1) archivedSessionIds.splice(archiveIndex, 1)
        emitHost({ type: 'host/archived-sessions-changed', archivedSessionIds: [...archivedSessionIds] })
        return ok(request, { archivedSessionIds: [...archivedSessionIds] })
      },
      deleteArchivedSession: (request) => {
        const { sessionId } = request.payload
        const at = sessions.findIndex(session => session.sessionId === sessionId)
...
export class FixtureApiClient extends AbstractApiClient {
  ...
  private dispatch(
    ...
    switch (method) {
      ...
      case 'workspace.browse': return this.api.workspace.browse(request, signal)
