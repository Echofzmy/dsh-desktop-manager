...
通用行把已知工具名称归类为 search、read、shell、write、edit、code 或 generic 变体。运行中、成功、失败、已停止和已阻止状态只来自冻结的 call/result slice，经 Runtime 的单一 `toolOutcomeKind` 分类器得出（基于结构化 `error.code`，绝不解析文本）：`blocked`（策略/审批拒绝、`INVALID_ARGS` 等）与 `stopped`（取消/中断）以非红警告点呈现，只有真正的 `error` 显示为红色。被阻止的行的收起拒绝行会去掉注册表的 `Error: ` 前缀（error 行保留原样）；展开后的输出文本绝不被改写。没有 terminal 卡片的已停止 Bash 调用与失败或拒绝一样保持可展开，可查看记录的 command 与完整输出——行保持警告（非红）样式，展开后的输出文本绝不被标红。只有用户调用 Host 打开文件回调时，文件路径才相对会话 `cwd` 解析；展示代码不读取会话服务。
