 ...
 *  semantic (error = red, blocked/stopped = amber halo). Running keeps the icon —
 ...
 *  row is running, failed, or interrupted. null in the ok and blocked states
 *  (the ok icon describes a settled row; a blocked row's collapsed summary IS
 *  the denial text, so the state is already readable). */
...
export function ToolRow({
  ...
  // An error, blocked, or stopped row's collapsed summary IS the outcome: the
  // first denial/failure/interruption line (error red; blocked and stopped
  // non-red) outranks both the args summary and a terminal description.
  const failureLine = state === 'error' || state === 'blocked' || state === 'stopped'
    ? errorSummary ?? null
    : null
          ...
          <>
            ...
            {fileLink ? (
              ...
              <span
                className={clsx(
                  css.summary,
                  failureLine !== null && (state === 'error' ? css.errorSummary : css.blockedSummary),
                )}
              >
                {summaryText}
              </span>
