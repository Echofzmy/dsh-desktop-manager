...
function leadingFor(state: ToolRowState) {
  switch (state) {
    ...
    case 'stopped':
    case 'blocked': return <StateDot state="warning" />
...
function stateStatus(state: ToolRowState, t: BashRowProps['t']): string | null {
  switch (state) {
    ...
    // A blocked row's collapsed summary IS the denial text, so the state is
    // already readable without a hidden label.
    default: return null
  }
}
...
export function BashRow({ toolName, block, sessionId, useSessions, inspect, t }: BashRowProps) {
  ...
  // Settled calls without terminal material (execution failures, policy or
  // approval denials, and user/parent stops) use the generic presenter. Keep
  // their recorded args and full result reachable instead of collapsing the
  // row to the first line; a stopped row keeps the warning (non-red) styling.
  const genericSettledBody = terminal === null
    && (model.state === 'error' || model.state === 'blocked' || model.state === 'stopped')
    && (model.body !== null || model.output !== null)
  const expandable = terminal !== null || genericSettledBody
  const open = expanded && expandable
  const failureLine = model.state === 'error' || model.state === 'blocked' || model.state === 'stopped'
    ? model.errorSummary
    : null
  const toggleExpand = () => {
    setExpanded(v => !v)
    ...
    <div className={css.card}>
      ...
      >
        ...
        {/* The terminal presenter's description is the contractual
            above-card summary; a failure/denial's first line outranks both. */}
        <span className={clsx(css.summary, failureLine !== null
          && (model.state === 'error' ? css.errorSummary : css.blockedSummary))}>
