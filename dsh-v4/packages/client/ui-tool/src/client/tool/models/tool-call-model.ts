...
import { abbreviateHomePath, toolOutcomeKind } from '@deepseek-ai/dsh-client-runtime/client'
import type { ToolCallBlock, ToolResultNode } from '@deepseek-ai/dsh-client-runtime/client'

export type { ToolCallBlock } from '@deepseek-ai/dsh-client-runtime/client'
export type { ToolOutcomeKind } from '@deepseek-ai/dsh-client-runtime/client'
 ...
 * `blocked` is a refusal or correctable condition (policy/approval deny,
 * `INVALID_ARGS`, …) and renders non-red, like the non-red `stopped`
 * cancellation state; only `error` is a real failure.
 */
export type ToolRowState = 'running' | 'ok' | 'error' | 'stopped' | 'blocked'
...
export interface ToolRowModel {
  ...
  output: string | null
  /** First line of the result text on an error or blocked row; null for every other state. */
...
/**
 * Strip the canonical `Error: ` envelope the registry writes into denied and
 * failed result content. Display-only: the durable text is untouched. An
 * error row keeps the envelope (the message IS the failure); a blocked row's
 * collapsed denial line drops it so the amber (non-red) state reads like a
...
export function toolRowModel(toolName: string, block: ToolCallBlock, cwd?: string, home?: string): ToolRowModel {
  ...
  const outcome = done ? toolOutcomeKind(block) : undefined
  const state: ToolRowState = outcome === undefined ? 'running'
    : outcome === 'success' ? 'ok'
      : outcome
  const base = argsRaw === ''
    ? block.callId
  ...
  // A blocked, stopped, or error row shows its outcome line as the collapsed
  // summary (the text is the state); only the blocked line drops the
  // registry's `Error: ` envelope — error and stopped rows keep it verbatim.
  const errorSummary = output === null
    ? null
    : state === 'error' || state === 'stopped'
      ? firstLine(output)
      : state === 'blocked'
        ? withoutErrorEnvelope(firstLine(output))
        : null
  return {
    variant,
