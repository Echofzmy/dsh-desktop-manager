...
describe('WorkspaceRuntime', () => {
  ...
  it('browses workspace levels through workspace.browse, forwarding payload and signal and wrapping business errors', async () => {
    const ctx = new Context()
    const api = new FakeApiClient()
    const workspaces = new WorkspaceRuntime(ctx, api, new SessionRuntime(ctx, api, fakeRemote()))
    const listing = {
      relativePath: '',
      entries: [
        { name: 'src', relativePath: 'src', path: '/w/alpha/src', kind: 'directory' as const, hidden: false },
        { name: 'README.md', relativePath: 'README.md', path: '/w/alpha/README.md', kind: 'file' as const, hidden: false },
      ],
      truncated: false,
    }
    api.onWorkspaceBrowse = () => Promise.resolve(ok(listing))
    await expect(workspaces.browse(wid('alpha'))).resolves.toEqual(listing)
    await expect(workspaces.browse(wid('alpha'), 'src')).resolves.toEqual(listing)
    // The optional path is omitted from the payload, not sent as undefined.
    expect(api.callsOf('workspace.browse')).toEqual([
      { workspaceId: 'alpha' },
      { workspaceId: 'alpha', relativePath: 'src' },
    ])
    const scan = new AbortController()
    await workspaces.browse(wid('alpha'), 'src', scan.signal)
    expect(api.lastBrowseSignal).toBe(scan.signal)
    api.onWorkspaceBrowse = () => Promise.resolve(err({
      code: 'workspace-not-found', message: 'gone', details: { workspaceId: 'ghost' },
    }))
    const missing = workspaces.browse(wid('ghost'))
    await expect(missing).rejects.toBeInstanceOf(DirectoryBrowseError)
    await expect(missing).rejects.toMatchObject({ rpcError: { code: 'workspace-not-found' } })
    api.onWorkspaceBrowse = () => Promise.resolve(err({
      code: 'directory-unreadable', message: 'denied', details: { path: '/w/alpha' },
    }))
    const denied = workspaces.browse(wid('alpha'))
    await expect(denied).rejects.toBeInstanceOf(DirectoryBrowseError)
    await expect(denied).rejects.toMatchObject({ rpcError: { code: 'directory-unreadable' } })
  })
  ...
  it('keeps an open session viewable when a remote archive frame outruns a stale baseline', async () => {
    const ctx = new Context()
    const api = new FakeApiClient()
    const sessions = new SessionRuntime(ctx, api, fakeRemote())
    const workspaces = new WorkspaceRuntime(ctx, api, sessions)
    ...
    // archive frame lands: the archive set survives the baseline's later
    // resolution, while the selected session stays open so archived history
    // remains viewable.
    const gate = deferred<Awaited<ReturnType<FakeApiClient['onWorkspaceList']>>>()
    api.onWorkspaceList = () => gate.promise
