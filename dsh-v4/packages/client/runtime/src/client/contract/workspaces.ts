 ...
 */
import type {
  DirectoryListing, SessionId, WorkspaceDirectoryListing, WorkspaceId, WorkspaceView,
...
export interface IWorkspaces {
  ...
  /**
   * List one directory level of a Workspace through the `workspace.browse`
   * RPC: the workspace-rooted browser, distinct from the Host-wide picker
   * (`listDirectory`). An absent `relativePath` (or `''`) lists the workspace
   * root; a child directory row's `relativePath` is passed back verbatim.
   * @param workspaceId - owning workspace.
   * @param relativePath - workspace-rooted directory; absent lists the root.
   * @param signal - aborts the wire request (and the Host's scan) when the caller supersedes it.
   * @returns the level's file and directory rows, directories first.
   */
  browse(workspaceId: WorkspaceId, relativePath?: string, signal?: AbortSignal): Promise<WorkspaceDirectoryListing>
  /**
  ...
  /** Restore an archived session to ordinary grouping surfaces. */
