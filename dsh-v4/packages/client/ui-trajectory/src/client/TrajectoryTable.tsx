...
import { cellOutcome, formatElapsedSeconds, trajectoryRecordId } from './trajectory-record.ts'
import {
...
type DetailTab =
  ...
  | 'timing'
  | 'diff'
type RecordState = 'complete' | 'running' | 'error' | 'stopped' | 'blocked'
...
function stateOf(record: TableRecord): RecordState {
  const outcome = cellOutcome(record.cell)
...
/**
 * Accessible state suffix for a settled non-success tool record's row label
 * (`, Stopped` / `, Blocked` / `, Failed`); null for successful, running, and
 * non-tool records, whose text already describes them.
 */
function stateWord(record: TableRecord): string | null {
  if (record.cell.isError !== true && record.cell.state === undefined) return null
  const state = stateOf(record)
  return state === 'complete' || state === 'running' ? null : `, ${statusLabel(state)}`
}
...
function RecordPayload({
  ...
  const error = direction === 'output' && cellOutcome(record.cell) === 'error'
          ...
          <tbody>
            ...
            {renderedRecords.map(({ record, position, terminalRequestBoundary }) => (
              ...
              >
                ...
                ?? (cellOutcome(record.cell) === 'error' ? 'error' : undefined)
                  ...
                  return (
                    <tr
                      ...
                      aria-label={isCollapsedSummary
                        ...
                        : isRequestOnly
                          ? `Request ${request ?? ''}${stateWord(record) ?? ''}`
                          : `${request === undefined ? '' : `Request ${request}, `}${KIND_LABEL[record.cell.kind]}, ${listDisplayText || 'no content'}${stateWord(record) ?? ''}`}
                      ...
                      data-error={cellOutcome(record.cell) === 'error' || undefined}
                            ...
                            : (
                              ...
                              >
                                ...
                                {resultText !== undefined && (
                                  <span className={cellOutcome(record.cell) === 'error'
