    ...
    if (entry.kind === 'request') {
      ...
      pushStep(request.turn, request.step, [{
        ...
        cell: {
          ...
          // A failed request keeps the legacy `isError` flag AND the
          // classifier outcome; an interrupted (user/parent stop) request
          // carries only the `stopped` outcome so it renders non-red.
          ...(request.status === 'error' ? { isError: true, state: 'error' as const } : {}),
          ...(request.status === 'stopped' ? { state: 'stopped' as const } : {}),
        },
      }])
