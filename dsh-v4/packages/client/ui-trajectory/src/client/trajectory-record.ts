...
import type { ConversationPromptSnapshot, ToolOutcomeKind } from '@deepseek-ai/dsh-client-runtime/client'
...
export interface TrajectoryCellProps extends HTMLAttributes<HTMLDivElement> {
  ...
  isError?: boolean
  /**
   * Tool-only settled outcome from the Runtime's single classifier
   * (`success` / `stopped` / `blocked` / `error`). Absent on cells that carry
   * no tool result (messages, context, boundaries); consumers fall back to
   * {@link isError} for legacy fixtures without it.
   */
  state?: ToolOutcomeKind
...
/**
 * Resolve one cell's settled outcome, preferring the classifier-derived
 * `state` and falling back to the legacy boolean `isError` for cells and
 * fixtures that predate it.
 * @param cell - projected trajectory record cell.
 * @returns the settled outcome kind.
 */
export function cellOutcome(cell: TrajectoryCellProps): ToolOutcomeKind {
  return cell.state ?? (cell.isError === true ? toolOutcomeKind({ isError: true }) : 'success')
}
