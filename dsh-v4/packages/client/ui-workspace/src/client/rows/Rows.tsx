...
/**
 * Render one archived session row with restore and permanent-delete actions.
 * @param props - archived row data and browser-owned action callbacks.
 * @returns the archived session row.
 */
export function ArchivedSessionNodeItem({ node, currentId, now, onOpen, onRestore, onDelete, t }: {
  node: SessionNode
  currentId: string | undefined
  now: number
  ...
  const [menuOpen, setMenuOpen] = useState(false)
  const busy = node.running || node.runningSubagentCount > 0
  return (
    <HoverCard
      disabled={menuOpen}
      ...
      anchor={(
        <div
          className={clsx(css.sessionRow, node.id === currentId && css.selected, menuOpen && css.menuOpen)}
          role="treeitem"
          aria-selected={node.id === currentId}
          onClick={() => { onOpen(node.id) }}
        >
          <span className={css.slot}>{showStatus && <SessionStatusDots statuses={statuses} />}</span>
          ...
          <span className={css.rowActions}>
            <Menu
              ...
              items={[
                ...
                { id: 'delete', label: t('menu.deleteSession'), icon: <IconTrashOutline16 />, disabled: busy },
              ]}
              onSelect={(id) => {
                setMenuOpen(false)
              ...
              anchor={(
                <button
                  type="button"
                  className={css.iconButton}
                  aria-label={t('actions.archivedSession.aria', { name: title })}
