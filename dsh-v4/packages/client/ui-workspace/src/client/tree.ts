...
/**
 * Derive archived rows in archive order. Missing list summaries are skipped
 * until the session baseline catches up; subagent descendants remain folded
 * into the same status projection as ordinary rows.
 * @param list - sessions list snapshot.
 * @param archivedSessionIds - registry-global archive order.
 * @returns archived rows that still have session metadata.
 */
export function deriveArchived(
  ...
  return archivedSessionIds.flatMap((id) => {
    const summary = list.byId[id]
    return summary === undefined ? [] : [sessionNode(summary, descendants)]
  })
}
