...
import type {
  SessionId, SessionSearchResultItem, WorkspaceDirectoryListing, WorkspaceId, WorkspaceView,
...
export type WorkspaceBrowserInjected = {
  ...
  /** Restore an archived Session to ordinary lists without changing its Workspace slot. */
  unarchiveSession: (sessionId: SessionId) => Promise<void>
  /** Permanently delete an archived Session and its durable log. */
  ...
  /**
   * List one directory level of a Workspace through `workspace.browse`: the
   * workspace-rooted file tree. An absent `relativePath` (or `''`) lists the
   * workspace root; a child directory row's `relativePath` is passed back
   * verbatim. Directories come first, as the Host sorts them.
   */
  browse: (
  ...
  /**
   * Open a workspace file with the Host operating system's default
   * application; the file tree passes the row's absolute path verbatim.
   */
