...
const filesCss = readFileSync(fileURLToPath(new URL('../src/client/files/FilesBrowser.module.css', import.meta.url)), 'utf8')
...
const filesDeclarations = (selector: string): Map<string, string> | undefined => declarationsFrom(filesCss, selector)
...
describe('WorkspaceBrowser.module.css list', () => {
  ...
  it('keeps the file tree rows at a stable height with depth-scaled insets and clipped names', () => {
    expect(filesDeclarations('.row')?.get('height')).toBe('32px')
    expect(filesDeclarations('.row')?.get('padding-left')).toBe('var(--dsh-files-indent)')
    expect(filesDeclarations('.row + .row')?.get('margin-top')).toBe('2px')
    expect(filesDeclarations('.name')?.get('overflow')).toBe('hidden')
    expect(filesDeclarations('.name')?.get('text-overflow')).toBe('ellipsis')
    expect(filesDeclarations('.name')?.get('white-space')).toBe('nowrap')
    expect(filesDeclarations('.list')?.get('scrollbar-gutter')).toBe('stable')
  })

  it('keeps the mode switch and files chrome within the header geometry', () => {
    expect(declarations('.modeSwitch')?.get('height')).toBe('26px')
    expect(declarations('.modeButton')?.get('height')).toBe('24px')
    expect(declarations('.filesHeaderTitle')?.get('overflow')).toBe('hidden')
    expect(declarations('.filesHeaderTitle')?.get('text-overflow')).toBe('ellipsis')
    expect(declarations('.filesHeaderTitle')?.get('white-space')).toBe('nowrap')
    expect(declarations('.filesRail')?.get('width')).toBe('36px')
    expect(declarations('.filesRailButton')?.get('width')).toBe('36px')
  })

  it('pins the files actions to two compact 28px icon controls in an exact 60px cluster', () => {
    // Refresh and the show-hidden toggle share the compact icon geometry.
    for (const selector of ['.iconButton', '.filesToggle']) {
      const rule = declarations(selector)
      expect(rule?.get('width')).toBe('28px')
      expect(rule?.get('height')).toBe('28px')
      expect(rule?.get('border-radius')).toBe('50%')
      expect(rule?.get('padding')).toBe('0')
    }
    // The pressed toggle keeps the solid active plate over the icon hover.
    expect(declarations('.filesToggleActive')?.get('background'))
      .toBe('var(--dsw-alias-interactive-bg-hover-solid)')
    // The files cluster is declared after the generic rule so the
    // same-specificity cascade lands on its exact width — 28 + 4 + 28 — and
    // the title truncation gets a deterministic remainder.
    expect(declarations('.filesHeaderActions')?.get('max-width')).toBe('60px')
    expect(declarations('.filesHeaderActions')?.get('width')).toBe('60px')
    expect(css.indexOf('.filesHeaderActions')).toBeGreaterThan(css.indexOf('.headerActions'))
  })
})
