...
declare module '@deepseek-ai/dsh-session/types' {
  interface SessionEventMap {
     ...
     * policy block (`'blocked'`), a cancellation, or the fail-closed
     * `'unavailable'`.
     */
    'approval/decided': {
      id: ApprovalRequestId
...
const OUTCOMES: readonly ApprovalOutcome[] = ['allowed-once', 'rejected', 'blocked', 'cancelled', 'unavailable']
...
export class ApprovalService extends Service {
  ...
  private async decide(req: ApprovalRequest, session: Session): Promise<ApprovalOutcome> {
    ...
    // documented promise that 'never' blocks deterministically regardless
    // of registration order — only the service's own request path can.
    if (this.effectivePolicy(session) === 'never') return 'blocked'
