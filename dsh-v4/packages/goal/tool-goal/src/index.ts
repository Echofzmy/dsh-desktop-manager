...
/** Action that owns one conditional update_goal field. */
const FIELD_OWNER: Record<string, 'edit' | 'blocked'> = {
  objective: 'edit',
  max_goal_rounds: 'edit',
  blocked_reason: 'blocked',
}

/**
 * Conditional fields provided with a meaningful value yet invalid for one
 * action. Empty-string and zero fillers stay normalized away exactly like the
 * execute replacements, so only genuine mis-fills surface.
 */
function invalidConditionalFields(
  action: UpdateAction,
  ...
  if (action !== 'edit' && hasText(objective)) invalid.push('objective')
  if (action !== 'edit' && hasRoundCap(maxGoalRounds)) invalid.push('max_goal_rounds')
  if (action !== 'blocked' && hasText(blockedReason)) invalid.push('blocked_reason')
  return invalid
}

/** Throw one precise omit-style GOAL_TOOL_INVALID_UPDATE naming the fields mis-filled for the selected action. */
function throwInvalidConditionalFields(
  action: UpdateAction,
  objective: string | undefined,
  maxGoalRounds: number | undefined,
  blockedReason: string | undefined,
): void {
  const invalid = invalidConditionalFields(action, objective, maxGoalRounds, blockedReason)
  if (invalid.length === 0) return
  throw new HarnessError(
    invalid
      .map(field => `omit "${field}" when action is "${action}": "${field}" is valid only with action "${FIELD_OWNER[field]}"`)
      .join('; '),
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Update the exact current goal revision. edit, pause, and resume require a direct '
      ...
      + 'Provide only the fields that belong to the chosen action and omit the others.',
    parameters: {
      ...
      objective: { type: 'string', description: 'Replacement objective; omit unless action is edit.' },
      max_goal_rounds: { type: 'number', description: 'Replacement cap; omit unless action is edit.' },
      blocked_reason: {
        type: 'string',
        description: 'Concrete blocking condition; required only with action blocked; omit for other actions.',
      },
    },
    output: GOAL_OUTPUT,
    execute(args, exec) {
      ...
      if (args.action === 'edit') {
        requireDirectHuman(ctx, execution)
        throwInvalidConditionalFields(args.action, args.objective, args.max_goal_rounds, args.blocked_reason)
