...
describe('goal tool state transitions', () => {
  ...
  it('rejects a fractional revision at the schema level, before authority checks', async () => {
    const { ctx, root } = await harness()
    openTurn(root, { kind: 'user' })
    const fractional = await execute(ctx, 'update_goal', {
      goal_id: 'goal-1', revision: 2.5, action: 'pause',
    }, root.agent)
    expect(fractional.error?.info?.code).toBe('INVALID_ARGS')
    expect(fractional.error?.message).toContain('"revision" must be an integer')
  })

  it('accepts an integer-valued revision and keeps the exact CAS ref', async () => {
    const { ctx, root } = await harness()
    openTurn(root, { kind: 'user' })
    const created = ctx.goals.create(root.agent, { objective: 'float revision' })
    const paused = await execute(ctx, 'update_goal', {
      goal_id: created.id, revision: 1.0, action: 'pause',
    }, root.agent)
    expect(resultGoal(paused)).toMatchObject({ phase: 'paused', revision: 2 })
  })

  it('names each mis-filled conditional field and its owning action in omit-style errors', async () => {
    const { ctx, root } = await harness()
    openTurn(root, { kind: 'user' })
    const created = ctx.goals.create(root.agent, { objective: 'valid' })
    const paused = await execute(ctx, 'update_goal', {
      goal_id: created.id, revision: created.revision, action: 'pause', objective: 'not for pause',
    }, root.agent)
    expect(paused.error?.info?.code).toBe('GOAL_TOOL_INVALID_UPDATE')
    expect(paused.error?.message).toContain('omit "objective" when action is "pause"')
    expect(paused.error?.message).toContain('"objective" is valid only with action "edit"')
    const complete = await execute(ctx, 'update_goal', {
      goal_id: created.id, revision: created.revision, action: 'complete', blocked_reason: 'Not a blocker.',
    }, root.agent)
    expect(complete.error?.info?.code).toBe('GOAL_TOOL_INVALID_UPDATE')
    expect(complete.error?.message).toContain('omit "blocked_reason" when action is "complete"')
    expect(complete.error?.message).toContain('"blocked_reason" is valid only with action "blocked"')
    const edited = await execute(ctx, 'update_goal', {
      goal_id: created.id, revision: created.revision, action: 'edit', blocked_reason: 'Not for edit.',
    }, root.agent)
    expect(edited.error?.info?.code).toBe('GOAL_TOOL_INVALID_UPDATE')
    expect(edited.error?.message).toContain('omit "blocked_reason" when action is "edit"')
    const mixed = await execute(ctx, 'update_goal', {
      goal_id: created.id, revision: created.revision, action: 'pause',
      objective: 'no', max_goal_rounds: 5, blocked_reason: 'no',
    }, root.agent)
    expect(mixed.error?.info?.code).toBe('GOAL_TOOL_INVALID_UPDATE')
    expect(mixed.error?.message).toContain('omit "objective" when action is "pause"')
    expect(mixed.error?.message).toContain('omit "max_goal_rounds" when action is "pause"')
    expect(mixed.error?.message).toContain('omit "blocked_reason" when action is "pause"')
  })
