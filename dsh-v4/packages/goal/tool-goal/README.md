...
- `update_goal(goal_id, revision, action, objective?, max_goal_rounds?, blocked_reason?)` supports `edit`, `pause`, `resume`, `complete`, and `blocked`. `revision` must be a positive integer. Replacements belong only to `edit`; `blocked_reason` is required only for `blocked` and is persisted with the stable code `model-reported`. The schema explicitly requires omitting every field that does not belong to the selected `action`; strict-schema empty-string and zero fillers still count as omitted.
