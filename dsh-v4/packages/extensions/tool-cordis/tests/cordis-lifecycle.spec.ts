...
describe('Tool inspect provider (listTools) projection routing', () => {
  const projectingTool = defineTool({
    name: 'projectable',
    description: 'static catalog description',
    parameters: { mode: { type: 'string', enum: ['a', 'b'] } },
    output: { schema: { type: 'json' }, render: () => [] },
    projectSchema: ({ agent }) => ({
      description: `projected for ${(agent as unknown as { id: string }).id}`,
      parameters: { mode: { type: 'string', enum: ['a'] } },
    }),
  ...
  it('listTools returns the calling agent dynamic projection, while schemas(scope) stays static', async () => {
    const ctx = new Context()
    await ctx.plugin(SystemPrompt)
    await ctx.plugin(ToolRuntime)
    ctx.tools.register(projectingTool)

    const provider = hostInspectProviders(ctx).find(entry => entry.manifest.id === 'Tool')
    expect(provider).toBeDefined()
    const signal = new AbortController().signal
    const data = await provider!.query('listTools', undefined, { agent: fakeAgent('inspect-agent'), signal }) as {
      tools: Array<{ name: string; description: string; parameters: Record<string, unknown> }>
    }
    const tool = data.tools.find(entry => entry.name === 'projectable')
    expect(tool?.description).toBe('projected for inspect-agent')
    expect(tool?.parameters).toEqual({ type: 'object', properties: { mode: { type: 'string', enum: ['a'] } } })

    // The static catalog projection (schemas(scope)) is untouched by the provider.
    const staticSchema = ctx.tools.schemas(fakeAgent('inspect-agent')).find(entry => entry.name === 'projectable')
    expect(staticSchema?.description).toBe('static catalog description')
    expect(staticSchema?.parameters).toEqual({ type: 'object', properties: { mode: { type: 'string', enum: ['a', 'b'] } } })
  })
})
