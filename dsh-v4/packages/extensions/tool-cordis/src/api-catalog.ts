  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        description: 'The session\'s effective policy: its own `approval/policy` fold, else the configured default (the schema already defaulted an omitted policy to `\'ask\'`; the `??` only narrows the optional-input TYPE). Public so consumers that shape model-visible surfaces (e.g. per-agent tool-schema projections) can apply the same policy every ask resolves under.',
  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        description: 'Permanently delete one materialized cold session. Implementations must serialize this operation against their append/read coordinator and reject while the identity is live or exclusively prepared.',
        parameters: [{ name: '_id', description: 'persisted session to delete.' }],
  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        parameters: [{ name: 'name', description: 'the tool name as registered.' }, { name: 'scope', description: 'the pure viewing scope key; omitted = the global view.' }],
        returns: 'the definition the scope resolves, or undefined when none is visible.',
      },
      {
        signature: 'schemas(scope?: ScopeKey): ToolSchema[]',
        description: 'Project visible definitions onto the allowlisted model-facing schema fields, excluding execution and presentation callbacks. This is the STATIC projection: it never invokes per-agent `projectSchema`, so it suits diagnostics and tool-catalog consumers. Model-bound surfaces (system prompt assembly, `schemasFor`) use the per-agent projection instead.',
        parameters: [{ name: 'scope', description: 'the pure viewing scope key; omitted = the global view.' }],
        returns: 'one deep-cloned static schema per visible tool.',
      },
      {
        signature: 'schemasFor(agent: Agent): ToolSchema[]',
        description: 'Project the schemas the given live agent sees, applying each visible tool\'s optional `projectSchema` per agent. The agent is the real calling agent (its session policy/mode drive the projection); `schemas(scope)` remains the static/diagnostic projection.',
        parameters: [{ name: 'agent', description: 'the exact live agent the schemas are assembled for.' }],
  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        description: 'Restore one archived session to every ordinary grouping surface. Its workspace account was retained by archive, so its prior position returns.',
      ...
      },
      {
        ...
        description: 'Forget a permanently deleted session from archive state, workspace accounting, and the registry\'s header indexes. Persistence deletion is owned by the caller and must commit before this cleanup begins.',
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  },
  {
    ...
    declaration: 'export type ApprovalOutcome = \'allowed-once\' | \'rejected\' | \'blocked\' | \'cancelled\' | \'unavailable\';',
  },
  {
    name: 'ApprovalPolicy',
  ...
  {
    name: 'ProjectedToolSchema',
    declaration: 'export interface ProjectedToolSchema {\n    description: string;\n    parameters: Record<string, unknown>;\n}',
  },
  {
  ...
  {
    name: 'ToolRenderContext',
    declaration: 'export interface ToolRenderContext {\n    readonly agent?: Agent;\n}',
  },
  {
  ...
  {
    name: 'ToolSchemaProjectionContext',
    declaration: 'export interface ToolSchemaProjectionContext {\n    readonly agent: Agent;\n}',
  },
  {
