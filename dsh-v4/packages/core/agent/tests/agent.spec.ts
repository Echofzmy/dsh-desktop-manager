...
describe('AgentRegistry factory seam', () => {
  ...
  it('retires only an exact idle top-level agent through its structural factory', async () => {
    const ctx = new Context()
    await ctx.plugin(AgentRegistry)
    const retired: Agent[] = []
    const factory = stubFactory().factory
    factory.retireIdleAgent = async (agent) => {
      retired.push(agent)
      return true
    }
    ctx.agents.setFactory(factory)

    const root = stubAgent('retire-root')
    const running = stubAgent('retire-running', { status: 'running' })
    const child = stubAgent('retire-child')
    ctx.agents.enter(root, undefined)
    ctx.agents.enter(running, undefined)
    ctx.agents.enter(child, root)

    await expect(ctx.agents.retireIdleRoot(root.id)).resolves.toBe('retired')
    await expect(ctx.agents.retireIdleRoot(running.id)).resolves.toBe('busy')
    await expect(ctx.agents.retireIdleRoot(child.id)).resolves.toBe('owned')
    await expect(ctx.agents.retireIdleRoot(SessionId('missing'))).resolves.toBe('not-found')
