...
export interface AgentFactory {
  ...
  /** Retire this exact idle Agent when the factory still structurally owns it. */
  retireIdleAgent?(agent: Agent): Promise<boolean>
}

/** Outcome of a constrained request to retire one live top-level idle Agent. */
export type AgentRetirementResult = 'retired' | 'not-found' | 'busy' | 'owned' | 'unsupported'
...
export class AgentRegistry extends Service {
  ...
  /**
   * Ask the structural factory owner to retire one exact idle top-level Agent.
   * Running agents and child agents are never affected. Object identity remains
   * the final stale-lifecycle boundary inside the factory.
   */
  async retireIdleRoot(id: SessionId): Promise<AgentRetirementResult> {
    const entry = this.store.get(id)
    if (entry === undefined) return 'not-found'
    if (entry.owner !== undefined) return 'owned'
    if (entry.agent.status !== 'idle') return 'busy'
    const retire = this.requireFactory().target.retireIdleAgent
    if (retire === undefined) return 'unsupported'
    return await retire.call(this.requireFactory().target, entry.agent) ? 'retired' : 'unsupported'
  }
