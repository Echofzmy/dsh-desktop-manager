...
/** Read-only per-call identity available to a tool's renderer for deterministic decisions (e.g. the calling agent's policy). */
export interface ToolRenderContext {
  /** The live agent the rendered call belongs to; absent when the call has none. */
  readonly agent?: Agent
}

/** Context for one per-agent tool-schema projection. */
export interface ToolSchemaProjectionContext {
  /** The exact live agent the schema is being assembled for — never a bare scope key. */
  readonly agent: Agent
}

/**
 * One tool's per-agent projection at the RAW registry level: replacement
 * model-facing description and the FINAL compiled object-rooted JSON Schema
 * for the arguments (the same shape as `ToolSchema.parameters`). Authoring
 * tools return the DSL property map through `defineTool`, which compiles it
 * here; the registry only snapshots.
 */
export interface ProjectedToolSchema {
  /** Replacement model-facing description. */
  description: string
  /** Replacement compiled JSON Schema object for the arguments (never the author-style property map). */
...
export interface ToolOutputDefinition {
   ...
   * An optional render context carries per-call identity (the calling agent)
   * so a tool can make deterministic per-agent decisions (e.g. whether to
   * advertise escalation guidance) without side effects.
   */
  render(args: unknown, value: JsonValue, context?: ToolRenderContext): ContentBlock[]
...
export interface ToolDefinition extends ToolSchema {
  ...
  /**
   * Optional per-agent schema projection. When present, every assembly for a
   * real agent (system-prompt tools, Code Mode SDK, `schemasFor`) calls it
   * with the exact live {@link Agent}; it returns replacement model-facing
   * `description`/`parameters`, or `undefined` to keep the static schema.
   * The projection must be a pure function of the context's agent — it is
   * evaluated per assembly and its result is losslessly snapshotted. Static
   * paths (`schemas(scope)`, registration, replay) never invoke it, and
   * omitting schema fields is NEVER enforcement: execution must still reject
   * calls that bypass the projection.
   * @param context - the projection context carrying the real calling agent.
   * @returns the projected description/parameters, or `undefined` for the static schema.
   */
  projectSchema?(context: ToolSchemaProjectionContext): ProjectedToolSchema | undefined
  /**
...
/**
 * Canonical failure code for a tool call denied BEFORE dispatch by the
 * extensible policy pipeline when no more specific code applies: a
 * `tools/pre-execute` listener's own `deny` decision without a `code`, or a
 * monotonic `tools.guard()` denial. An approval-routed deny keeps its more
 * specific `TOOL_APPROVAL_*` code instead (see {@link TOOL_APPROVAL_ERROR_CODES}).
 */
export const TOOL_POLICY_DENIED = 'TOOL_POLICY_DENIED'

/**
 * Canonical failure code for a tool call blocked AFTER dispatch by a
 * `tools/post-execute` `block` decision (corrective feedback turned into an
 * error result).
 */
export const TOOL_POST_BLOCKED = 'TOOL_POST_BLOCKED'

/**
 * Stable failure codes for tool calls denied through the approval seam
 * (`serviceAsk` routing of an `ask` decision), surfaced as
 * `ToolFailure.info.code` on the denied result so retry/sandbox/replay and the
 * client outcome classifier can route without parsing the message:
 *
 * - `TOOL_APPROVAL_BLOCKED` — the session approval policy (`'never'`)
 *   deterministically blocked the ask before any answerer saw it.
 * - `TOOL_APPROVAL_REJECTED` — a human or answerer explicitly rejected the ask.
 * - `TOOL_APPROVAL_CANCELLED` — the ask was withdrawn (aborted) before a decision.
 * - `TOOL_APPROVAL_UNAVAILABLE` — no approval service, no agent to route
 *   through, or the answerer chain resolved fail-closed.
 */
export const TOOL_APPROVAL_ERROR_CODES = {
  blocked: 'TOOL_APPROVAL_BLOCKED',
  rejected: 'TOOL_APPROVAL_REJECTED',
  cancelled: 'TOOL_APPROVAL_CANCELLED',
  unavailable: 'TOOL_APPROVAL_UNAVAILABLE',
} as const

/** One stable tool-approval failure code (see {@link TOOL_APPROVAL_ERROR_CODES}). */
export type ToolApprovalErrorCode = typeof TOOL_APPROVAL_ERROR_CODES[keyof typeof TOOL_APPROVAL_ERROR_CODES]
...
export class ToolRuntime extends Service {
  ...
  private sdkSection(): { name: string; order: number; text: (context: AssembleContext) => string } {
    return {
      ...
      // applying per-agent projections when the assembly carries the agent.
      text: (context) => {
        ...
        return render(this.sdkSchemas(context.agent, context.scope))
      },
    }
  }
   ...
   * deployment default.
   * @param scope - the pure viewing scope key, or undefined for the global view.
   ...
   * When the assembly context carries a REAL agent, each visible tool's
   * optional `projectSchema` projection is applied per agent; a bare
   * diagnostic assembly (no agent) falls back to the static schema.
   * Restrictions do not make known tools invalid, but a mode collapse does.
   * @param context - the prompt assembly context (its `agent`, when present,
   *   drives per-agent schema projection; `scope` selects the visible view).
   */
  private wireSchemas(context: AssembleContext): ToolProviderResult {
    const scope = context.scope
    const view = this.view(scope)
    const mode = this.modeFor(scope)
    const project = (definition: ToolDefinition): ToolSchema =>
      context.agent === undefined ? this.schemaOf(definition, false) : this.projectSchemaFor(definition, context.agent)
    if (mode === 'native') {
      const schemas = [...view.visible.values()].map(project)
   ...
   * @param scope - the pure viewing scope key; omitted = the global view.
   ...
   * excluding execution and presentation callbacks. This is the STATIC
   * projection: it never invokes per-agent `projectSchema`, so it suits
   * diagnostics and tool-catalog consumers. Model-bound surfaces (system
   * prompt assembly, `schemasFor`) use the per-agent projection instead.
   * @param scope - the pure viewing scope key; omitted = the global view.
   * @returns one deep-cloned static schema per visible tool.
   */
  ...
  /**
   * Project the schemas the given live agent sees, applying each visible
   * tool's optional `projectSchema` per agent. The agent is the real calling
   * agent (its session policy/mode drive the projection); `schemas(scope)`
   * remains the static/diagnostic projection.
   * @param agent - the exact live agent the schemas are assembled for.
   * @returns one per-agent projected schema per visible tool.
   */
  schemasFor(agent: Agent): ToolSchema[] {
    return [...this.view(agent).visible.values()].map(definition => this.projectSchemaFor(definition, agent))
  }

  /** Project visible callable tools onto the generated Code Mode SDK contract, per agent when one is given. */
  private sdkSchemas(agent: Agent | undefined, scope?: ScopeKey): ToolSdkSchema[] {
    return [...this.view(scope).visible.values()]
      ...
      .map((definition): ToolSdkSchema => {
        ...
        return {
          ...(agent === undefined ? this.schemaOf(definition, true) : this.projectSchemaFor(definition, agent)),
          output,
        }
      })
  }
  ...
  /**
   * Project one definition per agent: invoke its optional `projectSchema`
   * with the real agent and snapshot the replacement fields; a tool without a
   * projection (or one returning `undefined`) keeps the static schema. The
   * tool name is registry-owned and never replaced by a projection.
   * @param definition - the visible tool definition.
   * @param agent - the exact live agent the projection is for.
   * @returns the per-agent projected model-facing schema.
   */
  private projectSchemaFor(definition: ToolDefinition, agent: Agent): ToolSchema {
    const projected = definition.projectSchema?.({ agent })
    if (projected === undefined) return this.schemaOf(definition, true)
    // The raw definition's projection already carries the COMPILED
    // object-rooted JSON Schema (defineTool compiles the author-style map);
    // the registry only snapshots it, mirroring schemaOf's trust of the
    // compiled static `parameters` field. The snapshot's literal type is
    // exactly ToolSchema, so no assertion is needed.
    const detached = snapshotJsonValue({ name: definition.name, description: projected.description, parameters: projected.parameters })
    if (detached === undefined) {
      throw new Error(`tool "${definition.name}" projectSchema must return lossless JSON`)
    }
    return detached
  }
  ...
  private async prepareExecution<T>(
    ...
    try {
      ...
      if (denialReason !== undefined) {
        // A pre-execute listener's own deny or a guard denial carries the
        // default TOOL_POLICY_DENIED code; an approval-routed deny keeps its
        // more specific TOOL_APPROVAL_* code from serviceAsk.
        const code = decision.kind === 'deny'
          ? decision.code ?? TOOL_POLICY_DENIED
          : TOOL_POLICY_DENIED
        return await next({
          kind: 'post-result',
          exec,
   ...
   * one-to-one — `allowed-once` proceeds; the four non-grants deny with
   * distinct reasons and stable {@link TOOL_APPROVAL_ERROR_CODES} codes so the
   * model can tell a human "no" from a policy block or an absent approval
   * channel, and retry/sandbox/replay can route on `error.info.code`.
   */
  private async serviceAsk(
    ...
    if (approval === undefined) {
      return {
        decision: {
          ...
          code: TOOL_APPROVAL_ERROR_CODES.unavailable,
        },
    ...
    switch (outcome) {
      ...
      case 'rejected': return {
        decision: { kind: 'deny', reason: `the user rejected tool "${exec.name}"`, code: TOOL_APPROVAL_ERROR_CODES.rejected },
        approvalCancelled: false,
      }
      case 'blocked': return {
        decision: { kind: 'deny', reason: `tool "${exec.name}" is blocked by the current approval policy`, code: TOOL_APPROVAL_ERROR_CODES.blocked },
        approvalCancelled: false,
      }
      case 'cancelled': return {
        decision: { kind: 'deny', reason: `approval for tool "${exec.name}" was cancelled`, code: TOOL_APPROVAL_ERROR_CODES.cancelled },
        approvalCancelled: true,
      }
      case 'unavailable': return {
        decision: { kind: 'deny', reason: `tool "${exec.name}" requires approval, but no approval channel is available`, code: TOOL_APPROVAL_ERROR_CODES.unavailable },
        approvalCancelled: false,
      }
  ...
  private async postExecute(exec: ToolExecution, result: ToolExecutionResult): Promise<ToolExecutionResult> {
    ...
    if (decision.kind === 'block') {
      ...
      return this.markCanonical(exec, {
        ...
        isError: true,
        error: { message, info: { name: 'HarnessError', code: TOOL_POST_BLOCKED } },
  ...
  private createSuccessResult(exec: ToolExecution, tool: ToolDefinition, candidate: unknown): ToolExecutionSuccess {
    ...
    try {
      rendered = tool.output.render(exec.arguments, value, exec.agent === undefined ? {} : { agent: exec.agent })
    } catch (error: unknown) {
