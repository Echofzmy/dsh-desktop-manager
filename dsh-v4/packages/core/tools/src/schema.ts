...
import type { ToolDefinition, ToolExecution, ToolExecutionResult, ToolRenderContext, ToolResult, ToolRunContext, ProjectedToolSchema, ToolSchemaProjectionContext } from './index.ts'
...
/**
 * Author-facing per-agent projection: replacement model-facing description
 * plus the AUTHOR-STYLE parameter property map (the same DSL vocabulary the
 * static `parameters` field uses). `defineTool` compiles the map before the
 * raw {@link ToolDefinition.projectSchema} sees it.
 */
export interface ProjectedToolSchemaSpec {
  /** Replacement model-facing description. */
  description: string
  /** Replacement author-style parameter property map, compiled by `defineTool`. */
...
export interface DefineToolOptions<S extends ParameterSchemaSpec, O extends ValueSchemaSpec> {
  ...
  readonly output: {
    ...
    readonly schema: O
    /** Pure Native/model rendering of one validated canonical value; the optional context carries the calling agent. */
    render(args: InferArgs<S>, value: InferValue<NoInfer<O>>, context?: ToolRenderContext): ContentBlock[]
  ...
  /**
   * Optional per-agent schema projection at the AUTHORING level: returns
   * replacement model-facing description plus the AUTHOR-STYLE parameter
   * property map for the exact live agent, or `undefined` to keep the static
   * schema. `defineTool` compiles the map into the object-rooted JSON Schema
   * the model receives, so the raw {@link ToolDefinition.projectSchema}
   * always exposes the compiled form (mirroring how the static `parameters`
   * field is compiled at defineTool time).
   * @param context - the projection context carrying the real calling agent.
   * @returns the projected description + DSL parameter map, or `undefined` for the static schema.
   */
  projectSchema?(context: ToolSchemaProjectionContext): ProjectedToolSchemaSpec | undefined
  /**
...
export function defineTool<const S extends ParameterSchemaSpec, const O extends ValueSchemaSpec>(
  ...
  // oxlint-disable-next-line typescript/unbound-method
  const userProjectSchema = options.projectSchema
  ...
  const tool: ToolDefinition = {
    ...
    output: {
      ...
      render(args: unknown, value: JsonValue, context?: ToolRenderContext): ContentBlock[] {
        return userRender(args as InferArgs<S>, value as unknown as InferValue<NoInfer<O>>, context)
      },
    ...
    ...userProjectSchema !== undefined ? {
      // Compile the author-style DSL map into the object-rooted JSON Schema
      // at the ToolDefinition boundary, mirroring how the static `parameters`
      // field is compiled: the raw definition's projectSchema always exposes
      // the final compiled form, never the DSL map.
      projectSchema: (context: ToolSchemaProjectionContext): ProjectedToolSchema | undefined => {
        const projected = userProjectSchema(context)
        if (projected === undefined) return undefined
        return {
          description: projected.description,
          // Same compilation+cast the static `parameters` field receives.
