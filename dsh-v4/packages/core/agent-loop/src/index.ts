...
class FactoryOwnership {
  ...
  private readonly agents = new Map<Agent, () => Promise<void>>()
  ...
  /** Index one published-capable exact Agent by its structural teardown. */
  index(agent: Agent, dispose: () => Promise<void>): () => void {
    this.agents.set(agent, dispose)
    return () => {
      if (this.agents.get(agent) === dispose) this.agents.delete(agent)
    }
  }

  /** Retire only an exact idle Agent still owned by this factory. */
  retireIdle(agent: Agent): Promise<boolean> {
    const dispose = this.agents.get(agent)
    if (dispose === undefined || agent.status !== 'idle') return Promise.resolve(false)
...
export class AgentLoop extends Service implements AgentFactory {
  ...
  private prepare(ownerCtx: Context, id: SessionId, options: AgentOptions, session: Session, callerSignal?: AbortSignal): PreparedAgent {
    ...
    try {
      ...
      unindex = this.ownership.index(agent, dispose)
  ...
  /** Retire one exact idle Agent through this factory's structural ownership. */
  retireIdleAgent(agent: Agent): Promise<boolean> {
    return this.ownership.retireIdle(agent)
  }
