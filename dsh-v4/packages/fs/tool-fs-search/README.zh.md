...
| `globExcludeNodeModules` | `true` | `glob` 发现列表是否排除 `node_modules`——安全/规模默认，因为 `--no-ignore` 会让任何宽泛的 `*` 搜索暴露所有 vendored 文件。需要检查依赖源码的部署将其设为 `false`；无论取值如何，VCS 元数据名称始终被排除。 |
...
| 工具 | 参数 | 行为 |
|---|---|---|
| `glob` | `pattern`、`path?` | 运行 `rg --files --glob <pattern> --sort=modified --no-ignore --hidden`，并排除 VCS 元数据（`.git`、`.svn`、`.hg`、`.bzr`、`.jj`、`.sl`）；当 `globExcludeNodeModules` 为 `true`（默认）时还排除 `node_modules`（安全/规模默认；需要检查依赖源码的部署将其设为 `false`）。`path` 是可选的**目录**搜索根；省略时使用解析后的工作目录。每行返回一个**文件**路径；`rg --files` 从不输出目录条目。pattern 保留 ripgrep 语义：不含 `/` 时匹配任意深度的基名，因此 `*` 匹配整棵树。完整结果保持按修改时间排序；超过上限时的呈现方式遵循 `sampleOverCapGlobResults`。当 ripgrep 因部分路径不可读（macOS TCC 或权限拒绝，且所有诊断均为权限错误）在已输出结果后以退出码 2 结束时，调用返回可读结果并附显式的 `[warning: partial result …]` 行，而不是失败。 |
...
`glob` 每行返回一个路径；`grep` 在每个路径下分组展示 `Line <line>: <preview>` 匹配。空搜索返回 `No files found` 或 `No matches found`。达到上限的结果以省略计数结尾，并附 spill locator 与后端检索提示，或说明完整结果无法保存。部分结果（部分路径不可读）会在页面后追加 `[warning: partial result — some paths were not readable: <excerpt>]` 行，spill 产物也携带相同的部分依据，因此模型绝不会把不可读路径误当作不存在。启用 `sampleOverCapGlobResults: true` 时，超过上限的 `glob` 页面按实际搜索根正下方的条目轮转取路径，页脚说明采样依据及其覆盖的顶层条目数；无法覆盖全部条目时，页脚提示模型收窄 `path`。`false` 时页面是按修改时间排序的前部，并保留普通的上限结果页脚。未超过上限的结果原样呈现；扁平采样的结果也保留普通页脚，因为其采样等于按修改时间排序的前部。spill 产物始终持有按修改时间排序的完整列表。
