...
describe('registration', () => {
  ...
  it('advertises the node_modules exclusion only when globExcludeNodeModules is true', async () => {
    // The default composition claims the exclusion…
    const on = await setup()
    const onSchema = on.ctx.tools.schemas().find(schema => schema.name === 'glob')!
    expect(onSchema.description).toContain('node_modules directories are excluded')
    expect(renderPrompt(await on.ctx.systemPrompt.assemble())).toContain('node_modules directories are excluded by default')

    // …while a deployment that must inspect dependency sources (false) never
    // claims an exclusion it does not apply: schema, guidance, and argv agree.
    const off = await setup({ config: { globExcludeNodeModules: false } })
    const offSchema = off.ctx.tools.schemas().find(schema => schema.name === 'glob')!
    expect(offSchema.description).toContain('node_modules is searched')
    expect(offSchema.description).not.toContain('node_modules directories are excluded')
    const offPrompt = renderPrompt(await off.ctx.systemPrompt.assemble())
    expect(offPrompt).toContain('node_modules is searched')
    expect(offPrompt).not.toContain('node_modules directories are excluded')
    off.subprocess.handler = () => runResult('a.ts\n')
    const result = await call(off.ctx, 'glob', { pattern: '*' })
    expect(result.isError).toBe(false)
    expect(off.subprocess.spawns[0]?.argv).not.toContain('--glob=!**/node_modules')
  })
...
describe('command construction (plain argv)', () => {
  it('glob: fixed rg --files argv with the pattern and paired default excludes', () => {
  ...
  it('glob: excludeNodeModules false keeps only the always-excluded VCS names', () => {
    // A deployment that must inspect dependency sources turns the node_modules
    // arm off; the VCS metadata names stay excluded regardless.
    expect(buildGlobCommand({ pattern: '**/*.ts' }, false)).toEqual([
      '--files',
      '--glob=**/*.ts',
      '--sort=modified',
      '--no-ignore',
    ...
    expect(buildGlobCommand({ pattern: '*', path: 'sub' }, false)).not.toContain('--glob=!**/node_modules')
  })
...
describe('raw output acquisition', () => {
  it('fails with SEARCH_RAW_OUTPUT_OVERFLOW when truncated stdout has a raw spill path', async () => {
    ...
    // glob has no `include` parameter — the recovery copy must not name one.
    expect(text(result)).toContain('narrow pattern or path')
    expect(text(result)).not.toContain('include')
  })
...
describe('permission-only exit 2 is a PARTIAL success (macOS TCC / unreadable paths)', () => {
  /** One scripted exit-2 run: stdout already holds results, stderr carries the permission diagnostics. */
  function permissionRun(stdout: string, stderrText: string, overrides: Partial<SubprocessOutcome> & { stderr?: ScriptedStream } = {}) {
    const { stderr: stderrOverrides, ...outcome } = overrides
    return runResult(stdout, {
      exitCode: 2,
      stderr: { text: stderrText, ...stderrOverrides },
      ...outcome,
    })
  }

  it('glob: returns the readable paths with an explicit partial warning', async () => {
    const { ctx, subprocess } = await setup()
    subprocess.handler = () => permissionRun('a.ts\nb.ts\n', 'rg: /locked: Operation not permitted (os error 1)\n')
    const result = await call(ctx, 'glob', { pattern: '*' })
    expect(result.isError).toBe(false)
    if (result.isError) throw new Error('expected a partial glob success')
    expect(result.value).toMatchObject({ root: '.', paths: ['a.ts', 'b.ts'] })
    expect(result.value).toMatchObject({ warnings: ['rg: /locked: Operation not permitted (os error 1)'] })
    const output = text(result)
    expect(output).toContain('a.ts')
    expect(output).toContain('[warning: partial result — some paths were not readable: rg: /locked: Operation not permitted (os error 1)]')
  })

  it('grep: returns the readable matches with an explicit partial warning', async () => {
    const { ctx, subprocess } = await setup()
    subprocess.handler = () => permissionRun(
      `${matchLine('a.ts', 1, 'one')}\n`,
      'rg: /locked: Permission denied (os error 13)\n',
    )
    const result = await call(ctx, 'grep', { pattern: 'one' })
    expect(result.isError).toBe(false)
    if (result.isError) throw new Error('expected a partial grep success')
    expect(result.value).toMatchObject({ matches: [{ path: 'a.ts', lineNumber: 1, line: 'one' }] })
    expect(result.value).toMatchObject({ warnings: ['rg: /locked: Permission denied (os error 13)'] })
    expect(text(result)).toContain('[warning: partial result — some paths were not readable: rg: /locked: Permission denied (os error 13)]')
  })

  it('accepts any permission-denial wording and blank stderr lines', async () => {
    const { ctx, subprocess } = await setup()
    subprocess.handler = () => permissionRun('a.ts\n', '\nrg: /x: Access denied (os error 5)\n\n')
    const result = await call(ctx, 'glob', { pattern: '*' })
    expect(result.isError).toBe(false)
    expect(text(result)).toContain('Access denied')
  })

  it('keeps SEARCH_FAILED for a non-exit-2 nonzero exit with results on stdout', async () => {
    const { ctx, subprocess } = await setup()
    subprocess.handler = () => permissionRun('a.ts\n', 'rg: /x: Operation not permitted (os error 1)\n', { exitCode: 3 })
    const result = await call(ctx, 'glob', { pattern: '*' })
    expect(result.error).toMatchObject({ info: { code: 'SEARCH_FAILED' } })
  })

  it('keeps SEARCH_FAILED when stderr mixes a non-permission diagnostic', async () => {
    const { ctx, subprocess } = await setup()
    subprocess.handler = () => permissionRun('a.ts\n', 'rg: /x: Operation not permitted (os error 1)\nrg: /y: IO error: no such file\n')
    const result = await call(ctx, 'glob', { pattern: '*' })
    expect(result.error).toMatchObject({ info: { code: 'SEARCH_FAILED' } })
  })

  it('keeps SEARCH_FAILED when the stderr tail is truncated (lossy)', async () => {
    const { ctx, subprocess } = await setup()
    subprocess.handler = () => permissionRun('a.ts\n', 'rg: /x: Operation not permitted (os error 1)\n', { stderr: { text: 'tail', lossy: true } })
    const result = await call(ctx, 'glob', { pattern: '*' })
    expect(result.error).toMatchObject({ info: { code: 'SEARCH_FAILED' } })
  })

  it('keeps SEARCH_FAILED when stdout is empty despite the permission diagnostics', async () => {
    const { ctx, subprocess } = await setup()
    subprocess.handler = () => permissionRun('', 'rg: /locked: Operation not permitted (os error 1)\n')
    const result = await call(ctx, 'glob', { pattern: '*' })
    expect(result.error).toMatchObject({ info: { code: 'SEARCH_FAILED' } })
  })

  it('a regex parse error stays SEARCH_INVALID_PATTERN (never a partial success)', async () => {
    const { ctx, subprocess } = await setup()
    subprocess.handler = () => runResult('', { exitCode: 2, stderr: { text: 'rg: regex parse error:\n    (\nerror: unclosed group' } })
    const result = await call(ctx, 'grep', { pattern: '(' })
    expect(result.error).toMatchObject({ info: { code: 'SEARCH_INVALID_PATTERN' } })
