...
export function applyEditTool(ctx: Context, sandbox: FsSandboxController): void {
  ...
  ctx.tools.register(defineTool({
    ...
    // escalation fields — there is nothing to escalate TO. The static
    // `parameters` above stays the composition-wide advertisement for
    // diagnostics and `schemas(scope)`.
    projectSchema: ({ agent }) => ({
      description: 'Edit an existing UTF-8 text file by replacing literal text.',
      parameters: {
        file_path: { type: 'string', required: true, description: 'Path to edit, resolved by the filesystem backend.' },
        ...
        new_string: { type: 'string', required: true, description: 'Literal replacement text. Use an empty string to delete the match.' },
        replace_all: { type: 'boolean', description: 'Replace all matches. Defaults to false; when false, old_string must appear exactly once.' },
        ...sandbox.advertiseEscalationFor(agent) ? sandbox.schemaFields() : {},
      },
    }),
    output: {
      schema: {
    ...
    async execute(args: EditToolArgs, exec) {
      ...
      try {
        ...
        // recognizes it from bash); the escalation hint rides it only when
        // this agent's surface advertises escalation. Stale/not-observed
        // failures gain their model-facing remedy; anything else passes through.
        throw remediateFsError(sandbox.mapError(error, sandboxPolicy, sandbox.advertiseEscalationFor(exec.agent)))
      }
