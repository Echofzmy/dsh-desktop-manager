...
async function harness(
  ...
  const persistedSessions = new Map<SessionId, Session['header']>()
  ctx.provide('sessionPersistence', {
    list: () => Promise.resolve([...persistedSessions.values()]),
    locate: () => undefined,
    delete: (id: SessionId) => Promise.resolve(persistedSessions.delete(id)),
  } as never)
  await ctx.plugin(WorkspaceRegistry)

  const agentDisposers = new WeakMap<Agent, () => void>()
  const factory: AgentFactory = {
    async createAgent(_ownerCtx, options) {
      ...
      persistedSessions.set(session.id, session.header)
      ...
      agentDisposers.set(agent, unregister)
      return {
        agent,
        dispose: () => {
          unregister()
    ...
    async retireIdleAgent(agent) {
      const dispose = agentDisposers.get(agent)
      if (dispose === undefined || agent.status !== 'idle') return false
      dispose()
      agentDisposers.delete(agent)
      return true
    },
  ...
  const api = createApiProxy(ctx, {
    ...
    ...extras.workspaceBrowseMaxEntries === undefined
      ? {}
      : { workspaceBrowseMaxEntries: extras.workspaceBrowseMaxEntries },
  })
...
describe('Host Workspace increments', () => {
  ...
  it('restores an archived session and permanently deletes only from the archive set', async () => {
    const { api, root } = await harness()
    const workspace = expectOk(await api.workspace.create(request({ path: stageDir(root, 'archive-actions') }))).workspace
    const sessionId = SessionId('session-archive-actions')
    expectOk(await api.sessions.create(request({ workspaceId: workspace.workspaceId, sessionId })))
    ...
    expectOk(await api.workspace.archiveSession(request({ sessionId })))
    const deleted = expectOk(await api.workspace.deleteArchivedSession(request({ sessionId })))
    ...
    const repeated = await api.workspace.deleteArchivedSession(request({ sessionId }))
  ...
  it('retires an externally owned idle root before permanently deleting its archive', async () => {
    const { api, ctx, root } = await harness()
    const workspace = expectOk(await api.workspace.create(request({ path: stageDir(root, 'archive-external-owner') }))).workspace
    const sessionId = SessionId('session-archive-external-owner')
    expectOk(await api.sessions.create(request({ workspaceId: workspace.workspaceId, sessionId })))
    expectOk(await api.workspace.archiveSession(request({ sessionId })))
    ...
    expect(expectOk(await api.workspace.deleteArchivedSession(request({ sessionId }))))
...
describe('workspace.browse', () => {
  it('lists one direct level: directories first, then files, each name-sorted, with dot-prefixed hidden flags', async () => {
    const { api, root } = await harness()
    const workspace = expectOk(await api.workspace.create(request({ path: stageDir(root, 'browse-root') }))).workspace
    const project = workspace.path
    mkdirSync(join(project, 'z-dir'))
    mkdirSync(join(project, '.secret-dir'))
    mkdirSync(join(project, 'a-dir'))
    writeFileSync(join(project, 'z.txt'), '')
    writeFileSync(join(project, '.hidden'), '')
    writeFileSync(join(project, 'a.txt'), '')

    const listing = expectOk(await api.workspace.browse(
    ...
    expect(listing).toEqual({
      relativePath: '',
      entries: [
        { name: '.secret-dir', relativePath: '.secret-dir', path: join(project, '.secret-dir'), kind: 'directory', hidden: true },
        { name: 'a-dir', relativePath: 'a-dir', path: join(project, 'a-dir'), kind: 'directory', hidden: false },
        { name: 'z-dir', relativePath: 'z-dir', path: join(project, 'z-dir'), kind: 'directory', hidden: false },
        { name: '.hidden', relativePath: '.hidden', path: join(project, '.hidden'), kind: 'file', hidden: true },
        { name: 'a.txt', relativePath: 'a.txt', path: join(project, 'a.txt'), kind: 'file', hidden: false },
        { name: 'z.txt', relativePath: 'z.txt', path: join(project, 'z.txt'), kind: 'file', hidden: false },
      ],
      truncated: false,
    })
  })

  it('skips symlink and unusual node-type children while listing real ones', async () => {
    const { api, root } = await harness()
    const workspace = expectOk(await api.workspace.create(request({ path: stageDir(root, 'node-types') }))).workspace
    writeFileSync(join(workspace.path, 'real.txt'), '')
    symlinkSync(join(workspace.path, 'real.txt'), join(workspace.path, 'linked.txt'))
    const server = createServer()
    await new Promise<void>((resolve, reject) => {
      server.once('error', reject)
      // A listening unix socket is a socket node type the backend must skip.
      server.listen(join(workspace.path, 'sock'), resolve)
    })
    try {
      const listing = expectOk(await api.workspace.browse(
        request({ workspaceId: workspace.workspaceId }),
  ...
  it('browses a nested level and echoes its workspace-rooted relative path', async () => {
    const { api, root } = await harness()
    const workspace = expectOk(await api.workspace.create(request({ path: stageDir(root, 'nested') }))).workspace
    mkdirSync(join(workspace.path, 'notes'))
    mkdirSync(join(workspace.path, 'notes', 'drafts'))
    writeFileSync(join(workspace.path, 'notes', 'todo.md'), '')

    const listing = expectOk(await api.workspace.browse(
      request({ workspaceId: workspace.workspaceId, relativePath: 'notes' }),
      new AbortController().signal,
    ))
    expect(listing).toEqual({
      relativePath: 'notes',
      entries: [
        { name: 'drafts', relativePath: 'notes/drafts', path: join(workspace.path, 'notes', 'drafts'), kind: 'directory', hidden: false },
        { name: 'todo.md', relativePath: 'notes/todo.md', path: join(workspace.path, 'notes', 'todo.md'), kind: 'file', hidden: false },
      ],
      truncated: false,
    })
  })

  it('rejects traversal, absolute, Windows/UNC, NUL, and empty/`.`/`..` segments as directory-unreadable', async () => {
    const { api, root } = await harness()
    const workspace = expectOk(await api.workspace.create(request({ path: stageDir(root, 'guarded') }))).workspace
    const invalid = [
      '..', '../x', 'a/../b', '.', './a', 'a/.', 'a/./b',
      '/abs', '/abs/x', 'C:/x', 'C:\\x', '\\\\server\\share',
      'a//b', 'a/', 'a\0b',
    ]
    for (const relativePath of invalid) {
      const response = await api.workspace.browse(
        request({ workspaceId: workspace.workspaceId, relativePath }),
        new AbortController().signal,
      )
      expect(response.result).toMatchObject({
        ok: false,
        error: {
          ...
          details: { path: join(workspace.path, relativePath) },
        },
      })
    }
  })

  it('refuses a symlink whose canonical target escapes the workspace and follows one that stays inside', async () => {
    const { api, root } = await harness()
    const workspace = expectOk(await api.workspace.create(request({ path: stageDir(root, 'links') }))).workspace
    const outside = stageDir(root, 'outside-workspace')
    mkdirSync(join(outside, 'sub'))
    symlinkSync(outside, join(workspace.path, 'escape'), 'dir')
    mkdirSync(join(workspace.path, 'real-dir'))
    writeFileSync(join(workspace.path, 'real-dir', 'inside.txt'), '')
    symlinkSync(join(workspace.path, 'real-dir'), join(workspace.path, 'alias'), 'dir')

    for (const relativePath of ['escape', 'escape/sub']) {
      const response = await api.workspace.browse(
        request({ workspaceId: workspace.workspaceId, relativePath }),
        new AbortController().signal,
      )
      expect(response.result).toMatchObject({
        ok: false,
        error: { code: 'directory-unreadable', details: { path: join(workspace.path, relativePath) } },
      })
    }

    const followed = expectOk(await api.workspace.browse(
      request({ workspaceId: workspace.workspaceId, relativePath: 'alias' }),
      new AbortController().signal,
    ))
    expect(followed.entries.map(entry => entry.name)).toEqual(['inside.txt'])
  })

  it('answers workspace-not-found for an unknown id and directory-unreadable for a deleted root', async () => {
    const { api, root } = await harness()
    const unknown = await api.workspace.browse(
      request({ workspaceId: 'missing-workspace' as WorkspaceId }),
      new AbortController().signal,
    )
    ...
    const workspace = expectOk(await api.workspace.create(request({ path: stageDir(root, 'doomed') }))).workspace
    rmSync(workspace.path, { recursive: true, force: true })
    const gone = await api.workspace.browse(
      request({ workspaceId: workspace.workspaceId }),
      new AbortController().signal,
    )
    expect(gone.result).toMatchObject({
      ok: false,
      error: { code: 'directory-unreadable', details: { path: workspace.path } },
    })
  })

  it('maps a missing or non-directory target to directory-unreadable', async () => {
    const { api, root } = await harness()
    const workspace = expectOk(await api.workspace.create(request({ path: stageDir(root, 'targets') }))).workspace
    writeFileSync(join(workspace.path, 'plain.txt'), '')

    const missing = await api.workspace.browse(
      request({ workspaceId: workspace.workspaceId, relativePath: 'absent' }),
      new AbortController().signal,
    )
    expect(missing.result).toMatchObject({
      ok: false,
      error: { code: 'directory-unreadable', details: { path: join(workspace.path, 'absent') } },
    })

    const file = await api.workspace.browse(
      request({ workspaceId: workspace.workspaceId, relativePath: 'plain.txt' }),
      new AbortController().signal,
    )
    expect(file.result).toMatchObject({
      ok: false,
      error: { code: 'directory-unreadable', details: { path: join(workspace.path, 'plain.txt') } },
    })
  })

  it('cuts the combined listing at the configured bound with truthful truncated', async () => {
    const { api, root } = await harness(undefined, undefined, { workspaceBrowseMaxEntries: 3 })
    const exact = expectOk(await api.workspace.create(request({ path: stageDir(root, 'exact') }))).workspace
    for (const name of ['a.txt', 'b.txt', 'c.txt']) writeFileSync(join(exact.path, name), '')
    const exactListing = expectOk(await api.workspace.browse(
      request({ workspaceId: exact.workspaceId }),
      new AbortController().signal,
    ))
    expect(exactListing.entries.map(entry => entry.name)).toEqual(['a.txt', 'b.txt', 'c.txt'])
    expect(exactListing.truncated).toBe(false)

    const overflow = expectOk(await api.workspace.create(request({ path: stageDir(root, 'overflow') }))).workspace
    mkdirSync(join(overflow.path, 'z-dir'))
    mkdirSync(join(overflow.path, 'a-dir'))
    for (const name of ['d.txt', 'c.txt', 'b.txt', 'a.txt']) writeFileSync(join(overflow.path, name), '')
    const overflowListing = expectOk(await api.workspace.browse(
      request({ workspaceId: overflow.workspaceId }),
      new AbortController().signal,
    ))
    // Directories sort first and consume the whole bound; the file tail is cut.
    expect(overflowListing.entries.map(entry => entry.name)).toEqual(['a-dir', 'z-dir', 'a.txt'])
    expect(overflowListing.truncated).toBe(true)
  })

  it('answers cancelled for an already-aborted signal and for an abort in flight', async () => {
    const { api, root } = await harness()
    const workspace = expectOk(await api.workspace.create(request({ path: stageDir(root, 'abortable') }))).workspace
    for (let i = 0; i < 300; i++) writeFileSync(join(workspace.path, `file-${i}.txt`), '')

    const already = new AbortController()
    already.abort()
    const preAborted = await api.workspace.browse(
      request({ workspaceId: workspace.workspaceId }),
      already.signal,
    )
    expect(preAborted.result).toMatchObject({ ok: false, error: { code: 'cancelled' } })

    const inFlight = new AbortController()
    const pending = api.workspace.browse(
      request({ workspaceId: workspace.workspaceId }),
      inFlight.signal,
    )
    inFlight.abort()
