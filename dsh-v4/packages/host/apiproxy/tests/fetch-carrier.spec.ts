  ...
  return {
    ...
    workspace: {
      ...
      async browse(request) {
        if (request.payload.relativePath === 'missing') {
          return {
            rpcId: request.rpcId,
            result: { ok: false, error: { code: 'directory-unreadable', message: 'cannot list', details: { path: '/w/missing' } } },
          }
        }
        return {
          rpcId: request.rpcId,
          result: {
            ...
            value: {
              ...
              entries: [
                { name: 'docs', relativePath: 'docs', path: '/w/docs', kind: 'directory' as const, hidden: false },
                { name: 'README.md', relativePath: 'README.md', path: '/w/README.md', kind: 'file' as const, hidden: false },
                { name: '.config', relativePath: '.config', path: '/w/.config', kind: 'directory' as const, hidden: true },
              ],
              truncated: false,
            },
          },
...
describe('unary round trip (handler ⇄ client, no network)', () => {
  ...
  it('round-trips workspace.browse, its root default, and a browse failure through the wire form', async () => {
    const c = client()
    const listed = await c.workspace.browse({ workspaceId: 'w1' as never, relativePath: 'docs' })
    expect(listed.result).toEqual({
      ok: true,
      value: {
        relativePath: 'docs',
        entries: [
          { name: 'docs', relativePath: 'docs', path: '/w/docs', kind: 'directory', hidden: false },
          { name: 'README.md', relativePath: 'README.md', path: '/w/README.md', kind: 'file', hidden: false },
          { name: '.config', relativePath: '.config', path: '/w/.config', kind: 'directory', hidden: true },
        ],
        truncated: false,
      },
    })
    const root = await c.workspace.browse({ workspaceId: 'w1' as never })
    expect(root.result).toMatchObject({ ok: true, value: { relativePath: '' } })
    const missing = await c.workspace.browse({ workspaceId: 'w1' as never, relativePath: 'missing' })
    expect(missing.result.ok).toBe(false)
    if (!missing.result.ok) expect(missing.result.error.code).toBe('directory-unreadable')
  })

  it('round-trips archived-session restore and permanent deletion through the wire form', async () => {
    const c = client()
    const restored = await c.workspace.unarchiveSession({ sessionId: 's1' as never })
    expect(restored.result).toEqual({ ok: true, value: { archivedSessionIds: [] } })
    const deleted = await c.workspace.deleteArchivedSession({ sessionId: 's1' as never })
    expect(deleted.result).toEqual({ ok: true, value: { deleted: true, archivedSessionIds: [] } })
  })
