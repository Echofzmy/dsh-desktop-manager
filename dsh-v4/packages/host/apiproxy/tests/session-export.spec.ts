...
describe('session export compression config', () => {
  it('defaults to level 6 and rejects values outside the integer 0-9 range', () => {
    expect(ApiProxyService.Config({})).toEqual({
      ...
      workspaceBrowseMaxEntries: 1000,
    })
    expect(ApiProxyService.Config({ sessionExportCompressionLevel: 0 }))
      .toEqual({ sessionExportCompressionLevel: 0, coldBlankProbeMaxBytes: 1024, workspaceBrowseMaxEntries: 1000 })
    expect(ApiProxyService.Config({ sessionExportCompressionLevel: 9 }))
      .toEqual({ sessionExportCompressionLevel: 9, coldBlankProbeMaxBytes: 1024, workspaceBrowseMaxEntries: 1000 })
...
describe('cold blank probe config', () => {
  it('accepts a per-Session byte bound including zero and rejects invalid bounds', () => {
    expect(ApiProxyService.Config({ coldBlankProbeMaxBytes: 0 }))
      .toEqual({ sessionExportCompressionLevel: 6, coldBlankProbeMaxBytes: 0, workspaceBrowseMaxEntries: 1000 })
    expect(ApiProxyService.Config({ coldBlankProbeMaxBytes: 2048 }))
      .toEqual({ sessionExportCompressionLevel: 6, coldBlankProbeMaxBytes: 2048, workspaceBrowseMaxEntries: 1000 })
...
describe('workspace browse bound config', () => {
  it('defaults the combined listing bound to 1000 and rejects non-natural values', () => {
    expect(ApiProxyService.Config({ workspaceBrowseMaxEntries: 2000 })).toEqual({
      sessionExportCompressionLevel: 6,
      coldBlankProbeMaxBytes: 1024,
      workspaceBrowseMaxEntries: 2000,
    })
    for (const value of [-1, 1.5]) {
      expect(() => ApiProxyService.Config({ workspaceBrowseMaxEntries: value })).toThrow()
    }
  })
})
