...
function scriptedApi(overrides: {
  ...
  return {
    ...
    workspace: {
      ...
      browse: r => ok(r, { relativePath: r.payload.relativePath ?? '', entries: [], truncated: false }),
      ...
      deleteArchivedSession: r => ok(r, { deleted: true as const, archivedSessionIds: [] }),
    },
...
describe('workspace domain round trip', () => {
  ...
  it('routes workspace.browse through its handler row and value schema', async () => {
    const c = client(scriptedApi())
    const browsed = await c.workspace.browse({ workspaceId: 'w1' as never, relativePath: 'notes' })
    expect(browsed.result).toEqual({ ok: true, value: { relativePath: 'notes', entries: [], truncated: false } })
    const root = await c.workspace.browse({ workspaceId: 'w1' as never })
    expect(root.result).toEqual({ ok: true, value: { relativePath: '', entries: [], truncated: false } })
  })
  ...
  it('rejects a workspaceId-less browse payload at the handler schema', async () => {
    const response = await client(scriptedApi()).workspace.browse({} as never)
    expect(response.result.ok).toBe(false)
    if (!response.result.ok) expect(response.result.error.code).toBe('bad-request')
  })
})
