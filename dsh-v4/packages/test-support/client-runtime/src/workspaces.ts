...
import type {
  DirectoryListing, IWorkspaces, SessionId, SnapshotStore, WorkspaceDirectoryListing, WorkspaceId, WorkspaceListState, WorkspaceView,
...
export class TestWorkspaces implements IWorkspaces {
  ...
  /**
   * Workspace browse listing (recorded). The default serves a deterministic
   * workspace root with one file and one directory row, so feature tests see
   * both row kinds; stub to shape a tree. The signal is recorded and
   * forwarded like the production face passes it to the wire, so
   * cancellation integration tests can observe or reject on a superseded
   * scan.
   * @param workspaceId - owning workspace.
   * @param relativePath - workspace-rooted directory; absent lists the root.
   * @param signal - aborts the wire request when the caller supersedes it.
   * @returns the level's listing.
   */
  async browse(workspaceId: WorkspaceId, relativePath?: string, signal?: AbortSignal): Promise<WorkspaceDirectoryListing> {
    this.calls.push({ method: 'browse', args: [workspaceId, relativePath, signal] })
    const stub = this.stubs.get('browse')
    if (stub !== undefined) return await (stub(workspaceId, relativePath, signal) as Promise<WorkspaceDirectoryListing>)
    // One level of each kind, matching the wire contract's directories-first
    // order; deeper levels (or failures) are what feature tests stub.
    const root = relativePath ?? ''
    return {
      relativePath: root,
      entries: [
        {
          name: 'docs', relativePath: root === '' ? 'docs' : `${root}/docs`,
          path: `/ws/${workspaceId}/docs`, kind: 'directory', hidden: false,
        },
        {
          name: 'README.md', relativePath: root === '' ? 'README.md' : `${root}/README.md`,
          path: `/ws/${workspaceId}/README.md`, kind: 'file', hidden: false,
        },
      ],
      truncated: false,
    }
