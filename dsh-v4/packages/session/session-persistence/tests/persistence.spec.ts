...
describe('permanent deletion', () => {
  it('deletes a cold materialized session, invalidates prepared reads, and is idempotent', async () => {
    const ctx = new Context()
    await ctx.plugin(SessionStore)
    await ctx.plugin(MemoryPersistence)
  ...
  it('refuses deletion while the session identity is live', async () => {
    const ctx = new Context()
    await ctx.plugin(SessionStore)
    await ctx.plugin(MemoryPersistence)
    const session = ctx.sessions.create(SessionId('delete-live'))
