...
| `delete(id): Promise<boolean>` | Permanently remove one cold session's durable record. The coordinator serializes deletion with same-id work, waits retirement, invalidates prepared inspection state, and rejects while a live Session identity exists. Returns whether a stored record existed. |
| `delete(id): Promise<boolean>` | Permanently remove one cold session's durable record. The coordinator serializes deletion with same-id work, waits retirement, invalidates prepared inspection state, and rejects while a live Session identity exists. Returns whether a stored record existed. |
...
| `deleteStored(id)` | Atomically remove the durable record for one id and return whether it existed. |
