  ...
  return [
    ...
    } as unknown as JsonValue)),
    {
      ...
      query(method, _input, context) {
        ...
        // The model sees the per-agent projection of the CALLING agent's
        // surface (policy/mode-dependent schema advertisement), not the
        // static diagnostic projection.
        return Promise.resolve({ tools: ctx.tools.schemasFor(context.agent) } as unknown as JsonValue)
      },
    },
  ]
}
