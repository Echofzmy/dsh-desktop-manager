...
import { ESCALATION_TARGETS, ESCALATION_ERROR_CODES, approveEscalation, escalationError, escalationHintMarker, sandboxDenialMarker, validateEscalationArgs } from '@deepseek-ai/dsh-sandbox'
...
export class FsSandboxController {
  ...
  /**
   * Whether this agent's model surface advertises escalation for mutating
   * tools: false under a `'never'` approval policy (an approval can never
   * grant) or an effective mode of `danger-full-access` (nothing is strictly
   * wider), true otherwise. `undefined` (no live agent) keeps the
   * composition's static advertisement. The SAME predicate drives the
   * per-agent schema projection AND the denial hint, so the model is never
   * invited to escalate where escalation cannot be granted. Execution-time
   * enforcement is unchanged: schema omission is advertising, not a bypass
   * guard.
   * @param agent - the calling agent, or `undefined` for a static/diagnostic read.
   * @returns whether escalation fields and hints are advertised for that agent.
   */
  advertiseEscalationFor(agent: Agent | undefined): boolean {
    if (this.escalationModes.length === 0) return false
    if (agent === undefined) return true
    if (this.ctx.get('approval')?.effectivePolicy(agent.session) === 'never') return false
    return this.policy?.resolve({ session: agent.session }).mode !== 'danger-full-access'
  }
   ...
   * executes), else the session's standing mode. A requested mode that is NOT
   * strictly wider than the standing mode (equal or narrower — a strict-schema
   * redundant field) resolves to the standing mode with no approval and no
   * error. The calling session's cwd is always carried as the workspace root.
  ...
  async resolvePolicy(toolName: string, args: FsEscalationArgs, exec: ToolExecution): Promise<SandboxExecutionPolicy | undefined> {
    ...
    if (this.escalationModes.length === 0) {
      throw escalationError(ESCALATION_ERROR_CODES.unavailable, 'sandbox_permissions is not available in this composition (no sandboxing filesystem to escalate)')
    }
   ...
   * `FsError` whose text is the shared `[sandbox: …]` denial marker plus,
   * when escalation is advertised for the calling agent, the same-turn
   * escalation hint — so a policy denial reads identically to bash's WHILE
   * keeping the structured `FS_SANDBOX_DENIED` code (`ToolRuntime` populates
   * `result.error` only for `HarnessError` instances, so a plain `Error`
   * would strip the code retry/observers key off). Any other error passes
   * through unchanged. A `FS_SANDBOX_DENIED` only arises under a confining
   * backend, which resolves a policy before mutation; whether the hint rides
   * the marker follows the same per-agent predicate as the schema
   * advertisement (`advertiseEscalationFor`), so an agent that cannot be
   * granted escalation is never invited to request it.
   * @param error - the error thrown by the mutation.
   * @param policy - the policy stamped onto the call (names the mode in the marker).
   * @param advertise - whether this agent's surface advertises escalation (the hint rides only when true).
   * @returns the error to throw — the marker `FsError` for a sandbox denial, else the original.
   */
  mapError(error: unknown, policy: SandboxExecutionPolicy | undefined, advertise: boolean): unknown {
