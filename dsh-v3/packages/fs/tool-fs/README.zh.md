...
当 `ctx.fs.sandboxMode` 表明提供方施加沙箱限制时，write/edit 会公开 `sandbox_permissions` 与 `justification`，并通过 `ctx.approval` 处理获批后的重试。所请求的模式不严格宽于有效模式（相等或更窄）时，它属于严格 schema 的冗余字段：变更在有效模式下执行，不发起审批、也不报错。升权失败抛出带稳定错误码的结构化错误（`SANDBOX_ESCALATION_BLOCKED` 策略阻止、`SANDBOX_ESCALATION_REJECTED` 人类／应答者拒绝、`SANDBOX_ESCALATION_CANCELLED` 撤销、`SANDBOX_ESCALATION_UNAVAILABLE` 通道缺失）。Schema 广告按 agent 进行：有效审批策略为 `never`、或有效模式已是 `danger-full-access` 的 agent，其 schema 会省略升权字段，其拒绝结果也不附带升权提示——将会话模式切回 `danger-full-access` 之下（或策略切回 `ask`）即可同时恢复；执行期强制保持不变。策略归属方会贡献与具体能力无关的常驻策略；工具结果仍保留针对具体操作的拒绝与重试引导。
