...
export { GLOB_DEFAULT_EXCLUDES, GLOB_MAX_RESULTS, GLOB_VCS_EXCLUDES, applyGlobTool, buildGlobCommand, formatGlobOutput, parseGlobArgs, presentGlobCall, presentGlobResult, sampleAcrossTopLevel } from './glob.ts'
...
export interface Config {
  ...
  /**
   * Whether a `glob` discovery listing excludes `node_modules` (default true —
   * a safety/size default: `--no-ignore` would otherwise surface every vendored
   * file in any broad `*` search). Deployments that must inspect dependency
   * sources set it to false; the VCS metadata names are always excluded
   * regardless.
   */
  globExcludeNodeModules?: boolean
...
export const Config: z<Config> = z.object({
  ...
  globExcludeNodeModules: z.boolean().default(true),
...
export async function apply(ctx: Context, config: Config): Promise<void> {
  ...
  applyGlobTool(ctx, {
    ...
    excludeNodeModules: resolved.globExcludeNodeModules,
