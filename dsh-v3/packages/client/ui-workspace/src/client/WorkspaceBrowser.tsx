...
import {
  Button, IconBrowseOutline16, IconCloseFill14, IconFolderOpenOutline16, IconPersonalizationOutline16,
  IconProjectAddOutline16, IconRefreshOutline16, IconSearchOutline16, Menu, Modal, Tooltip,
...
import { resolveFilesWorkspace } from './files/tree.ts'
import { FilesBrowser } from './files/FilesBrowser.tsx'
import { ArchivedSessionNodeItem, ProjectRowItem, SearchResultItem, SessionNodeItem } from './rows/Rows.tsx'
...
/** Archived-session body: archive order with restore and destructive delete actions. */
function ArchivedList({
  useSessions, open, archivedSessionIds, onRestore, onDelete, t,
}: Pick<WorkspaceBrowserProps, 'useSessions' | 'open' | 't'> & {
  archivedSessionIds: readonly SessionId[]
  onRestore: (sessionId: SessionId) => void
  onDelete: (sessionId: SessionId, title: string) => void
}) {
  const list = useSessions(state => state)
  const rows = useMemo(() => deriveArchived(list, archivedSessionIds), [archivedSessionIds, list])
  const now = Date.now()
  return (
    <div className={clsx(css.treeBody, css.wide)}>
...
export function WorkspaceBrowser({
  ...
  // The Workspace the file tree browses: the current Session's Workspace, then
  // the recent Workspace, then the first registered Workspace; none with no
  // Workspaces at all.
  const filesWorkspace = useMemo(
    () => resolveFilesWorkspace(workspaces, currentSessionId, recentWorkspaceId),
    [currentSessionId, recentWorkspaceId, workspaces],
  )
  // Files chrome that outlives the tree's own cache: the hidden-entry reveal
  // and the refresh trigger (the cache clears and the root reloads on bump).
  ...
  const [archivedDeleteTarget, setArchivedDeleteTarget] = useState<{ sessionId: SessionId; title: string } | null>(null)
  ...
  const closeArchivedDelete = () => {
    if (archivedDeleting) return
    setArchivedDeleteTarget(null)
    setArchivedDeleteError(null)
  }
  const confirmArchivedDelete = () => {
    if (archivedDeleting || archivedDeleteTarget === null) return
    setArchivedDeleting(true)
    setArchivedDeleteError(null)
    deleteArchivedSession(archivedDeleteTarget.sessionId).then(() => {
      setArchivedDeleting(false)
      setArchivedDeleteTarget(null)
    }).catch((reason: unknown) => {
      setArchivedDeleting(false)
      setArchivedDeleteError(reason instanceof Error ? reason.message : String(reason))
    })
  }
  ...
  return (
    <div className={clsx(css.root, !wide && css.rail)}>
      <div className={css.sectionHeader}>
        {/* The region's one mode switch replaces the former section label:
            Sessions keeps the grouped/flat list + search, Files the current
            Workspace's tree. The switch folds with the search expansion so
            the field can take the whole header, like the label did. */}
        {wide && (
          <div
            className={clsx(css.modeSwitch, css.wide, searchExpanded && css.modeSwitchHidden)}
            role="group"
            aria-label={t('mode.aria')}
          >
            <button
              type="button"
              className={clsx(css.modeButton, mode === 'sessions' && css.modeButtonActive)}
              aria-pressed={mode === 'sessions'}
              onClick={() => { actions.setMode('sessions') }}
            >
              {t('mode.sessions')}
            </button>
            <button
              type="button"
              className={clsx(css.modeButton, mode === 'files' && css.modeButtonActive)}
              aria-pressed={mode === 'files'}
              onClick={() => {
                // Search is Sessions chrome: leaving the mode withdraws it.
                setSearchExpanded(false)
                actions.setMode('files')
              }}
            >
              {t('mode.files')}
            </button>
            <button
              type="button"
              className={clsx(css.modeButton, mode === 'archived' && css.modeButtonActive)}
              aria-pressed={mode === 'archived'}
              onClick={() => {
                setSearchExpanded(false)
                actions.setMode('archived')
              }}
            >
              {t('mode.archived')}
            </button>
        ...
        {wide && mode === 'files' && (
          <div className={css.filesHeader}>
            <span className={css.filesHeaderTitle}>
              {filesWorkspace === undefined ? '' : filesWorkspace.title}
            </span>
          </div>
        )}
        <div className={clsx(
          ...
          wide && mode === 'files' && css.filesHeaderActions,
        )}>
          {wide && mode === 'sessions' && (
            <ViewOptionsMenu
              groupBy={groupBy}
          ...
          {wide && mode === 'files' && filesWorkspace !== undefined && (
            <>
              <Tooltip label={t('files.refresh')} side="bottom" delayMs={500}>
                <button
                  type="button"
              ...
              <Tooltip label={t('files.showHidden')} side="bottom" delayMs={500}>
                <button
                  type="button"
                  className={clsx(css.filesToggle, filesShowHidden && css.filesToggleActive)}
                  aria-label={t('files.showHidden')}
                  aria-pressed={filesShowHidden}
                  onClick={() => { setFilesShowHidden(value => !value) }}
                >
                  <IconBrowseOutline16 />
                </button>
              </Tooltip>
          ...
          {/* Adding is the button's one action, so a composition with no
              ...
              button rather than leaving a dead one in the header. Wide Files
              mode swaps the actions for the tree chrome; the rail keeps the
              button (its click returns the region to Sessions). */}
          {directoryFlowAvailable && (mode === 'sessions' || !wide) && (
            <Tooltip label={t('workspace.add')} side="bottom" delayMs={500}>
              <button
                ...
                onClick={() => {
                  // The rail keeps adding as a Sessions-mode control: a rail
                  // pick targets a Session list, so the gesture returns there.
                  actions.setMode('sessions')
                  setWsPickerOpen(v => !v)
                }}
              >
      ...
      {/* The collapsed rail keeps search as its own 36px control; search is
          Sessions chrome, so the gesture returns the region to Sessions
          before expanding into the field. */}
      ...
      {/* The rail's Files entry: one 36px control that expands the sidebar
          into the Files mode (the same expand-plus-select contract as the
          rail search). */}
      {!wide && <div className={css.filesRail}>
        <Tooltip label={t('mode.files')}>
          <button
            type="button"
            className={css.filesRailButton}
            aria-label={t('files.rail.aria')}
            onClick={() => {
              actions.setMode('files')
              expandSidebar()
            }}
      ...
      <div className={css.listArea}>
        {wide && mode === 'files'
          ? filesWorkspace === undefined
            ? <div className={css.empty}>{t('files.empty.noWorkspaces')}</div>
            : (
              <FilesBrowser
                workspace={filesWorkspace}
                browse={browse}
                openFile={openFile}
                showHidden={filesShowHidden}
            ...
            ? (
              <ArchivedList
                ...
                onDelete={(sessionId, title) => {
                  setArchivedDeleteTarget({ sessionId, title })
                  setArchivedDeleteError(null)
                }}
                t={t}
              />
      ...
      <Modal
        ...
        onClose={closeArchivedDelete}
        closeLabel={t('close')}
        title={t('delete.session.title')}
        {...archivedDeleteTarget === null
          ? {}
          : { description: t('delete.session.desc', { name: archivedDeleteTarget.title }) }}
        footer={(
          <>
            <Button variant="outline" disabled={archivedDeleting} onClick={closeArchivedDelete}>{t('cancel')}</Button>
            <Button
              variant="outline"
              className={css.deleteAction}
              disabled={archivedDeleting}
              onClick={confirmArchivedDelete}
            >
      ...
      >
        {archivedDeleting && <div className={css.deleteStatus} role="status">{t('delete.session.pending')}</div>}
        {archivedDeleteError !== null && <div className={css.renameError} role="alert">{archivedDeleteError}</div>}
      </Modal>
      <Modal
