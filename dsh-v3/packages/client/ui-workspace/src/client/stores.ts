...
/** Region mode: the Session list, archived Sessions, or the current Workspace's file tree. */
export type WorkspaceBrowserMode = 'sessions' | 'archived' | 'files'

/** Workspace browser viewing state persisted across surface remounts and reloads. */
type WorkspaceViewState = {
  /** Region mode: Sessions vs Files (the tree's own cache is component-local). */
...
type WorkspaceViewActions = {
  setMode: (draft: WorkspaceViewState, mode: WorkspaceBrowserMode) => void
...
export function createWorkspaceViewStore(): EngineStoreHandle<WorkspaceViewState, WorkspaceViewActions> {
  return defineStore({
    ...
    actions: {
      setMode: (d, mode: WorkspaceBrowserMode) => { d.mode = mode },
