...
function mount(overrides: Partial<WorkspaceBrowserProps> = {}) {
  ...
  const props: WorkspaceBrowserProps = {
    ...
    browse: vi.fn(async () => ({ relativePath: '', entries: [], truncated: false })),
...
describe('WorkspaceBrowser', () => {
  ...
  it('renders the grouped tree by default and switches to the flat list via Group by', () => {
    ...
    expect(screen.getByRole('button', { name: '会话', pressed: true })).toBeTruthy()
    ...
    // Store-driven flip: the Sessions segment stays pressed, rows flatten
    // newest-first, headers gone.
  ...
  it('opens archived sessions and offers restore plus confirmed permanent delete', async () => {
    const open = vi.fn()
    const unarchiveSession = vi.fn(async () => {})
    const deleteArchivedSession = vi.fn(async () => {})
    mount({
      useSessions: hook(sessionState([summary('archived-s', 1)])),
    ...
    fireEvent.click(screen.getByText('archived-s'))
    expect(open).toHaveBeenCalledWith(sid('archived-s'))

    fireEvent.click(screen.getByRole('button', { name: '归档会话“archived-s”的操作' }))
    fireEvent.click(screen.getByRole('menuitem', { name: '恢复会话' }))
    expect(unarchiveSession).toHaveBeenCalledWith(sid('archived-s'))

    fireEvent.click(screen.getByRole('button', { name: '归档会话“archived-s”的操作' }))
    fireEvent.click(screen.getByRole('menuitem', { name: '永久删除' }))
    expect(screen.getByText('将永久删除“archived-s”及其会话记录。此操作无法撤销。')).toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: '永久删除' }))
    await waitFor(() => { expect(deleteArchivedSession).toHaveBeenCalledWith(sid('archived-s')) })
  })

  it('disables permanent delete while an archived session is active', () => {
    mount({
      useSessions: hook(sessionState([{ ...summary('busy-s', 1), running: true }])),
      useWorkspaces: hook(workspaceState([], [sid('busy-s')])),
    })
    fireEvent.click(screen.getByRole('button', { name: '归档' }))
    fireEvent.click(screen.getByRole('button', { name: '归档会话“busy-s”的操作' }))
    expect((screen.getByRole('menuitem', { name: '永久删除' }) as HTMLButtonElement).disabled).toBe(true)
  })
  ...
  it('switches between Sessions and Files through the header segments', async () => {
    const browse = vi.fn(async () => ({
      relativePath: '',
      entries: [
        { name: 'src', relativePath: 'src', path: '/projects/alpha/src', kind: 'directory' as const, hidden: false },
        { name: 'README.md', relativePath: 'README.md', path: '/projects/alpha/README.md', kind: 'file' as const, hidden: false },
      ],
      truncated: false,
    }))
    const sessions = sessionState([summary('alpha-s', 1)], { current: sid('alpha-s') })
    const b = mount({
      useSessions: hook(sessions),
      useWorkspaces: hook(workspaceState([workspace('alpha', ['alpha-s'])])),
      browse,
    })
    // Sessions mode is the default; the segments replace the old section label.
    expect(screen.getByRole('button', { name: '会话', pressed: true })).toBeTruthy()
    expect(screen.getByRole('button', { name: '文件', pressed: false })).toBeTruthy()
    expect(screen.getByPlaceholderText('搜索会话…')).toBeTruthy()

    fireEvent.click(screen.getByRole('button', { name: '文件' }))
    expect(b.store.getSnapshot().mode).toBe('files')
    // The files header identifies the resolved Workspace (the current
    // Session's) and hides the Sessions chrome.
    expect(screen.getByText('alpha')).toBeTruthy()
    expect(screen.queryByPlaceholderText('搜索会话…')).toBeNull()
    expect(screen.queryByRole('button', { name: '视图选项' })).toBeNull()
    await waitFor(() => {
      expect(browse).toHaveBeenCalledWith(wid('alpha'), undefined, expect.any(AbortSignal))
      expect(screen.getByText('src')).toBeTruthy()
    })
    expect(screen.getByText('README.md')).toBeTruthy()

    // Back to Sessions restores the session tree; the current Session's
    // group stays auto-expanded, so the row is immediately visible.
    fireEvent.click(screen.getByRole('button', { name: '会话' }))
    expect(b.store.getSnapshot().mode).toBe('sessions')
    expect(screen.getByPlaceholderText('搜索会话…')).toBeTruthy()
    expect(screen.getByText('alpha-s')).toBeTruthy()
  })

  it('resolves the file Workspace from the current Session, then recency, then the first', async () => {
    const sessions = sessionState([summary('alpha-s', 1)], { current: sid('alpha-s') })
    const workspaces = workspaceState([workspace('alpha', ['alpha-s']), workspace('beta', [])])
    const b = mount({
      useSessions: hook(sessions),
      useWorkspaces: hook(workspaces),
    })
    fireEvent.click(screen.getByRole('button', { name: '文件' }))
    // Current Session's Workspace wins even though the recent Workspace is beta.
    expect(screen.getAllByText('alpha')).toHaveLength(1)

    // No current Session: the recent Workspace is the target.
    rerender(b, { useSessions: hook(sessionState([])) })
    expect(screen.getAllByText('alpha')).toHaveLength(1)
    rerender(b, { useWorkspaces: hook({ ...workspaces, recentWorkspaceId: wid('beta') }) })
    expect(screen.getAllByText('beta')).toHaveLength(1)
    // No recency projection: the first registered Workspace wins.
    rerender(b, { useWorkspaces: hook({ ...workspaces, recentWorkspaceId: undefined }) })
    expect(screen.getAllByText('alpha')).toHaveLength(1)
  })

  it('files header offers refresh and the hidden toggle; refresh reloads the root', async () => {
    const browse = vi.fn(async () => ({
      relativePath: '',
      entries: [
        { name: '.env', relativePath: '.env', path: '/projects/alpha/.env', kind: 'file' as const, hidden: true },
        { name: 'README.md', relativePath: 'README.md', path: '/projects/alpha/README.md', kind: 'file' as const, hidden: false },
      ],
      truncated: false,
    }))
    ...
    // The files chrome is two compact icon controls: an SVG glyph each and no
    // visible label, both reachable by name at the default 280px sidebar
    // width; the reveal affordance lives in the accessible names.
    for (const name of ['显示隐藏文件', '刷新文件列表']) {
      const control = screen.getByRole('button', { name })
      expect(control.querySelector('svg')).toBeTruthy()
      expect(control.textContent?.trim()).toBe('')
    }
    // Hidden entries default hidden; the toggle reveals them and stays pressed.
    ...
    // Refresh clears the cache and re-lists the root.
    fireEvent.click(screen.getByRole('button', { name: '刷新文件列表' }))
    await waitFor(() => {
      expect(browse).toHaveBeenCalledTimes(2)
      expect(browse).toHaveBeenLastCalledWith(wid('alpha'), undefined, expect.any(AbortSignal))
    })
  })

  it('shows the no-workspaces state in Files mode', () => {
    mount()
    fireEvent.click(screen.getByRole('button', { name: '文件' }))
  ...
  it('rail exposes a Files icon that expands the sidebar into Files mode; search returns to Sessions', () => {
    vi.useFakeTimers()
    try {
      const expandSidebar = vi.fn()
      const b = mount({ wide: false, expandSidebar })
      const filesRail = screen.getByRole('button', { name: '查看文件' })
      fireEvent.click(filesRail)
      expect(expandSidebar).toHaveBeenCalledTimes(1)
      expect(b.store.getSnapshot().mode).toBe('files')
      // The wide flip renders the Files mode without any further gesture.
      rerender(b, { wide: true })
      expect(screen.getByText('暂无工作区')).toBeTruthy()
      expect(screen.getByRole('button', { name: '文件', pressed: true })).toBeTruthy()

      // Rail search is Sessions chrome: it returns to Sessions and expands.
  ...
  it('rail add-workspace returns to Sessions mode before raising the flow', () => {
    const b = mount({
      wide: false,
      useWorkspaces: hook(workspaceState([workspace('alpha', [])])),
    })
    b.store.actions.setMode('files')
    fireEvent.click(screen.getByRole('button', { name: '添加工作区' }))
    expect(b.store.getSnapshot().mode).toBe('sessions')
    expect(screen.getByTestId('directory-flow')).toBeTruthy()
  })
