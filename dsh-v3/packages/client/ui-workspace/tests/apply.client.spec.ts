...
async function bench() {
  ...
  const browse = vi.fn(async () => ({ relativePath: '', entries: [], truncated: false }))
  ...
  ctx.provide('workspaces', {
    create, startSession, rename, insertSessionBefore, browse, openPath,
  } as never)
  ...
  return {
    ...
    insertSessionBefore, browse, openPath, open, clear, search, renameSession, binding, fork,
  }
}
...
describe('ui-workspace apply', () => {
  ...
  it('routes browser actions and picker creation to the services', async () => {
    ...
    await browser.browse('ws' as never, 'src', signal)
    expect(b.browse).toHaveBeenCalledWith('ws', 'src', signal)
    await browser.browse('ws' as never)
    expect(b.browse).toHaveBeenLastCalledWith('ws', undefined, undefined)
    await browser.openFile('/tmp/browser-project/README.md')
    expect(b.openPath).toHaveBeenCalledWith('/tmp/browser-project/README.md')
