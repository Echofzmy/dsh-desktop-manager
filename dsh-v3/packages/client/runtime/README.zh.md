...
`WorkspaceListState.archivedSessionIds` 镜像 Host 的注册表级全局归档集合（一个按 Host 顺序的 `readonly SessionId[]`，仅在成员变化时才替换；需要 O(1) 查询的消费方自建临时 Set）。它是全快照状态：`workspace.list` 基线、归档／恢复／删除的一元回声和 `host/archived-sessions-changed` 帧各自安装完整集合。`WorkspaceRuntime.archiveSession(sessionId)` 通过 wire 归档，并只为发起该本地操作的 selection 清空当前会话。`WorkspaceRuntime.unarchiveSession(sessionId)` 恢复归档成员关系且不移动其 Workspace 记账，`deleteArchivedSession(sessionId)` 则永久删除 Host 记录并从归档集合移除该 id。远程归档的 Session 仍可保持选中，以便查看其历史。在 `workspace.list` 请求进行中安装的集合还会取代该过期基线携带的集合。各分组视图在所有位置隐藏集合成员，而会话行本身仍留在列表 store 中。
...
`toolOutcomeKind()` 是客户端唯一的已结算工具结果分类器（`ToolOutcomeKind` = `success | stopped | blocked | error`）。它只读持久化的 `isError` 标志与 `error.code`，绝不解析模型可见文本。失败调用在码为取消/中断时归为 `stopped`（`ABORTED`、`ABORTED_BEFORE_DISPATCH`、`TOOL_NOT_STARTED`、`TOOL_OUTCOME_UNKNOWN`、`ASK_ABORTED`、`ASK_CANCELLED`、客户端合成的 `interrupted`，以及预留的 `TOOL_APPROVAL_CANCELLED` / `SANDBOX_ESCALATION_CANCELLED`）；在码为拒绝或可纠正条件时归为 `blocked`（`TOOL_POLICY_DENIED`、`TOOL_APPROVAL_BLOCKED`/`REJECTED`/`UNAVAILABLE`、`TOOL_POST_BLOCKED`、`SANDBOX_ESCALATION_BLOCKED`/`REJECTED`/`UNAVAILABLE`、`INVALID_ARGS`、`UNKNOWN_TOOL`、`GOAL_TOOL_*` 族，以及显式列出的轨迹审计条件 `FS_NOT_FOUND`、`FS_NOT_OBSERVED`、`FS_STALE_VERSION`、`SEARCH_INVALID_PATTERN`、`SEARCH_RAW_OUTPUT_OVERFLOW`——不使用宽泛的 `FS_`/`SEARCH_` 前缀规则，因此 `SEARCH_FAILED` 等同前缀码仍保持 `error`）；其余未知或无码失败保持 `error`。Chat 行与 Trajectory 视图都消费该分类器，因此 stopped/blocked 的调用以非红呈现，真实故障保持红色。
...
Trajectory Definition 组装出一条按时间顺序排列、以用途为判别字段的提供方请求流。助手请求始终携带数值型 `turn` 与 `step`；压缩请求携带 `step: 0`，其 `turn` 所有者可以是 `null`。这个 null 所有者表示手动压缩独立运行在两个轮次之间，并不表示它属于任一相邻轮次。由取消定稿的 `assistant/message` 会保留持久结果 seq 和提供方信息，但不会将请求标记为完成：它结算为 `stopped`（用户/父级中止）；因重试耗尽或非用户/父级中止的边界而关闭的请求则结算为 `error`。`session/end-seed` 边界会在边界时刻将未匹配的压缩请求以错误状态结束，错误固定为 `Compaction was interrupted before completion.`；后续 start 会投影为独立请求，而不会覆盖这项遗留的未匹配请求。
