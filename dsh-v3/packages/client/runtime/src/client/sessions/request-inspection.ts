...
interface RequestViewBase {
  ...
  /**
   * Request lifecycle: `running` while the step is open, `complete` once the
   * assistant message landed, `stopped` when a user/parent abort interrupted
   * the request (never a provider failure), `error` for a real failure (a
   * provider error, an exhausted retry, or a boundary closed without an
   * interrupted result).
   */
