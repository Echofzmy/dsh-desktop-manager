...
describe('deriveTrajectoryLayout', () => {
  ...
  it('maps a stopped request to a non-red stopped message record and an error request to error', () => {
    const requestCell = (status: RequestView['status']) => deriveTrajectoryLayout({
      nodes: [],
      partial: null,
      requests: [{
        purpose: 'assistant', startSeq: 3, turn: 1, step: 1,
        startedAt: 3_000, completedAt: 4_000, status,
      }] as unknown as RequestView[],
      runningCalls: [],
    })[0]?.groups[0]?.cells[0]

    const stopped = requestCell('stopped')
    expect(stopped).toMatchObject({ kind: 'message', state: 'stopped' })
    expect(stopped?.isError).toBeUndefined()
    const failed = requestCell('error')
    expect(failed).toMatchObject({ kind: 'message', state: 'error', isError: true })
    const running = requestCell('running')
...
describe('tool outcome states in trajectory cells', () => {
  const toolResult = (
    callId: string, isError: boolean,
    code: string | undefined,
  ): unknown => ({
    kind: 'tool-result', seq: 10, time: 3_000, callId,
    call: { name: 'bash', argsRaw: '{}' }, callTime: 1_000,
    content: [{ type: 'text', text: 'outcome text' }],
    isError, ...(code === undefined ? {} : { error: { name: 'E', code } }),
    callView: null, resultView: null,
  })

  const derive = (node: unknown) => deriveTrajectoryLayout({
    nodes: [
      { kind: 'user', seq: 1, time: 500, content: [{ type: 'text', text: 'hi' }], source: null },
      {
        kind: 'assistant', seq: 2, time: 1_000, turn: 1, step: 1,
        blocks: [{ kind: 'tool-call', callId: 'c1', name: 'bash', argsRaw: '{}' }],
      },
      node,
    ] as unknown as ConversationSnapshot['nodes'],
  ...
  const toolCell = (node: unknown) =>
    derive(node)[0]!.groups.flatMap(group => group.cells).find(cell => cell.kind === 'tool')

  it('classifies a successful call success', () => {
    const cell = toolCell(toolResult('c1', false, undefined))
    expect(cell?.isError).toBe(false)
    expect(cell?.state).toBe('success')
  })

  it('classifies interrupted and aborted calls stopped', () => {
    expect(toolCell(toolResult('c1', true, 'interrupted'))?.state).toBe('stopped')
    expect(toolCell(toolResult('c1', true, 'ABORTED_BEFORE_DISPATCH'))?.state).toBe('stopped')
  })

  it('classifies policy denials and correctable conditions blocked', () => {
    expect(toolCell(toolResult('c1', true, 'TOOL_POLICY_DENIED'))?.state).toBe('blocked')
    expect(toolCell(toolResult('c1', true, 'INVALID_ARGS'))?.state).toBe('blocked')
  })

  it('keeps unknown failures error', () => {
    expect(toolCell(toolResult('c1', true, 'PLUGIN_ONLY_CODE'))?.state).toBe('error')
    expect(toolCell(toolResult('c1', true, undefined))?.state).toBe('error')
  })
})
