...
/**
 * A closed step/turn boundary narrowed to the fields the request lifecycle
 * needs: enough event identity to tell a user/parent abort (a `turn/end` with
 * an `aborted` reason) from a provider failure. Structural shape, so the
 * definition needs no direct `dsh-session` dependency.
 */
type ClosedBoundary =
  | {
...
/** Whether a closed boundary is a turn ended by an abort (user/parent cancel), not a provider failure. */
function isAbortBoundary(boundary: ClosedBoundary): boolean {
  return boundary.type === 'turn/end' && boundary.data.reason.kind === 'aborted'
}
...
function assistantRequest(
  ...
  boundary: ClosedBoundary | undefined,
): Extract<RequestView, { purpose: 'assistant' }> | undefined {
  if (!state.started) return undefined
  // A user/parent stop (an interrupted message, or an aborted turn with no
  // visible result) settles as `stopped`, never as a failure; a retry or a
  // boundary closed by anything else keeps `error` so real provider failures
  ...
  const status = node !== undefined && node.interrupted !== true
    ...
    : state.retry !== undefined
      ...
      : node !== undefined
        ? 'stopped'
        : boundary !== undefined && isAbortBoundary(boundary)
          ? 'stopped'
