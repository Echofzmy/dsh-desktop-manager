...
type SkillRowState = 'running' | 'ok' | 'error' | 'stopped' | 'blocked'
...
/** Drop the registry's `Error: ` envelope from a blocked denial line (display only; the durable text is untouched). */
...
function skillRowModel(block: ToolCallViewProps['block']): SkillRowModel {
  ...
  const outcome = settled ? toolOutcomeKind(block) : undefined
  const state: SkillRowState = outcome === undefined
    ? 'running'
    : outcome === 'success' ? 'ok' : outcome
  const output = resultText(block)
  return {
    name: skillName(argsRaw, block.callId),
    output,
    // The blocked denial line drops the registry's `Error: ` envelope; an
    // error row keeps it, and a stopped row keeps the args summary.
    errorSummary: output === null
      ? null
      : state === 'error'
        ? firstLine(output)
        : state === 'blocked'
  ...
  return (
    <div className={css.card} data-tool="skill" data-state={model.state}>
      ...
      >
        ...
        <span className={model.errorSummary === null
          ? css.summary
          : `${css.summary} ${model.state === 'error' ? css.errorSummary : css.blockedSummary}`}>
          {summary}
        </span>
      </div>
      {open ? (
