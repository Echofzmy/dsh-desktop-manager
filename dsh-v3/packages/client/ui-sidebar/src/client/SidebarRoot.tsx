            ...
            <span className={css.brandIdentity} aria-hidden="true">
              ...
              <span className={css.brandName}>
                {renderSlot('sidebar.brand.name', {}, {
                  fallback: (
                    <>
                      <span className={css.fallbackBrandName}>dsh</span>
                      {typeof window !== 'undefined' && new URLSearchParams(window.location.search).get('dsh-instance')?.trim()
                        ? <span style={{ padding: '2px 7px', borderRadius: 4, background: '#111', color: '#fff', fontSize: '0.78em', fontWeight: 600, lineHeight: 1.4 }}>{new URLSearchParams(window.location.search).get('dsh-instance')?.trim()}</span>
                        : null}
                    </>
                  ),
                })}
