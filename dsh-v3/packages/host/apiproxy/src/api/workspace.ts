...
/** One child row of a workspace browse listing. */
export interface WorkspaceFileEntry {
  /** Base name shown in a browser row. */
  name: string
  /**
   * Workspace-rooted path relative to the workspace directory; the client
   * never joins path segments itself and passes it back verbatim to browse
   * a child directory.
   */
  relativePath: string
  /** Absolute Host path for the entry; passed verbatim to openPath, the client does not join it. */
  path: string
  /** Directory entries are themselves browsable; the v1 backend skips symlinks and never lists them. */
  kind: 'file' | 'directory'
  /** Hidden by the host platform's convention (dot-prefixed on POSIX); the client owns whether to show it. */
  hidden: boolean
}

/** workspace.browse response value: one workspace-rooted directory level. */
export interface WorkspaceDirectoryListing {
  /** The listed directory's workspace-rooted path; '' is the workspace root. */
  relativePath: string
  /** Direct children, name-sorted; the complete-result bound may cut the tail. */
  entries: WorkspaceFileEntry[]
  /** True when the backend cut `entries` at its bound (the name-sorted tail is absent). */
  truncated: boolean
}
...
export interface WorkspaceApi {
  ...
  /**
   * Lists one direct level of a workspace-rooted directory for the in-app
   * file browser; an absent `relativePath` (or `''`) lists the workspace
   * root. Rows are the real direct children — symlinks are never listed, and
   * unusual node types (FIFO, socket, device) are skipped — sorted
   * directories-first then by name, with dot-prefixed names flagged
   * `hidden`. The backend realpaths the target and refuses any path whose
   * canonical form escapes the workspace, so a symlinked parent cannot smuggle
   * a listing out of it. An unknown workspace fails with
   * `workspace-not-found`; a syntactically invalid `relativePath` (absolute
   * POSIX/Windows/UNC, NUL, or `.`/`..`/empty segments), one escaping the
   * workspace, or one resolving to a missing, unreadable, or non-directory
   * target all fail with `directory-unreadable`. The combined file+directory
   * row bound is the deployment's `workspaceBrowseMaxEntries` (default 1000);
   * a cut tail reports `truncated: true`. The carrier's request signal follows
   * the caller, stopping the backend's scan and answering `cancelled` on
   * disconnect or timeout.
   */
  browse(
    request: RpcRequest<{ workspaceId: WorkspaceId; relativePath?: string }>,
    signal: AbortSignal,
   ...
   * Archiving also releases the current Web runtime's live owner after the
   * archive commit; other runtimes observe the same commit and release theirs.
  ...
  /** Restore one archived session to ordinary lists without moving its workspace slot. */
  unarchiveSession(request: RpcRequest<{ sessionId: SessionId }>):
  Promise<RpcResponse<{ archivedSessionIds: SessionId[] }>>

  /**
   * Permanently delete one archived session and its durable log. Running live
   * sessions fail with `agent-busy`; an archived idle session may be deleted
   * even while an external runtime is finishing its lifecycle. A non-archived
   * id fails with `session-not-archived`.
   */
  deleteArchivedSession(request: RpcRequest<{ sessionId: SessionId }>):
