...
import type { WorkspaceDirectoryListing, WorkspaceFileEntry, WorkspaceView } from './workspace.ts'
...
/** workspace.browse request payload; an absent relativePath lists the workspace root. */
export const workspaceBrowseRequestSchema = z.object({
  workspaceId: workspaceIdSchema,
  relativePath: z.string().optional(),
}) satisfies z.ZodType<Wire<RequestPayload<'workspace.browse'>>>
...
export const workspaceDirectoryListingSchema = z.object({
  relativePath: z.string(),
  entries: z.array(workspaceFileEntrySchema),
  truncated: z.boolean(),
}) satisfies z.ZodType<Wire<WorkspaceDirectoryListing>>

/** workspace.browse response value (the listing is the whole value). */
export const workspaceBrowseValueSchema = workspaceDirectoryListingSchema satisfies z.ZodType<Wire<ResponseValue<'workspace.browse'>>>
...
/** workspace.deleteArchivedSession request payload. */
export const workspaceDeleteArchivedSessionRequestSchema = z.object({
  sessionId: sessionIdSchema,
}) satisfies z.ZodType<Wire<RequestPayload<'workspace.deleteArchivedSession'>>>

/** workspace.deleteArchivedSession response value. */
export const workspaceDeleteArchivedSessionValueSchema = z.object({
  deleted: z.literal(true),
  archivedSessionIds: z.array(sessionIdSchema),
}) satisfies z.ZodType<Wire<ResponseValue<'workspace.deleteArchivedSession'>>>
