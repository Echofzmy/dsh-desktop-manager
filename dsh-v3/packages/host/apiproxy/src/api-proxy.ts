...
/**
 * Default maximum combined file+directory rows one workspace.browse listing
 * carries. The gateway config schema's default; a deployment raises or lowers
 * it from cordis.yml, never here.
 */
export const DEFAULT_WORKSPACE_BROWSE_MAX_ENTRIES = 1000
...
export interface ApiProxyDefaults {
  ...
  /**
   * Maximum combined file+directory rows one workspace.browse listing
   * carries: the name-sorted tail past the bound is cut and reported as
   * `truncated`. Defaults to {@link DEFAULT_WORKSPACE_BROWSE_MAX_ENTRIES},
   * the gateway config schema's default.
   */
...
/**
 * Validate one workspace.browse relativePath as a workspace-rooted POSIX-style
 * relative path. Runtime enforcement at the wire boundary, independent of the
 * zod schema: the backend must never join a caller-supplied path that could
 * escape the workspace or read outside it, and schema-only enforcement would
 * let a direct or alternate caller bypass the rule.
 * @param relativePath - the request path; '' is the workspace root.
 * @returns a human-readable problem description, or `undefined` when the path
 * is a legal workspace-rooted relative path.
 */
function browseRelativePathError(relativePath: string): string | undefined {
  if (relativePath === '') return undefined
  if (relativePath.includes('\0')) return 'the path contains a NUL character'
  if (relativePath.startsWith('/')) return 'the path is absolute'
  if (/^[A-Za-z]:/.test(relativePath)) return 'the path is an absolute Windows path'
  if (relativePath.startsWith('\\')) return 'the path is a UNC path'
  for (const segment of relativePath.split('/')) {
    if (segment === '') return 'the path contains an empty segment'
    if (segment === '.') return 'the path contains a "." segment'
    if (segment === '..') return 'the path contains a ".." segment'
  }
  return undefined
}

/** Project one scanned dirent into its wire row: workspace-rooted and absolute paths, host-owned kind and hidden flag. */
function browseRow(target: string, relativePath: string, entry: Dirent, kind: 'file' | 'directory'): WorkspaceFileEntry {
  return {
    name: entry.name,
    relativePath: relativePath === '' ? entry.name : `${relativePath}/${entry.name}`,
...
/** The deterministic name order of every browse group. */
function browseRowByName(left: WorkspaceFileEntry, right: WorkspaceFileEntry): number {
  return left.name < right.name ? -1 : left.name > right.name ? 1 : 0
}

/**
 * Append `row` to the name-sorted capped array and keep only the smallest
 * `max` names. Memory stays bounded by the listing bound (at most `max + 1`
 * rows transiently), and the kept set always equals the full name-sorted
 * order's prefix, so the final slice is exactly what a complete
 * collect-and-sort would have produced.
 */
function capBrowseRow(rows: WorkspaceFileEntry[], row: WorkspaceFileEntry, max: number): void {
  rows.push(row)
  rows.sort(browseRowByName)
  if (rows.length > max) rows.pop()
}

/**
 * List one direct level of a workspace-rooted directory. `root` is the
 * workspace's canonical path (realpath'd at create), and the target is
 * realpath'd again here so a symlinked parent or child segment cannot escape
 * containment. The scan reads every dirent (the truncated flag must be
 * truthful) but holds at most `maxEntries` rows per kind, skips symlinks and
 * unusual node types, and stops on abort via the checked signal.
 * @param root - canonical workspace directory.
 * @param relativePath - workspace-rooted target path ('' = the root); already
 * syntax-validated by {@link browseRelativePathError}.
 * @param maxEntries - combined file+directory row bound; the name-sorted tail
 * past it is cut and reported as `truncated`.
 * @param signal - the carrier's request signal; an abort throws from the
 * scan's next check so the caller maps it to `cancelled`.
 * @returns the one-level listing, with rows sorted directories-first then by
 * name and dot-prefixed names flagged `hidden`.
 * @throws on any missing, unreadable, outside, or non-directory target, and
 * on abort.
 */
async function browseWorkspaceLevel(
  root: string,
  relativePath: string,
  maxEntries: number,
  signal: AbortSignal,
  ...
  signal.throwIfAborted()
  const target = await realpath(relativePath === '' ? canonicalRoot : join(canonicalRoot, relativePath))
  signal.throwIfAborted()
  if (target !== canonicalRoot && !target.startsWith(canonicalRoot + sep)) {
    throw new Error('the path resolves outside the workspace')
  }
  signal.throwIfAborted()
  if (!(await stat(target)).isDirectory()) {
    throw new Error('the path does not resolve to a directory')
  }
  const dir = await opendir(target)
  const directories: WorkspaceFileEntry[] = []
  ...
  try {
    while (true) {
      ...
      if (entry === null) break
      // A symlink is never a browsable row: its target may leave the
      // workspace, and the listing deliberately shows only real children.
      if (entry.isSymbolicLink()) continue
      if (entry.isDirectory()) {
        directoryCount++
        capBrowseRow(directories, browseRow(target, relativePath, entry, 'directory'), maxEntries)
      } else if (entry.isFile()) {
        fileCount++
        capBrowseRow(files, browseRow(target, relativePath, entry, 'file'), maxEntries)
      }
      // Unusual node types (FIFO, socket, device, unknown) are not rows.
    }
  } finally {
    await dir.close()
  }
  const shown = directories.slice(0, maxEntries)
  return {
    relativePath,
    entries: [...shown, ...files.slice(0, maxEntries - shown.length)],
    truncated: directoryCount + fileCount > maxEntries,
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  const workspaceBrowseMaxEntries = defaults.workspaceBrowseMaxEntries
    ?? DEFAULT_WORKSPACE_BROWSE_MAX_ENTRIES
  ...
  /** Exact Web-runtime lifecycle capabilities for sessions this proxy created or resumed. */
  const sessionHandles = new Map<SessionId, AgentHandle>()
  /** Serializes owner release when archive notifications and delete requests race. */
  const sessionReleases = new Map<SessionId, Promise<void>>()
  const workspaceRegistry = ctx.get('workspaceRegistry')
  const sessionPersistence = ctx.get('sessionPersistence')
  let archivedSessionIds = new Set<SessionId>(workspaceRegistry?.archivedSessionIds ?? [])
  const releaseSessionHandle = (sessionId: SessionId): Promise<void> => {
    const pending = sessionReleases.get(sessionId)
    ...
    })().finally(() => { sessionReleases.delete(sessionId) })
    sessionReleases.set(sessionId, release)
    return release
  }
  // Archive is a lifecycle transition: every Web runtime that owns the session
  // releases its handle after the durable archive commit is observed.
  ctx.on('domain/changed', change => {
    if (change.domain !== 'workspace' || change.table !== '' || change.operation !== 'put') return
    const state = workspaceDomainState.parse(change.value)
    const previous = archivedSessionIds
    const next = new Set<SessionId>(state.archivedSessionIds)
    archivedSessionIds = next
    for (const sessionId of next) {
      if (!previous.has(sessionId)) void releaseSessionHandle(sessionId).catch(error => console.error('Failed to release archived session', error))
    }
  })
  ...
  return {
    ...
    workspace: {
      ...
      async browse(request, signal) {
        // An already-aborted call answers cancelled without touching the
        // registry or the filesystem, like the other signal-following RPCs.
        if (signal.aborted) {
          return err(request, { code: 'cancelled', message: 'workspace browse was cancelled', details: {} })
        }
        ...
        const relativePath = request.payload.relativePath ?? ''
        const attempted = relativePath === '' ? workspace.path : join(workspace.path, relativePath)
        const pathError = browseRelativePathError(relativePath)
        if (pathError !== undefined) {
          return err(request, {
            code: 'directory-unreadable',
            message: `cannot browse "${attempted}": ${pathError}`,
            details: { path: attempted },
          })
        }
        try {
          return ok(request, await browseWorkspaceLevel(
            workspace.path,
            relativePath,
            workspaceBrowseMaxEntries,
            signal,
          ))
        } catch (error: unknown) {
          // An abort is the caller's own timeout/disconnect, not a server
          // failure — same code pickDirectory and listDirectory report.
          if (signal.aborted) {
            return err(request, { code: 'cancelled', message: 'workspace browse was cancelled', details: {} })
          }
          return err(request, {
            code: 'directory-unreadable',
            message: `cannot browse "${attempted}": ${error instanceof Error ? error.message : String(error)}`,
            details: { path: attempted },
          })
        }
      },
      ...
      async archiveSession(request) {
        ...
        try {
          ...
          await releaseSessionHandle(sessionId)
        } catch (error: unknown) {
      ...
      async deleteArchivedSession(request) {
        const { sessionId } = request.payload
        if (!ctx.workspaceRegistry.archivedSessionIds.includes(sessionId)) {
          return err(request, {
            code: 'session-not-archived',
            message: `cannot permanently delete session '${sessionId}': it is not archived`,
            details: { sessionId },
          })
        }
        try {
          ...
          return err(request, {
            code: 'agent-busy',
            message: `cannot permanently delete session '${sessionId}': its owner failed to close`,
        ...
        if (live?.status === 'running') {
          return err(request, {
            code: 'agent-busy',
            message: `cannot permanently delete session '${sessionId}' while it is running`,
            details: { reason: 'session is running' },
          })
        }
        if (live !== undefined) {
          const retirement = await ctx.agents.retireIdleRoot(sessionId)
          if (retirement !== 'retired' && retirement !== 'not-found') {
            return err(request, {
              code: 'agent-busy',
              message: `cannot permanently delete session '${sessionId}': its live owner cannot be retired`,
        ...
        if (sessionPersistence === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session delete failed: session persistence is unavailable',
            details: { sessionId },
          })
        }
        let deleted: boolean
        try {
          deleted = await sessionPersistence.delete(sessionId)
        } catch (error: unknown) {
          if (String(error).includes('while it is live')) {
            return err(request, {
              code: 'agent-busy',
              message: `cannot permanently delete session '${sessionId}': its live owner has not finished closing`,
              details: { reason: 'session is still live' },
            })
          }
          return err(request, {
            code: 'internal',
            message: `session delete failed: ${error instanceof Error ? error.message : String(error)}`,
            details: { sessionId },
          })
        }
        if (!deleted) {
          return err(request, {
            code: 'session-not-found',
            message: `cannot permanently delete session '${sessionId}': no durable log exists`,
            details: { sessionId },
          })
        }
        try {
          ...
          return err(request, {
            code: 'internal',
            message: `session registry cleanup failed: ${error instanceof Error ? error.message : String(error)}`,
            details: { sessionId },
          })
        }
    ...
    events: {
      ...
      host(_request, signal) {
        ...
        let streamedArchivedSessionIds = ctx.workspaceRegistry.archivedSessionIds
        const disposers = [
          ctx.on('session/created', (session: Session) => {
            queue.push(frame({
          ...
          ctx.on('session/disposed', (session: Session) => {
            // Archiving retires the live lifecycle but keeps the durable session.
            // Permanent deletion emits its removal explicitly after storage commits.
          ...
          ctx.on('domain/changed', (change) => {
            ...
            if (change.table === '') {
              ...
              if (state.archivedSessionIds.length !== streamedArchivedSessionIds.length
                || state.archivedSessionIds.some((id, index) => id !== streamedArchivedSessionIds[index])) {
                streamedArchivedSessionIds = state.archivedSessionIds
                queue.push(frame({
