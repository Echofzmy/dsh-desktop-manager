...
describe('approval policy (the approval/policy fold)', () => {
  ...
  it('a never config blocks deterministically without consulting any answerer', async () => {
    const ctx = new Context()
    ...
    await expect(ctx.approval.request({ agent, toolName: 'bash' })).resolves.toBe('blocked')
  ...
  it('never is unbypassable even by an answerer PREPENDED after the service mounts', async () => {
    ...
    const { agent, appended } = fakeAgent()
    await expect(ctx.approval.request(requestOf(agent))).resolves.toBe('blocked')
    expect(consulted).not.toHaveBeenCalled()
    expect(appended.map(e => e.type)).toEqual(['approval/asked', 'approval/decided'])
  })
