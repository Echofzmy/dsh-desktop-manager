...
The shared escalation choreography ([`approveEscalation`](src/escalation.ts), used by the bash/pwsh and filesystem tool families) resolves a `sandbox_permissions` request against the call's effective mode: a requested mode that is NOT strictly wider (equal or narrower — a strict-schema redundant field) runs under the effective mode with no approval and no error; only a strictly wider request goes through `ctx.approval`. Every escalation failure throws a `HarnessError` carrying a stable code from `ESCALATION_ERROR_CODES` (malformed escalation arguments — `INVALID_ARGS`; `SANDBOX_ESCALATION_BLOCKED` policy block, `SANDBOX_ESCALATION_REJECTED` human/answerer rejection, `SANDBOX_ESCALATION_CANCELLED` withdrawal, `SANDBOX_ESCALATION_UNAVAILABLE` missing channel).
