...
共享的升权编排（[`approveEscalation`](src/escalation.ts)，bash/pwsh 与文件系统工具族共用）会针对调用的有效模式解析 `sandbox_permissions` 请求：所请求的模式不严格更宽时（相等或更窄——即严格 schema 的冗余字段），调用在有效模式下运行，不发起审批、也不报错；只有严格更宽的请求才会走 `ctx.approval`。每次升权失败都会抛出携带稳定错误码的 `HarnessError`，错误码来自 `ESCALATION_ERROR_CODES`（升权参数格式错误——`INVALID_ARGS`；`SANDBOX_ESCALATION_BLOCKED` 策略阻止、`SANDBOX_ESCALATION_REJECTED` 人类／应答者拒绝、`SANDBOX_ESCALATION_CANCELLED` 撤销、`SANDBOX_ESCALATION_UNAVAILABLE` 通道缺失）。
