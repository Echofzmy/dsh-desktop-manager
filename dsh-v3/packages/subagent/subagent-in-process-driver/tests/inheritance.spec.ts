...
describe('in-process policy inheritance', () => {
  ...
  it('rejects a child escalation deterministically even when an answerer would allow it', async () => {
    ...
    try {
      ...
      expect(toolResultTexts(child).join('\n'))
        .toContain('sandbox escalation to "workspace-write" is blocked by the current approval policy')
      ...
      expect(decided?.data).toMatchObject({ id: asked?.data.id, outcome: 'blocked' })
    } finally {
      await run.dispose()
    }
  })
