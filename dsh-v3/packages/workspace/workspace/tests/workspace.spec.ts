...
describe('registry-global session archive', () => {
  ...
  it('restores the retained workspace position and forgets a permanently deleted id', async () => {
    const dir = await makeDir('archive-lifecycle')
    const result = await harness({ sessions: [header('first', dir, 100), header('gone', dir, 200)] })
    const workspace = result.registry.list()[0]!
    ...
    expect(result.registry.archivedSessionIds).toEqual([])
    expect(workspace.sessionIds).toEqual(original.filter(id => id !== 'gone'))
