...
export interface PersistenceBackend<TornMarker = unknown> {
  ...
  /**
   * Permanently delete one stored session and every backend-owned event row.
   * @param id - stored session identity.
   * @returns whether storage contained the session.
   */
...
export class PersistenceCoordinator<TornMarker = unknown> {
  ...
  /**
   * Permanently delete one cold materialized session. The same per-id chain as
   * appends owns the commit, and prepared snapshots are invalidated before the
   * backend artifact disappears.
   * @param id - persisted session to delete.
   * @returns whether a durable artifact existed.
   */
