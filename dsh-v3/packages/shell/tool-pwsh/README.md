...
| `sandbox_permissions` | string enum | Advertised only when a sandboxing executor is mounted (`ctx.shell.sandboxMode` defined). The wider sandbox mode for a one-shot retry of a command the sandbox just denied — the narrowest wider mode that suffices, requiring `justification` and user approval through `ctx.approval` BEFORE execution. A strictly wider request is resolved through approval; an equal or narrower request is a strict-schema redundant field that runs under the effective mode with no approval and no error; an unapprovable wider request fails closed without running anything. Advertised per agent: an agent whose effective approval policy is `never`, or whose effective mode is already `danger-full-access`, sees the fields and the escalation description clause omitted, and its denial results carry no escalation hint — switching the session mode below `danger-full-access` (or policy back to `ask`) restores both. Execution-time enforcement is unchanged. |
...
Validation and infrastructure failures are normalized as `Error: <message>`. This package's stable messages are `invalid command: expected a non-empty string`, `invalid description: expected a non-empty string`, `invalid timeoutMs: expected a positive number, got <value>`, `invalid escalation: sandbox_permissions requires a justification` (code `INVALID_ARGS`), `invalid escalation: justification is only valid together with sandbox_permissions` (code `INVALID_ARGS`), `invalid justification: expected a non-empty sentence` (code `INVALID_ARGS`), `sandbox_permissions is not available in this composition (no sandboxing executor to escalate)` (code `SANDBOX_ESCALATION_UNAVAILABLE`), the shared escalation failures (blocked by the current approval policy — code `SANDBOX_ESCALATION_BLOCKED` / no approval service — `SANDBOX_ESCALATION_UNAVAILABLE` / no agent to route — `SANDBOX_ESCALATION_UNAVAILABLE` / no approval channel — `SANDBOX_ESCALATION_UNAVAILABLE` / user rejected — `SANDBOX_ESCALATION_REJECTED` / was cancelled — `SANDBOX_ESCALATION_CANCELLED`), `run_in_background is disabled for this deployment (enableRunInBackground: false)`, `background jobs unavailable: load @deepseek-ai/dsh-jobs and @deepseek-ai/dsh-tool-jobs`, and `tool call aborted`.
