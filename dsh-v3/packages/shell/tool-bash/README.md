...
| `sandbox_permissions` | string enum | ADVERTISED ONLY when the mounted executor sandboxes (`ctx.shell.sandboxMode` reports a confining default): the wider mode a denied command needs, from the closed target vocabulary `workspace-write`/`danger-full-access` (never cut down to the executor's default — the effective mode is per-session). Strict widening is checked at execution against the effective mode: a strictly wider request goes through `ctx.approval`, while an equal or narrower request is a strict-schema redundant field that runs under the effective mode with no approval and no error. |
...
Escalating bash calls resolve `ctx.approval` before execution. `allowed-once` applies the requested mode only to that call; policy blocking (`SANDBOX_ESCALATION_BLOCKED`), rejection (`SANDBOX_ESCALATION_REJECTED`), cancellation (`SANDBOX_ESCALATION_CANCELLED`), unavailability (`SANDBOX_ESCALATION_UNAVAILABLE`), or missing approval context executes nothing and returns a distinct error with that stable code. On a real denial, the model may retry the same command once in the same turn with the narrowest sufficient mode and justification; the approval prompt itself is the consent step. Escalation is never speculative, and a disabled or rejected approval is final. The [sandbox Agent Note](../../../.agents/notes/implemented/feature/2026-07-06-sandbox.md) owns the rationale. Schema advertisement is per agent: an agent whose effective approval policy is `never`, or whose effective sandbox mode is already `danger-full-access`, sees the escalation fields and description clause omitted (there is nothing to escalate TO), and its denial results carry no escalation hint — the same predicate gates the model surface and the hint. Switching the session's mode back below `danger-full-access` (or the policy back to `ask`) restores both. Execution-time enforcement is unchanged: schema omission is advertising, never a bypass guard.
...
Validation and policy failures are normalized as `Error: <message>`. This package's stable messages are `invalid command: expected a non-empty string`, `invalid description: expected a non-empty string`, `invalid timeoutMs: expected a positive number, got <value>`, `invalid escalation: sandbox_permissions requires a justification` (code `INVALID_ARGS`), `invalid escalation: justification is only valid together with sandbox_permissions` (code `INVALID_ARGS`), `invalid justification: expected a non-empty sentence` (code `INVALID_ARGS`), `background execution is disabled for this bash tool`, `background jobs unavailable: load @deepseek-ai/dsh-jobs and @deepseek-ai/dsh-tool-jobs`, `sandbox_permissions is not available in this composition (no sandboxing executor to escalate)` (code `SANDBOX_ESCALATION_UNAVAILABLE`), `sandbox escalation to "<mode>" is blocked by the current approval policy` (code `SANDBOX_ESCALATION_BLOCKED`), the approval-availability/rejection/cancellation variants (codes `SANDBOX_ESCALATION_UNAVAILABLE`/`SANDBOX_ESCALATION_REJECTED`/`SANDBOX_ESCALATION_CANCELLED`), and `tool call aborted`. An equal-or-narrower `sandbox_permissions` request is not an error: it runs under the effective mode.
