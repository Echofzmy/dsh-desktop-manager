...
describe('tools/pre-execute gate (native-plugin permission pattern, end-to-end through the loop)', () => {
  it('deny short-circuits dispatch into an isError result the model sees', async () => {
    ...
    // The plain pre-execute deny persists a durable TOOL_POLICY_DENIED code
    // through the loop's tool/result event (error.info passes through
    // unchanged).
    expect(result?.type === 'tool/result' && result.data.error)
      .toEqual({ name: 'HarnessError', code: 'TOOL_POLICY_DENIED' })
  })

  it('a post-execute block persists the TOOL_POST_BLOCKED code through tool/result', async () => {
    const adapter = new MockAdapter([toolCallResponse('c1', 'echo', { text: 'hi' }), textResponse('ok')])
    const ctx = await harness(adapter)
    ctx.tools.register(defineContentToolFixture({
      name: 'echo', description: 'echo', parameters: { text: { type: 'string' } },
      async execute(args) { return [{ type: 'text', text: String(args.text) }] },
    ...
    ctx.on('tools/post-execute', async (_exec, _result, _next): Promise<PostToolDecision> => ({
      kind: 'block',
      feedback: [{ type: 'text', text: 'corrective block' }],
    }))
    ...
    expect(result?.type === 'tool/result' && result.data.message.content[0].isError).toBe(true)
    expect(result?.type === 'tool/result'
      && result.data.message.content[0].content.some(b => b.type === 'text' && b.text.includes('corrective block'))).toBe(true)
    expect(result?.type === 'tool/result' && result.data.error)
      .toEqual({ name: 'HarnessError', code: 'TOOL_POST_BLOCKED' })
  })
})
